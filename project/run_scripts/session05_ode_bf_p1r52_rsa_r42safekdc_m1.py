#!/usr/bin/env python3
"""Fail-closed P1R52 full-cell runner."""
...
from project.run_scripts.ode_bf.p1r52_independent_panel import LOCK_FILE, PARENT, load_and_validate_lock
from project.run_scripts.ode_bf.p1r52_independent_runtime import ARMS, INSTRUCTION_ID


RUN_TOKEN = "p1r52-rsa-r42safekdc-m1-v1"
SOURCE_MANIFEST = "source_manifest_s05_p1r52_rsa_r42safekdc_m1.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(["git", "merge-base", "--is-ancestor", PARENT, source_head], cwd=REPO_ROOT, check=False).returncode != 0:
        raise ValueError("P1R52 source ancestry differs")
    manifest_path = REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST
    manifest, raw_sha = load_rooted_json(manifest_path, expected_schema="ode-edit-s05-p1r52-rsa-r42safekdc-source-manifest/v1")
    ...
    if manifest.get("instruction_id") != INSTRUCTION_ID or manifest.get("parent") != PARENT or not isinstance(entries, list) or not entries:
        raise ValueError("P1R52 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P1R52 source entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file() or path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P1R52 source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P1R52 source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = {
            **result,
            "p1r52_lock_sha256": lock_sha,
            "p1r52_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r52-rsa-r42safekdc-job-failure/v1",
        )
