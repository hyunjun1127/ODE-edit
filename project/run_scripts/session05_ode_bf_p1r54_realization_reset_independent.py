#!/usr/bin/env python3
"""Fail-closed launcher for one B2-B10 independent reset cell."""
...
from project.run_scripts.session05_ode_bf_p1r54_realization_reset import (
    _owned_runtime_root,
)
from project.run_scripts.session05_ode_bf_p1r54_realization_reset_independent_preflight import (
    ARRAY_THROTTLE,
    EXTRACT_ROOT,
    JOB_GPU_COUNT,
    PRIOR_HF_GATE,
...
RUN_TOKEN = "p1r54-realization-reset-independent-b2b10-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("P1R54 reset independent pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema")
        != "ode-edit-s05-p1r54-realization-reset-independent-final-pre-gpu/v1"
        ...
        or value.get("array_throttle") != ARRAY_THROTTLE
        or value.get("required_array") != REQUIRED_ARRAY
        or value.get("source_manifest") != source_manifest(source_head, source_tree)
    ):
        raise ODEBFContractError("P1R54 reset independent pre-GPU content differs")
...
def main() -> int:
    ...
    config = independent_expansion_cell(args.cell)
    ...
    try:
        ...
        if (
            ...
            or args.output_root.name != config.result_name
        ):
            raise ODEBFContractError("P1R54 reset independent source/output differs")
        state_root = _owned_runtime_root(
            args.state_root,
            parent=REPO_ROOT / "local/odebf/state",
            expected_name=config.result_name,
        )
        _owned_runtime_root(
            args.log_root,
            parent=REPO_ROOT / "local/odebf/logs",
            expected_name=config.result_name,
        )
        final = _load_final(
            args.final_pre_gpu_receipt, source_head=head, source_tree=tree
        )
        plan = final["dry_plan"]["cells"][args.cell]
        if (
            plan["role"] != config.role
            or plan["result_name"] != config.result_name
            or plan["batch_index"] != config.batch_index
        ):
            raise ODEBFContractError("P1R54 reset independent released cell differs")
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("P1R54 reset independent deployment differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r54-realization-reset-independent-cell-timing/v1",
        ...
        result.update({
            ...
            "state_terminal_sha256": _atomic_write_once(
                state_root / "terminal.json",
                {
                    "schema": "ode-edit-s05-p1r54-realization-reset-independent-cell-state/v1",
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r54-realization-reset-independent-cell-failure/v1",
        )
        if state_root is not None:
            _atomic_write_once(state_root / "failure.json", {
                "schema": "ode-edit-s05-p1r54-realization-reset-independent-cell-state/v1",
                "status": "FAIL_CLOSED",
                "cell": config.cell,
                "role": config.role,
