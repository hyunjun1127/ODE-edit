"""Independent B10x10 P1R43 rho-free semantic-first Atomic runtime."""
...
B1_METHODS = (
    "PR-P1R43-B1-RHO-FREE-SEMANTIC-FIRST-NEUTRAL",
    "PR-P1R43-B1-RHO-FREE-SEMANTIC-FIRST-SOFT",
)
CASE_COUNT = 10
HISTORY_MODE = "OFF"
...
def expected_p1r43_independent_result_name(
    alias: str,
    method: str,
    *,
    ...
    if method not in METHODS + B1_METHODS:
        raise ODEBFContractError("P1R43 independent method differs")
...
def run_p1r43_independent_b10x10(
    ...
    *,
    ...
    if method not in METHODS + B1_METHODS or len(stream_batches) != CASE_COUNT:
        raise ODEBFContractError("P1R43 independent B10 matrix/count differs")
    if any(len(batch) != BATCH_SIZE for batch in stream_batches):
        raise ODEBFContractError("P1R43 population is not ten B10 batches")
    if stream.get("root_digest") != STREAM_ROOT:
        raise ODEBFContractError("P1R43 frozen stream root differs")
    if stream.get("all_request_order_sha256") != STREAM_ORDER:
        raise ODEBFContractError("P1R43 frozen stream order differs")

    expected_w0 = _model_w0_contract(touched)
    if _hashes(touched) != dict(base_receipt.parameter_sha256):
        raise ODEBFStateError("P1R43 entry W0 differs")
    arm_index = 0 if method.endswith("-NEUTRAL") else 1
    executed_method = METHODS[arm_index]
    ...
    for case_index, requests in enumerate(executed_batches, start=1):
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R43 cross-case W0 state leak detected")
        try:
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R43 post-case W0 differs")
        stages.record(
            f"post_p1r43_independent_b10_case_{case_index}",
            {
                "case_index": case_index,
                "method": method,
    ...
    terminal = {
        "schema": "ode-edit-s05-p1r43-rho-free-independent-b10x10-terminal/v1",
        ...
        "completed": completed,
        "failed_case_identity_sha256": [
            item["identity_sha256"] for item in failed
        ],
        "history_mode": _history_off_receipt(),
        "cross_case_state_count": 0,
        "target_controller_state_reset_count": 1 if smoke else CASE_COUNT,
        "retry_count": 0,
        "rng_reset_count": 1 if smoke else CASE_COUNT,
    ...
    manifest = {
        "schema": "ode-edit-s05-p1r43-rho-free-independent-b10x10-manifest/v1",
        "source_head": source_head,
    ...
    return {
        "status": "P1R43_RHO_FREE_INDEPENDENT_B10X10_TERMINAL",
        "terminal_sha256": terminal_sha,
