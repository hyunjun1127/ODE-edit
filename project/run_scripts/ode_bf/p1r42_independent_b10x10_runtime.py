"""Independent B10x10 P1R42 objective-aligned Atomic runtime."""
...
def expected_p1r42_independent_result_name(
    alias: str,
    method: str,
    *,
    ...
    if method not in METHODS + B1_METHODS:
        raise ODEBFContractError("P1R42 independent method differs")
...
def run_p1r42_independent_b10x10(
    ...
    *,
    ...
    if method not in METHODS + B1_METHODS or len(stream_batches) != CASE_COUNT:
        raise ODEBFContractError("P1R42 independent B10 matrix/count differs")
    if any(len(batch) != BATCH_SIZE for batch in stream_batches):
        raise ODEBFContractError("P1R42 population is not ten B10 batches")
    if stream.get("root_digest") != STREAM_ROOT:
        raise ODEBFContractError("P1R42 frozen stream root differs")
    if stream.get("all_request_order_sha256") != STREAM_ORDER:
        raise ODEBFContractError("P1R42 frozen order differs")

    expected_w0 = _model_w0_contract(touched)
    if _hashes(touched) != dict(base_receipt.parameter_sha256):
        raise ODEBFStateError("P1R42 entry W0 differs")
    ...
    for case_index, requests in enumerate(executed_batches, start=1):
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R42 cross-case W0 state leak detected")
        try:
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R42 post-case W0 differs")
        stages.record(
            f"post_p1r42_independent_b10_case_{case_index}",
            {
                "case_index": case_index,
                "method": method,
    ...
    terminal = {
        "schema": "ode-edit-s05-p1r42-objective-alignment-independent-b10x10-terminal/v1",
    ...
    manifest = {
        "schema": "ode-edit-s05-p1r42-objective-alignment-independent-b10x10-manifest/v1",
        "source_head": source_head,
    ...
    return {
        "status": "P1R42_OBJECTIVE_ALIGNMENT_INDEPENDENT_B10X10_TERMINAL",
        "terminal_sha256": terminal_sha,
