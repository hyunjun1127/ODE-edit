...
from .p1r52_sequential_scale import (
...
from .p1r52_joint_pc_runtime import (
    ...
    PILOT_TECH_R5_ROLE as JOINT_PC_PILOT_TECH_R5_ROLE,
    PILOT_TECH_R6_ROLE as JOINT_PC_PILOT_TECH_R6_ROLE,
...
from .p1r52_joint_pc_fp32_runtime import (
    RESULT_NAME as JOINT_PC_FULL_FP32_RESULT_NAME,
    ROLE as JOINT_PC_FULL_FP32_ROLE,
    run_joint_pc_full_fp32_b100,
)
from .p1r52_joint_pc_independent_fp32_runtime import (
    ROLES as JOINT_PC_INDEPENDENT_FP32_ROLES,
    expected_result_name as expected_joint_pc_independent_fp32_result_name,
...
from .p1r52_target_timescale_b100 import (
    ROLES as TARGET_TIMESCALE_ROLES,
    expected_result_name as expected_target_timescale_result_name,
    run_target_timescale_b100,
)
from .p1r52_target_timescale_native_reference import (
    ROLES as TARGET_TIMESCALE_NATIVE_REFERENCE_ROLES,
...
from .p1r54_energyfree_localz_b100 import (
    ROLES as P1R54_ENERGYFREE_LOCALZ_ROLES,
    expected_result_name as expected_p1r54_energyfree_localz_result_name,
    run_p1r54_energyfree_localz_b100,
)
from .p1r54_pdz_ablation_b100 import (
    ROLES as P1R54_PDZ_ABLATION_ROLES,
    expected_result_name as expected_p1r54_pdz_ablation_result_name,
    run_p1r54_pdz_ablation_b100,
)
from .p1r54_fz_c3_sequential import (
    ROLE as P1R54_FZ_SEQUENTIAL_ROLE,
    expected_result_name as expected_p1r54_fz_sequential_result_name,
    run_p1r54_fz_c3_sequential,
)
from .p1r54_fz_c3_independent import (
    ROLES as P1R54_FZ_INDEPENDENT_ROLES,
...
from .p1r54_fz_finalz_oneshot_sequential import (
    ROLE as P1R54_FZ_FINALZ_ONESHOT_ROLE,
    expected_result_name as expected_p1r54_fz_finalz_oneshot_result_name,
    run_p1r54_fz_finalz_oneshot_sequential,
)
from .p1r55_rms_pdz_rate_experiment import (
    PHASE1_ROLES as P1R55_RMS_PDZ_RATE_PHASE1_ROLES,
    expected_phase1_result_name as expected_p1r55_rms_pdz_rate_result_name,
...
RESULT_NAMES_B100X10_ACCEPTED_Z_OBS_TECH_R1_R52 = {
    role: name.removesuffix("-v1") + "-tech-r1-r52-v1"
    for role, name in RESULT_NAMES_B100X10_ACCEPTED_Z_OBS.items()
}
...
def expected_p1r52_sequential_result_name(
    ...
    *,
    ...
    if role in P1R55_RMS_PDZ_RATE_PHASE1_ROLES:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("P1R55 RMS/PDZ-rate result scope differs")
        return expected_p1r55_rms_pdz_rate_result_name(role)
    if role == P1R54_FZ_FINALZ_ONESHOT_ROLE:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("P1R54 FZ final-z result scope differs")
        return expected_p1r54_fz_finalz_oneshot_result_name(role)
    ...
    if role in P1R54_FZ_INDEPENDENT_ROLES:
        if (
            ...
            raise ODEBFContractError("P1R54 FZ independent result scope differs")
        return expected_p1r54_fz_independent_result_name(role)
    if role in P1R54_PDZ_ABLATION_ROLES:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("P1R54 PDZ ablation result scope differs")
        return expected_p1r54_pdz_ablation_result_name(role)
    if role == P1R54_FZ_SEQUENTIAL_ROLE:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("P1R54 FZ sequential result scope differs")
        return expected_p1r54_fz_sequential_result_name(role)
    if role in P1R54_ENERGYFREE_LOCALZ_ROLES:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("P1R54 energy-free Local-Z result scope differs")
        return expected_p1r54_energyfree_localz_result_name(role)
    ...
    if role in TARGET_TIMESCALE_NATIVE_REFERENCE_ROLES:
        if (
            ...
            raise ODEBFContractError("target-timescale Native result scope differs")
        return expected_target_timescale_native_reference_result_name(role)
    if role in TARGET_TIMESCALE_ROLES:
        if (
            alias != "llama3-8b-inst"
            or scale != P1R52_B100X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("target-timescale result scope differs")
        return expected_target_timescale_result_name(role)
    ...
    if role in JOINT_PC_INDEPENDENT_FP32_ROLES:
        if (
            ...
            raise ODEBFContractError("independent FP32 joint P/C result scope differs")
        return expected_joint_pc_independent_fp32_result_name(role)
    ...
    if role == JOINT_PC_FULL_FP32_ROLE:
        if (
            ...
            raise ODEBFContractError("FP32 joint P/C result scope differs")
        return JOINT_PC_FULL_FP32_RESULT_NAME
    if role in (JOINT_PC_PILOT_ROLE, JOINT_PC_PILOT_TECH_R1_ROLE, JOINT_PC_PILOT_TECH_R2_ROLE, JOINT_PC_PILOT_TECH_R3_ROLE, JOINT_PC_PILOT_TECH_R4_ROLE, JOINT_PC_PILOT_TECH_R5_ROLE, JOINT_PC_PILOT_TECH_R6_ROLE) or role.startswith(JOINT_PC_PRODUCTION_ROLE_PREFIX):
        if (
            alias != "llama3-8b-inst"
            or scale == P1R52_B10X10_SCALE
            or attempt_suffix is not None
        ):
            raise ODEBFContractError("joint P/C result scope differs")
        return expected_joint_pc_result_name(role)
    ...
    *,
    ...
    if role in P1R55_RMS_PDZ_RATE_PHASE1_ROLES:
        if (
            ...
            raise ODEBFContractError("P1R55 RMS/PDZ-rate runtime scope differs")
        return run_p1r55_rms_pdz_rate(
    ...
    if role == P1R54_FZ_FINALZ_ONESHOT_ROLE:
        if (
            ...
            raise ODEBFContractError("P1R54 FZ final-z runtime scope differs")
        return run_p1r54_fz_finalz_oneshot_sequential(
            model, tokenizer, alias=alias, role=role, destination=destination,
            raw_root=raw_root, stages=stages, source_head=source_head,
            stream_batches=stream_batches, stream=stream, hparams=hparams,
            projector=projector, contexts=contexts,
    ...
    if role in P1R54_FZ_INDEPENDENT_ROLES:
        if (
            ...
            raise ODEBFContractError("P1R54 FZ independent runtime scope differs")
        return run_p1r54_fz_c3_independent(
            model, tokenizer, alias=alias, role=role, destination=destination,
            raw_root=raw_root, stages=stages, source_head=source_head,
            stream_batches=stream_batches, stream=stream, hparams=hparams,
            projector=projector, contexts=contexts,
    ...
    if role in P1R54_PDZ_ABLATION_ROLES:
        if (
            ...
            raise ODEBFContractError("P1R54 PDZ ablation runtime scope differs")
        return run_p1r54_pdz_ablation_b100(
            model, tokenizer, alias=alias, role=role, destination=destination,
            raw_root=raw_root, stages=stages, source_head=source_head,
            stream_batches=stream_batches, stream=stream, hparams=hparams,
            projector=projector, contexts=contexts,
    ...
    if role == P1R54_FZ_SEQUENTIAL_ROLE:
        if (
            ...
            raise ODEBFContractError("P1R54 FZ sequential runtime scope differs")
        return run_p1r54_fz_c3_sequential(
            model, tokenizer, alias=alias, role=role, destination=destination,
            raw_root=raw_root, stages=stages, source_head=source_head,
            stream_batches=stream_batches, stream=stream, hparams=hparams,
            projector=projector, contexts=contexts,
    ...
    if role in P1R54_ENERGYFREE_LOCALZ_ROLES:
        if (
            ...
            raise ODEBFContractError("P1R54 energy-free Local-Z runtime scope differs")
        return run_p1r54_energyfree_localz_b100(
            model, tokenizer, alias=alias, role=role, destination=destination,
            raw_root=raw_root, stages=stages, source_head=source_head,
            stream_batches=stream_batches, stream=stream, hparams=hparams,
            projector=projector, contexts=contexts,
    ...
    if role in TARGET_TIMESCALE_NATIVE_REFERENCE_ROLES:
        if (
            ...
            raise ODEBFContractError("target-timescale Native runtime scope differs")
        return run_target_timescale_native_reference_b100(
            model,
            tokenizer,
            alias=alias,
            role=role,
    ...
    if role in TARGET_TIMESCALE_ROLES:
        if (
            ...
            raise ODEBFContractError("target-timescale runtime scope differs")
    ...
    if role in JOINT_PC_INDEPENDENT_FP32_ROLES:
        if (
            ...
            raise ODEBFContractError("independent FP32 joint P/C runtime scope differs")
        return run_joint_pc_independent_full_fp32(
            model,
            tokenizer,
            alias=alias,
            role=role,
    ...
    if role == JOINT_PC_FULL_FP32_ROLE:
        if (
            ...
            raise ODEBFContractError("FP32 joint P/C runtime scope differs")
        return run_joint_pc_full_fp32_b100(
            model,
            tokenizer,
            alias=alias,
            role=role,
    ...
    if role in (JOINT_PC_PILOT_ROLE, JOINT_PC_PILOT_TECH_R1_ROLE, JOINT_PC_PILOT_TECH_R2_ROLE, JOINT_PC_PILOT_TECH_R3_ROLE, JOINT_PC_PILOT_TECH_R4_ROLE, JOINT_PC_PILOT_TECH_R5_ROLE, JOINT_PC_PILOT_TECH_R6_ROLE) or role.startswith(JOINT_PC_PRODUCTION_ROLE_PREFIX):
        if (
            ...
            raise ODEBFContractError("joint P/C runtime scope differs")
        return run_joint_pc_case(
            model,
            tokenizer,
            alias=alias,
            role=role,
