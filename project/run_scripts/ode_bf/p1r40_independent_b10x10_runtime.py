"""Ten independent B10 P1R40 Atomic cases for one model/router cell."""
...
def run_p1r40_independent_b10x10(
    ...
    *,
    ...
    if method not in METHODS or len(stream_batches) != CASE_COUNT:
        raise ODEBFContractError("P1R40 independent B10 matrix/count differs")
    if any(len(batch) != BATCH_SIZE for batch in stream_batches):
        raise ODEBFContractError("P1R40 population is not ten B10 batches")
    if stream.get("root_digest") != STREAM_ROOT:
        raise ODEBFContractError("P1R40 frozen stream root differs")
    if stream.get("all_request_order_sha256") != STREAM_ORDER:
        raise ODEBFContractError("P1R40 frozen order differs")
    expected_w0 = _model_w0_contract(touched)
    if _hashes(touched) != dict(base_receipt.parameter_sha256):
        raise ODEBFStateError("P1R40 entry W0 differs")
    ...
    for case_index, requests in enumerate(stream_batches, start=1):
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R40 cross-case W0 state leak detected")
        try:
        ...
        if _model_w0_contract(touched) != expected_w0:
            raise ODEBFStateError("P1R40 post-case W0 differs")
        stages.record(
            f"post_p1r40_independent_b10_case_{case_index}",
            {
                "case_index": case_index,
                "method": method,
    ...
    terminal = {
        "schema": "ode-edit-s05-p1r40-independent-b10x10-terminal/v1",
    ...
    manifest = {
        "schema": "ode-edit-s05-p1r40-independent-b10x10-manifest/v1",
        "source_head": source_head,
    ...
    return {
        "status": "P1R40_SEMANTIC_DEFICIT_VELOCITY_DECAY_B10X10_TERMINAL",
        "terminal_sha256": terminal_sha,
