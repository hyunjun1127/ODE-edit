...
class P3R1SourceLockTest(unittest.TestCase):
    def test_tech_r3_namespace_is_shared_by_dispatcher_and_runtime(self) -> None:
        root = Path(__file__).resolve().parents[4]
        role = case_role(1)
        parent = expected_result_parent(root)
        self.assertEqual(
            parent,
            (root / "local/odebf/results/p3r1-two-timescale-fastz-fh-c013-fp32-tech-r3").resolve(
                strict=False
            ),
        )
        self.assertEqual(
            expected_result_name("llama3-8b-inst", role),
            "s05-p3r1-two-timescale-fastz-fh-c013-fp32-tech-r3-llama3-8b-inst-case-01-v1",
        )
        dispatcher = (root / "project/run_scripts/session05_ode_bf_p3r1_fastz_fh.sbatch").read_text(
            encoding="utf-8"
        )
        self.assertIn("p3r1-two-timescale-fastz-fh-c013-fp32-tech-r3-${ALIAS}", dispatcher)
