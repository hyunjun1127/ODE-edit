"""Single-arm Stage-A rollout for the P1R16 integrated method repair.
...
from .coupled_demand_soft_transport_runtime import (
...
P1R16_STAGE_A_R13_CHECKPOINT = (
    "e6facd2d5dfae12d3c094b51981ad99951174109"
)
P1R16_STAGE_A_KIND = "OUTCOME_SELECTED_R13_MECHANISTIC_REGRESSION"


class P1R16OutputRootCollision(FileExistsError):
...
def _atomic_write_once(path: Path, value: Mapping[str, Any]) -> str:
    if path.exists() or path.is_symlink():
        raise ODEBFStateError("P1R16 raw-free receipt is create-once")
    ...
    if temporary.exists() or temporary.is_symlink():
        raise ODEBFStateError("P1R16 temporary receipt collides")
    try:
...
class P1R16StageRecorder:
    ...
    def record(self, name: str, payload: Mapping[str, Any]) -> str:
        if not name or "/" in name or ".." in name:
            raise ODEBFContractError("P1R16 stage name differs")
        path = self.root / f"stage-{self.ordinal:04d}-{name}.json"
        value = {
            "schema": f"{P1R16_SCHEMA}-stage",
...
class P1R16StageARolloutResult:
    ...
    def raw_free_payload(self) -> dict[str, Any]:
        return {
            "schema": f"{P1R16_SCHEMA}-stage-a-rollout",
            "method_id": P1R16_METHOD_ID,
            "stage_a_kind": P1R16_STAGE_A_KIND,
            "stage_a_seal_root": P1R16_STAGE_A_R13_SEAL_ROOT,
            ...
            "target_clock_count": self.target_clock_count,
            "weight_clock_count": self.weight_clock_count,
            "cold_target_only_recovery_count": (
                self.cold_target_only_recovery_count
            ),
...
def _allocation_receipt(
    ...
    if (
        ...
        raise ODEBFContractError("P1R16 allocation identity differs")
...
def _partial_compute_receipt(
    ...
    *,
    ...
    payload = {
        "schema": f"{P1R16_SCHEMA}-partial-compute-ledger",
...
def run_p1r16_stage_a_virtual_rollout(
    ...
    *,
...
) -> P1R16StageARolloutResult:
    """Run one P1R16 Dynamic target trajectory without parameter mutation."""
    ...
    if (
        ...
        or order != P1R16_STAGE_A_R13_REQUEST_ORDER
        ...
        raise ODEBFContractError("P1R16 Stage-A rollout inventory differs")
    ...
    target_clock = 0
    weight_clock = 0
    cold_recovery_count = 0
    attempted_full = 0
    target_groups = 0
    terminal_status = "P1R16_FIXED_E8_TAU_COMPLETE"
    terminal_component = "NONE"
    for step in range(PHYSICAL_WRITER_GRID_COUNT):
        cumulative = accumulator.factors_by_weight()
        keys, current, residual, desired, factor_demand, demand_receipt, target_group = (
            perrequest_dynamic_target_factor_group(
                model,
                tokenizer,
                batch,
        ...
        ledger.record(
            ...
            physical_microbatch_graphs=target_group.physical_target_graphs,
        ...
        if target_group.target_step.status is TargetStepStatus.TARGET_NO_DESCENT_DIRECTION:
            ...
            boundary = {
                "schema": f"{P1R16_SCHEMA}-target-boundary",
                "step_index": step,
                "tau_before": target_clock * PHYSICAL_WRITER_H,
        ...
        field, field_build = build_integrated_physical_field_from_keys(
            ...
            precomputed_residual=factor_demand,
        )
        if field_build.residual_sha256 != tensor_sha256(factor_demand):
            raise ODEBFContractError("P1R16 coupled field demand identity differs")
        writer_micro = tuple(
            writer_edit_transport_vjp_microbatch(
                model,
                tokenizer,
        ...
        def observe(value: Mapping[str, Any]) -> None:
            if pre_guard:
                raise ODEBFContractError("P1R16 pre-guard receipt repeated")
            pre_guard.update(dict(value))

        routing = solve_coupled_demand_soft_transport(
            technical.problem,
            a_edit=writer.a_edit,
            a_transport=writer.a_transport,
            current_nohook_loss=writer.loss_new,
            controller_requested_reduction=(
                target_group.controller_requested_reduction
            ),
            ...
            cold_entry=(step == 0 and target_clock == 0 and weight_clock == 0),
            target_equals_z_base=torch.equal(target, z_base),
            old_residual_exact_zero=(int(torch.count_nonzero(residual)) == 0),
            pre_guard_observer=observe,
        )
        if not pre_guard:
            raise ODEBFContractError("P1R16 pre-guard receipt is absent")
        field_payload = {
            "schema": f"{P1R16_SCHEMA}-field",
            "step_index": step,
            "tau_before": target_clock * PHYSICAL_WRITER_H,
            "target_factor": target_group.raw_free_payload(),
            "coupled_demand": demand_receipt.raw_free_payload(),
        ...
        if routing.status is CoupledStepStatus.EDIT_NO_DIRECTION:
            terminal_status = routing.status.value
            terminal_component = routing.failure_component.value
            break
        if routing.status is CoupledStepStatus.COLD_TARGET_ONLY_RECOVERY:
            if cold_recovery_count != 0 or step != 0:
                raise ODEBFContractError("P1R16 cold recovery repeated")
            cold_recovery_count = 1
            target_clock += 1
            target = desired.detach().to(
                device="cpu", dtype=torch.float32
            ).contiguous()
            cumulative_g_path = target_group.target_step.cumulative_g_path_after
            transition = {
                "schema": f"{P1R16_SCHEMA}-transition",
                "step_index": step,
                "tau_after": target_clock * PHYSICAL_WRITER_H,
        ...
        factor_receipt = accumulator.append(
            field, routing.applied_theta, step_index=weight_clock
        )
        target_clock += 1
        weight_clock += 1
        ...
        if write_stage is not None:
            write_stage(f"transition-{step + 1:04d}", transition)
    if weight_clock == PHYSICAL_WRITER_GRID_COUNT:
    ...
    snapshot_sha = canonical_hash(
        {
            ...
            "weight_clock": weight_clock,
            "cold_target_only_recovery_count": cold_recovery_count,
            "attempted_full": attempted_full,
            "target_groups": target_groups,
    ...
    if write_stage is not None:
        write_stage(
            "action-freeze",
            {
                "schema": f"{P1R16_SCHEMA}-action-freeze",
                **asdict(freeze),
                "terminal_component": terminal_component,
    ...
    return P1R16StageARolloutResult(
        target,
        ...
        terminal_status,
        terminal_component,
        target_clock * PHYSICAL_WRITER_H,
        tuple(fields),
        tuple(transitions),
        compute,
        freeze,
...
def _validate_source_freeze(repo_root: Path, source_head: str) -> None:
    if len(source_head) != 40 or any(
        ...
        raise ODEBFContractError("P1R16 source head differs")
    ...
    if observed != source_head or tracked:
        raise ODEBFContractError("P1R16 source freeze differs")


def run_p1r16_stage_a_experiment(
    *,
    repo_root: Path,
    alias: str,
    output_root: Path,
    ...
    from .cold_start_target import cold_lookup_positions
    ...
    from .p1_coupled_demand_soft_transport_panel import (
    ...
    if alias not in MODEL_ALIASES:
        raise ODEBFContractError("P1R16 experiment alias differs")
    ...
    if destination.exists() or destination.is_symlink():
        raise P1R16OutputRootCollision("P1R16 result root is create-once")
    try:
        destination.mkdir(mode=0o700, parents=True)
    except FileExistsError as exc:
        raise P1R16OutputRootCollision("P1R16 result root is create-once") from exc
    stages_root = destination / "raw" / "stages"
    stages_root.mkdir(mode=0o700, parents=True)
    stages = P1R16StageRecorder(stages_root)
    started = time.time()
    ...
    stage_a_seal, stage_a_seal_sha = load_and_validate_p1r16_stage_a_seal(locks)
    numerical, numerical_sha = load_and_validate_p1r16_numerical_lock(
        locks / P1R16_NUMERICAL_LOCK_FILE
    )
    source_closure = validate_p1r16_source_closure(root)
    source_manifest, source_manifest_sha = load_and_validate_p1r16_source_manifest(
        root,
        locks / P1R16_SOURCE_MANIFEST_FILE,
    ...
    anchor_seal, anchor_seal_sha = load_rooted_json(
        locks / P1R16_P_ANCHOR_SEAL_FILE
    )
    anchor_seal = verify_r14_fresh_seal(
        anchor_seal, exclusion_root=historical["root_digest"]
    )
    if anchor_seal["root_digest"] != P1R16_P_ANCHOR_SEAL_ROOT:
        raise ODEBFContractError("P1R16 P-anchor seal differs")
    forecast = p1r16_stage_a_forecast(alias)
    if not forecast.fits:
        raise ODEBFContractError("P1R16 resource forecast differs")
    ...
    if len(anchor_groups) != PHYSICAL_WRITER_MICROBATCH_GROUPS:
        raise ODEBFContractError("P1R16 P-anchor grouping differs")
    stages.record(
        "preflight",
        {
            "source_head": source_head,
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise ODEBFContractError("P1R16 runtime requires one visible GPU")
    ...
    if int(free_bytes) < 32 * 1024**3 or int(allocatable_bytes) < 32 * 1024**3:
        raise ODEBFContractError("P1R16 live GPU capacity differs")
    stages.record(
        "runtime-capacity",
        {
    ...
    if tuple(len(group) for group in contexts) != (1, 5):
        raise ODEBFContractError("P1R16 context geometry differs")
    expected_context_sha = expected_p1r16_stage_a_context_sha256(
        stage_a_seal, alias
    )
    if context_sha != expected_context_sha:
        raise ODEBFContractError("P1R16 Stage-A R13 context identity differs")
    target_layer_name = hparams.layer_module_tmp.format(int(hparams.layers[-1]))
    order = ordered_request_digest_v1(
        [str(item["request_sha256"]) for item in requests]
    )
    if order != P1R16_STAGE_A_R13_REQUEST_ORDER:
        raise ODEBFContractError("P1R16 runtime request order differs")
    ...
    canonical_lookup_positions = tuple(
        int(lookup_positions[row * PHYSICAL_WRITER_MICROBATCH_GROUPS])
        for row in range(BATCH_SIZE)
    )
    z_base, z_base_receipt = capture_canonical_b10_z_base(
        model,
        tokenizer,
        requests,
        target_layer_name=target_layer_name,
        canonical_lookup_positions=canonical_lookup_positions,
        canonical_context_template=str(contexts[0][0]),
    )
    stages.record(
        "model-context-entry",
        {
    ...
    if (
        ...
        raise ODEBFContractError("P1R16 projector tensor differs")
    covariance_registry = PinnedCovarianceRegistry(
    ...
    rollout = run_p1r16_stage_a_virtual_rollout(
        model,
        tokenizer,
        requests=requests,
        contexts=contexts,
    ...
    if any(tensor_sha256(touched[name]) != digest for name, digest in w0_sha.items()):
        raise ODEBFStateError("P1R16 virtual rollout changed W0")
    ...
    if final_w0_sha != w0_sha:
        raise ODEBFStateError("P1R16 final W0 restore differs")
    ...
    result = {
        "schema": f"{P1R16_SCHEMA}-stage-a-terminal",
        "status": "P1R16_STAGE_A_EXPERIMENT_TERMINAL",
        "model_alias": alias,
        "source_head": source_head,
        "method_id": P1R16_METHOD_ID,
        "stage_a_kind": P1R16_STAGE_A_KIND,
        ...
        "stage_b_access_count": 0,
        "accepted_transition_count": rollout.accepted_transition_count,
        "target_clock_count": rollout.target_clock_count,
        "weight_clock_count": rollout.weight_clock_count,
        "cold_target_only_recovery_count": (
            rollout.cold_target_only_recovery_count
        ),
    ...
    manifest = {
        "schema": f"{P1R16_SCHEMA}-stage-a-result-manifest",
