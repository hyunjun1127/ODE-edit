...
def prepare_p1r52_target_proposal(
    ...
    *,
    ...
    if not bool(torch.isfinite(semantic).all()) or not bool(torch.isfinite(aggregate_kl).all()):
    ...
    preservation = (lock.kl_factor * aggregate_kl + decay_gradient) * request_count
    ...
    if bool(torch.any(active)):
        kdc_direction[:, active] = combined_direction[:, active] / (
            combined_norm[active] + P1R52_NUMERICAL_EPSILON
        ).unsqueeze(0)
    ...
    pre_clamp_energy_by_request = torch.square(torch.linalg.vector_norm(nominal_delta / P1R24_H, dim=0))
    post_clamp_energy_by_request = torch.square(torch.linalg.vector_norm(actual_delta / P1R24_H, dim=0))
    clamp_removed_energy_by_request = torch.clamp(
        pre_clamp_energy_by_request - post_clamp_energy_by_request, min=0.0
    )
    pre_clamp_energy = float(torch.sum(pre_clamp_energy_by_request))
    post_clamp_energy = float(torch.sum(post_clamp_energy_by_request))
    rsa_shares = (
    ...
    receipt: dict[str, Any] = {
        "schema": "ode-edit-s05-p1r52-rsa-r42safekdc-primary-target/v1",
        ...
        "pre_clamp_energy_by_request": [float(item) for item in pre_clamp_energy_by_request],
        "post_clamp_energy_by_request": [float(item) for item in post_clamp_energy_by_request],
        "clamp_removed_energy_by_request": [float(item) for item in clamp_removed_energy_by_request],
        "pre_clamp_energy": pre_clamp_energy,
        "post_clamp_energy": post_clamp_energy,
        "clamp_removed_energy": float(torch.sum(clamp_removed_energy_by_request)),
        ...
        "target_kl_decision_influence_count": int(bool(torch.any(preservation_norm > 0.0))),
        "target_decay_decision_influence_count": int(bool(torch.any(nonzero_decay))),
    ...
    return P1R52TargetProposal(step, next_state, semantic, actual_delta, step.receipt)
...
def prepare_p1r52_rescue_proposal(
    ...
    *,
    ...
    receipt = {
        **dict(parent.receipt),
        "schema": "ode-edit-s05-p1r52-rsa-r42safekdc-scalar-rescue-plan/v1",
        "instruction_id": P1R52_INSTRUCTION_ID,
        "method_id": P1R52_METHOD_ID,
    ...
    if rescue_step is not None:
        step_receipt = {
            **dict(rescue_step.receipt),
            "schema": "ode-edit-s05-p1r52-rsa-r42safekdc-scalar-rescue-target/v1",
            "instruction_id": P1R52_INSTRUCTION_ID,
            "method_id": P1R52_METHOD_ID,
        }
    ...
    *,
    ...
    receipt = {
        **dict(selected.receipt),
        "schema": "ode-edit-s05-p1r52-rsa-r42safekdc-target-selection/v1",
        "instruction_id": P1R52_INSTRUCTION_ID,
        "method_id": P1R52_METHOD_ID,
