...
def _case_failure(
    ...
    *,
    ...
    classification = (
        "SCIENTIFIC_FAIL"
        if isinstance(exc, P1R34NonSemanticTargetMove)
        else "TECHNICAL_FAIL"
    )
    prefixes = sorted(
    ...
    payload = {
        "schema": (
            "ode-edit-s05-p1r41-trust-clipped-independent-b10-case-failure/v1"
            if p1r41
            else "ode-edit-s05-p1r39-normalized-gradient-independent-b10-case-failure/v1"
            if p1r39
            else
...
def _case_freeze(
    *,
    ...
    payload = {
        "schema": (
            "ode-edit-s05-p1r41-trust-clipped-independent-b10-action-freeze/v1"
            if p1r41
            else "ode-edit-s05-p1r39-normalized-gradient-independent-b10-action-freeze/v1"
            if p1r39
            else
...
def _run_ode_case(
    ...
    *,
    ...
    p1r41_methods = ("P1R41-TRUST-CLIPPED-NEUTRAL", "P1R41-TRUST-CLIPPED-SOFT")
    if method not in (p1r41_methods if p1r41 else p1r39_methods if p1r39 else p1r38_methods if p1r38 else METHODS):
        raise ODEBFContractError("independent method differs")
    allocation = "RS" if p1r38 or p1r39 or p1r41 else method.split("-", 1)[0]
    arm = (
        FixedE8Arm.NEUTRAL
    ...
    if (
        public["status"]
        != (
            "P1R41_TRUST_CLIPPED_GRADIENT_P1R35_K8_COMPLETE"
            if p1r41
    ...
    if technical_smoke:
        ...
        terminal = {
            "schema": "ode-edit-s05-p1r41-trust-clipped-b1-technical-terminal/v1" if p1r41 else "ode-edit-s05-p1r39-normalized-gradient-b1-technical-terminal/v1" if p1r39 else "ode-edit-s05-p1r38-perrequest-b1-technical-terminal/v1",
            "instruction_id": P1R41_INSTRUCTION_ID if p1r41 else P1R39_INSTRUCTION_ID if p1r39 else "ODEEDIT-S05-P1R38-PR-P1R35-PERREQUEST-TARGET-ATOMIC-V1",
            "method_id": P1R41_METHOD_ID if p1r41 else P1R39_METHOD_ID if p1r39 else P1R38_METHOD_ID,
            "case_index": case_index,
            "alias": alias,
            "method": method,
            "request_count": 1,
        ...
        manifest = {
            "schema": "ode-edit-s05-p1r41-trust-clipped-b1-technical-manifest/v1" if p1r41 else "ode-edit-s05-p1r39-normalized-gradient-b1-technical-manifest/v1" if p1r39 else "ode-edit-s05-p1r38-perrequest-b1-technical-manifest/v1",
    ...
    terminal = {
        "schema": (
            "ode-edit-s05-p1r41-trust-clipped-independent-b10-ode-terminal/v1"
            if p1r41
            else "ode-edit-s05-p1r39-normalized-gradient-independent-b10-ode-terminal/v1"
            if p1r39
            else
        ...
        "method_id": P1R41_METHOD_ID if p1r41 else P1R39_METHOD_ID if p1r39 else P1R38_METHOD_ID if p1r38 else P1R35_METHOD_ID,
        "case_index": case_index,
        "alias": alias,
        "method": method,
        "allocation": (
            "NOT_AN_EXPERIMENT_FACTOR" if p1r38 or p1r39 or p1r41 else allocation
        ),
        "inherited_writer_router_family": (
            "P1R35_RS" if p1r38 or p1r39 or p1r41 else allocation
        ),
        "arm": arm.value,
        "request_count": BATCH_SIZE,
        "request_order_sha256": request_order,
    ...
    manifest = {
        "schema": (
            "ode-edit-s05-p1r41-trust-clipped-independent-b10-case-manifest/v1"
            if p1r41
            else "ode-edit-s05-p1r39-normalized-gradient-independent-b10-case-manifest/v1"
            if p1r39
            else
