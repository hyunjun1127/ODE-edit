...
def _report(analysis: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "## C0–C3 writer 정의",
        "",
        "네 C arm은 같은 P1R52 K8 accepted-z와 같은 writer-entry W를 사용하고 writer만 비교한다. 차이는 다음과 같다.",
        "",
        "|arm|writer 정의|layer allocation / residual execution|이 비교에서의 역할|",
        "|---|---|---|---|",
        "|C0|현재 PIR-U control writer|기존 P/H-first·capacity-late routing을 유지하고 suffix-normalized remaining-residual 규칙으로 L4→L8을 실행|현행 writer control|",
        "|C1|Joint P+C + Remaining-residual writer|entry에서 pretrained-risk P와 capacity C를 epigraph minimax로 동시에 최적화해 layer share를 정한 뒤, C0와 같은 suffix-normalized remaining-residual 규칙으로 실행|routing만 Joint P+C로 바꾼 비교|",
        "|C2|Joint P+C + Fixed-contribution-quota writer|C1과 동일한 Joint P+C entry allocation을 사용하지만 suffix normalization을 제거하고, entry residual 기준의 고정 per-layer contribution quota를 L4→L8에 적용|마지막 layer catch-up/concentration을 제거한 실행 비교|",
        "|C3|Direct Official EasyEdit AlphaEdit writer|P1R52 accepted-z를 native `compute_z` 재호출 없이 Official AlphaEdit의 static-P/cache_c FP32 writer에 직접 연결; Joint P+C/PIR-U routing 영향 0|동일 accepted-z에서 Official writer 전달 특성을 보는 reference|",
        "",
        "따라서 C0↔C1은 allocation/routing 차이, C1↔C2는 residual execution 차이, C3는 P1R52 writer 계열이 아닌 Official AlphaEdit writer reference로 해석한다.",
        "",
        "## 결론",
        "",
