"""Outcome-free panel, lock, and resource contracts for fixed E8 P1R7."""
...
FIXED_E8_RESULT_TOKEN = "cold-fixed-e8-structfunc-soft-p1r7-r6-v1"
...
def expected_fixed_e8_result_name(alias: str) -> str:
    ...
    return f"s05-cold-fixed-e8-soft-p1r7-r6-{alias}-v1"
...
def fixed_e8_schedule(base: SamplingSeal) -> StatelessReplaySchedule:
    ...
    for lineage in base.lineages:
        seed = int(
            hashlib.sha256(
                f"{FIXED_E8_INSTRUCTION_ID}|{lineage.lineage.value}".encode(
                    "utf-8"
                )
            ).hexdigest()[:15],
            16,
...
class FixedE8ResourceForecast:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or self.qp_backend_invocations_per_arm != 40
...
def validate_fixed_e8_lock(
    ...
    *,
    ...
    expected = {
        ...
        "instruction_id": FIXED_E8_INSTRUCTION_ID,
        "parent_checkpoint": FIXED_E8_PARENT_HEAD,
