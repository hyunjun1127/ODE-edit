#!/usr/bin/env python3
"""Fail-closed Alpha-cache-on, Structural-H-off sequential control runner."""
...
from project.run_scripts.ode_bf.p1r52_sequential_control_panel import (
    LOCK_FILE,
    PARENT,
    SOURCE_MANIFEST,
    load_and_validate_control_lock,
)
from project.run_scripts.ode_bf.p1r52_sequential_runtime import R52_CONTROL_ROLE


RUN_TOKEN = "p1r52-sequential-alphacache-on-structuralh-off-v1"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P1R52 sequential control source ancestry differs")
    manifest, raw_sha = load_rooted_json(
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST,
        expected_schema="ode-edit-s05-p1r52-sequential-control-source-manifest/v1",
    )
    if manifest.get("instruction_id") != INSTRUCTION_ID or manifest.get("parent") != PARENT:
        raise ValueError("P1R52 sequential control source manifest header differs")
    entries = manifest.get("entries")
    if not isinstance(entries, list) or not entries:
        raise ValueError("P1R52 sequential control source manifest is empty")
    for entry in entries:
        ...
        if (
            ...
            raise ValueError("P1R52 sequential control source manifest member differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        lock, lock_sha = load_and_validate_control_lock(
        ...
        result = run_p1(
            ...
            p1r52_sequential_role=R52_CONTROL_ROLE,
        )
        result.update(
            {
                "sequential_control_lock_sha256": lock_sha,
                "sequential_control_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r52-sequential-control-job-failure/v1",
        )
        print(
            json.dumps(
                {
