#!/usr/bin/env python3
"""No-model dry plan for P1R38 four-cell independent B10x10."""
...
from project.run_scripts.ode_bf.p1r38_independent_b10x10_panel import LOCK_FILE, expected_result_name, load_and_validate_lock


CELLS = (
    ("llama3-8b-inst", "PR-P1R35-NEUTRAL"),
    ("llama3-8b-inst", "PR-P1R35-SOFT"),
    ("qwen2.5-7b-inst", "PR-P1R35-NEUTRAL"),
    ("qwen2.5-7b-inst", "PR-P1R35-SOFT"),
)
...
def build_plan(source_head: str, *, repository_root: Path = REPO_ROOT) -> dict[str, object]:
    ...
    if lock["fresh_stream_root"] != seal["root_digest"] or lock["all_request_order_sha256"] != seal["all_request_order_sha256"]:
        raise RuntimeError("P1R38 lock/seal differs")
    jobs = [
        {
            "array_index": index,
            "alias": alias,
    ...
    return {
        "schema": "ode-edit-s05-p1r38-pr-p1r35-perrequest-b10x10-dry-plan/v1",
