#!/usr/bin/env python3
"""Fail-closed P1R52-FPiQ Atomic B10x10 cell runner."""
...
RUN_TOKEN = "p1r52-fpiq-atomic-b10x10-v1"
SOURCE_MANIFEST = "source_manifest_s05_p1r52_fpiq_atomic_b10x10.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P1R52-FPiQ source ancestry differs")
    manifest_path = (
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST
    )
    manifest, raw_sha = load_rooted_json(
        manifest_path,
        expected_schema="ode-edit-s05-p1r52-fpiq-source-manifest/v1",
    )
    entries = manifest.get("entries")
    if (
        manifest.get("instruction_id")
        != "ODEEDIT-S05-P1R52-FROZEN-PI-SEQUENTIAL-QUOTA-WRITER-V1"
        or manifest.get("parent") != PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P1R52-FPiQ source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P1R52-FPiQ source entry differs")
        ...
        if (
            ...
            raise ValueError("P1R52-FPiQ source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P1R52-FPiQ source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = {
            **result,
            "p1r52_fpiq_lock_sha256": lock_sha,
            "p1r52_fpiq_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r52-fpiq-job-failure/v1",
        )
        print(
            json.dumps(
                {
