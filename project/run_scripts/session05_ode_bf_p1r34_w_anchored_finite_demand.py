#!/usr/bin/env python3
"""Fail-closed P1R34 finite-demand Atomic entry."""
...
from project.run_scripts.ode_bf.p1r34_w_anchored_finite_demand import P1R34_INSTRUCTION_ID
from project.run_scripts.ode_bf.p1r34_w_anchored_finite_demand_panel import P1R34_ROLES

RUN_TOKEN = "p1r34-w-anchored-finite-demand-v1"
BASE = "ce8c6c36348752f1407f7d713d30e6b5c727379b"


def _source_manifest_gate(source_head: str) -> str:
    path = REPO_ROOT / "project/run_scripts/ode_bf/locks/source_manifest_s05_p1r34_w_anchored_finite_demand.json"
    manifest, raw_sha = load_rooted_json(
        path, expected_schema="ode-edit-s05-p1r34-w-anchored-finite-demand-source-manifest/v1"
    )
    entries = manifest.get("entries")
    if manifest.get("instruction_id") != P1R34_INSTRUCTION_ID or manifest.get("base_checkpoint") != BASE or not entries:
        raise ValueError("P1R34 source manifest header differs")
    observed = []
    for entry in entries:
        ...
        if candidate.is_symlink() or not candidate.is_file() or candidate.stat().st_size != entry.get("size") or sha256_file(candidate) != entry.get("sha256"):
            raise ValueError("P1R34 source manifest entry differs")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P1R34 source manifest order differs")
    ...
    if changed != set(observed) | {path.relative_to(REPO_ROOT).as_posix()}:
        raise ValueError("P1R34 source manifest closure differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--role", required=True, choices=P1R34_ROLES)
    ...
    try:
        lock, lock_sha = load_rooted_json(
            REPO_ROOT / "project/run_scripts/ode_bf/locks/numerical_lock_s05_p1r34_w_anchored_finite_demand.json",
            expected_schema="ode-edit-s05-p1r34-w-anchored-finite-demand-lock/v1",
        )
        if lock.get("instruction_id") != P1R34_INSTRUCTION_ID:
            raise ValueError("P1R34 numerical lock differs")
        manifest_sha = _source_manifest_gate(args.source_head)
        result = run_p1(
            repo_root=REPO_ROOT, alias=args.model, output_root=args.output_root,
            source_head=args.source_head, atomic_strength_recovery_role=args.role,
        )
        result = {**result, "p1r34_numerical_lock_sha256": lock_sha, "p1r34_source_manifest_sha256": manifest_sha}
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r34-w-anchored-finite-demand-failure/v1",
        )
