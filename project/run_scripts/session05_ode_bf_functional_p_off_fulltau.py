#!/usr/bin/env python3
"""Fail-closed entry for the S05 functional-P-off full-tau panel."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="session05-ode-bf-functional-p-off-fulltau", allow_abbrev=False
    )
    parser.add_argument("--model", required=True, choices=MODEL_ALIASES)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--source-head", required=True)
    parser.add_argument(
        "--run-token", required=True, choices=(FUNCTIONAL_P_OFF_RESULT_TOKEN,)
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        failure_sha256, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=FUNCTIONAL_P_OFF_INSTRUCTION_ID,
            failure_schema="ode-edit-s05-functional-p-off-fulltau-failure/v1",
        )
        print(
            json.dumps(
                {
