#!/usr/bin/env python3
"""Held-inspect-release invalid-R52-only TECH-R1 observation repair."""
...
BRANCH = "codex/p1r52-b100-accepted-z-rephrase-obs-tech-r1-r52-v1"
...
EXPECTED_ACTIVE_TASKS = {"20599_0", "20599_1"}
REPAIR_ROLES = (R52_H_ROLE, R52_CONTROL_ROLE)
...
def main() -> int:
    ...
    if branch != BRANCH or run(["git", "status", "--porcelain"]).stdout:
        raise ODEBFContractError("accepted-z TECH-R1-R52 source differs")
    result_names = {
        role: expected_result_name(role, repair_revision="TECH-R1-R52")
        for role in REPAIR_ROLES
    }
    if any((RESULT_PARENT / name).exists() for name in result_names.values()):
        raise ODEBFContractError("accepted-z TECH-R1-R52 result exists")
    queue = run(
        [
    ...
    for line in queue:
        ...
        if count:
            ...
            if state in {"RUNNING", "CONFIGURING"}:
                allocated += count
    if {str(item["job_id"]) for item in jobs} != EXPECTED_ACTIVE_TASKS or allocated != 2:
        raise ODEBFContractError("accepted-z TECH-R1-R52 active scope differs")
    memory_mib = next(
    ...
    if allocated + 2 > 4 or memory_mib < 130000:
        raise ODEBFContractError("accepted-z TECH-R1-R52 cap/memory differs")
    state = REPO_ROOT / "local/odebf/state/p1r52-b100-accepted-z-observation-tech-r1-r52-v1"
    logs = REPO_ROOT / "local/odebf/logs/p1r52-b100-accepted-z-observation-tech-r1-r52-v1"
    logs.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-p1r52-b100-accepted-z-tech-r1-r52-intent/v1",
        "source_head": head,
        "repair_parent": "baa20e0ec05fdf6318af3a345af952c8f76ce8ee",
        "roles": list(REPAIR_ROLES),
        "result_names": result_names,
        "active_baseline_repair_jobs": jobs,
    ...
    command = [
        "sbatch", "--hold", "--parsable", "--array", "2-3%2",
        "--chdir", str(REPO_ROOT), "--nodelist", "devbox",
        "--job-name", "odeedit_p1r52_b100_accepted_z_tech_r1_r52",
        "--output", str(logs / "%A_%a.out"),
        "--error", str(logs / "%A_%a.err"), str(SBATCH), head,
        str(RESULT_PARENT), "accepted-z-rephrase-obs-tech-r1-r52",
    ]
    ...
    if not all(
        ...
        raise ODEBFContractError("accepted-z TECH-R1-R52 held inspection differs")
    receipt = {
        **intent,
        "job_id": job_id,
        "array": "2-3%2",
