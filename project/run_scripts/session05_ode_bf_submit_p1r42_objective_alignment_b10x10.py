#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R42."""
...
SBATCH = (
    REPO_ROOT
    / "project/run_scripts/session05_ode_bf_p1r42_objective_alignment_b10x10.sbatch"
)
B1_SBATCH = (
    REPO_ROOT
    / "project/run_scripts/session05_ode_bf_p1r42_objective_alignment_b1.sbatch"
)
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r42-objective-alignment-b10x10"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r42-objective-alignment-b10x10"
BRANCH = "codex/p1r42-objective-alignment-b10x10-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 4
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R42 execution source differs")
    plan = dry.build_plan(source_head)
    if smoke:
        if not attempt_suffix or not attempt_suffix.replace("-", "").isalnum():
            raise ODEBFContractError("P1R42 B1 attempt suffix differs")
        for job in plan["jobs"]:
            arm = "NEUTRAL" if str(job["method"]).endswith("-NEUTRAL") else "SOFT"
            arm_token = arm.lower()
            job["method"] = f"PR-P1R42-B1-OBJECTIVE-ALIGNED-{arm}"
            job["result_name"] = (
                f"s05-p1r42-objective-alignment-b1-{job['alias']}-"
                f"{arm_token}-{attempt_suffix}-v1"
            )
            job["case_count"] = 1
    ...
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R42 result namespace exists")

    active, active_jobs = _active_gpu_allocations()
    available = PROJECT_GPU_CAP - active
    if available <= 0:
        raise ODEBFContractError("P1R42 server2 project GPU cap has no free allocation")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R42 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-3%{stage_concurrency}"
    intent = {
        "schema": f"ode-edit-s05-p1r42-objective-alignment-{phase}-intent/v1",
    ...
    submitted = _run(
        [
            ...
            str(B1_SBATCH if smoke else SBATCH),
            source_head,
            str(RESULT_PARENT),
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R42 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R42 held scheduler contract differs")
    receipt = {
        "schema": f"ode-edit-s05-p1r42-objective-alignment-{phase}-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "job_id": job_id,
        "array": array,
