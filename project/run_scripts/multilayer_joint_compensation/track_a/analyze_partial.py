...
def attribution(states):
    """Four-state effects and design §13 symmetric two-block attribution."""
    ...
    for key, base in reference.items():
        ...
        rows.append(dict(metric=key[0], case_id=base['case_id'], prompt_index=base['prompt_index'],
                         identity=key[1], **symmetric_effects(e4,e8,e48),
    ...
    for m in ('RS', 'PS'):
        ...
        for field in ('standalone_E4', 'standalone_E8', 'standalone_E48', 'signed_L4', 'signed_L8', 'interaction',
                      'marginal_L4_given_L8', 'marginal_L8_given_L4'):
...
def symmetric_effects(e4,e8,e48):
    """Closed-form symmetric effects; preserve negative/zero effects, no ratios."""
    require(all(math.isfinite(x) for x in (e4,e8,e48)), 'NONFINITE_ATTRIBUTION')
    s4=.5*(e4+e48-e8);s8=.5*(e8+e48-e4)
    residual=(s4+s8)-e48
    bound=8*float(np.finfo(np.float64).eps)*max(1.,abs(e4)+abs(e8)+abs(e48))
    require(abs(residual)<=bound,'SYMMETRIC_ATTRIBUTION_IDENTITY')
    return dict(standalone_E4=e4,standalone_E8=e8,standalone_E48=e48,signed_L4=s4,signed_L8=s8,
                interaction=e48-e4-e8,signed_sum_residual=residual,signed_sum_roundoff_bound=bound)
...
def plots(summary, attr, trajectory, physical):
    ...
    fields=['signed_L4','signed_L8','standalone_E48','interaction']
...
def report(summary,attr,functional,traj,physical,schedule,common,terminal,generation,verify,command):
    ...
    def table(headers,rows):return '| '+' | '.join(h.replace('|','&#124;') for h in headers)+' |\n| '+' | '.join(['---']*len(headers))+' |\n'+''.join('| '+' | '.join(fmt(x).replace('|','&#124;') for x in row)+' |\n' for row in rows)
...
L(W)를 Current target-new NLL이라 할 때 standalone_E4=L(We)−L(We+D4), standalone_E8=L(We)−L(We+D8), standalone_E48=L(We)−L(We+D4+D8)이다. 설계 §13의 대칭 기여는 signed_L4=.5·(E4+E48−E8), signed_L8=.5·(E8+E48−E4)이며 합은 총 E48과 같다. interaction=E48−E4−E8이다. 아래 값은 실제 저장된 네 state의 request/prompt-matched 관측이며 단순 weight norm share가 아니다. Conditional marginal은 E48−E8 및 E48−E4다. 음의 interaction은 개별 효과의 중복/비선형성까지 포함하며 곧바로 방해 또는 실패를 뜻하지 않는다. 합의 NLL 개선이 양수인 사실만으로 50:50 부담 분산이나 A0-L4 대비 capacity 우위를 입증하지 않는다.
...
def build_attribution_supplement(args):
    """Repair only publication interpretation from sealed scalar CSV bytes."""
    source=Path(args.attribution_supplement).absolute();out=Path(args.output).absolute()
    require(not out.exists(),'SUPPLEMENT_CREATE_ONCE')
    manifest=read(source/'manifest.json');receipt=read(source/'rooted-receipt.json')
    require(digest(manifest['members'])==manifest['members_root'],'V1_MANIFEST_ROOT')
    require(sha(source/'manifest.json')==receipt['manifest_sha256'],'V1_MANIFEST_BINDING')
    require(digest({k:v for k,v in receipt.items() if k!='identity'})==receipt['identity'],'V1_RECEIPT_IDENTITY')
    expected=next(x for x in manifest['members'] if x['name']=='signed-attribution.csv')
    csv_member=member(source/expected['name'])
    require(all(csv_member[k]==expected[k] for k in ('sha256','bytes','mode')),'V1_ATTRIBUTION_CSV_HASH')
    # Verify small package metadata only; raw model/tensor inputs are not opened.
    old_csv=source/'signed-attribution.csv'
    with old_csv.open() as f:original=list(csv.DictReader(f))
    rows=[]
    for r in original:
        rows.append(dict(metric=r['metric'],case_id=int(r['case_id']),prompt_index=int(r['prompt_index']),identity=r['identity'],
                         **symmetric_effects(float(r['e4']),float(r['e8']),float(r['e48']))))
    require(len(rows)==300 and len({(r['metric'],r['identity']) for r in rows})==300,'ATTRIBUTION_SUPPLEMENT_DENOMINATOR')
    require(sum(r['metric']=='RS' for r in rows)==100 and sum(r['metric']=='PS' for r in rows)==200,'ATTRIBUTION_METRIC_COUNTS')
    aggregate=[]
    for metric in ('RS','PS'):
        rr=[r for r in rows if r['metric']==metric]
        for field in ('standalone_E4','standalone_E8','standalone_E48','signed_L4','signed_L8','interaction'):
            v=[r[field] for r in rr]
            aggregate.append(dict(metric=metric,quantity=field,denominator=len(rr),**stats(v),
                                  positive_n=sum(x>0 for x in v),zero_n=sum(x==0 for x in v),negative_n=sum(x<0 for x in v)))
    check=dict(row_count=300,RS_rows=100,PS_prompt_rows=200,
      formula='signed_L4=.5*(standalone_E4+standalone_E48-standalone_E8); signed_L8=.5*(standalone_E8+standalone_E48-standalone_E4)',
      sum_identity='signed_L4+signed_L8=standalone_E48',failure_count=0,
      max_absolute_residual=max(abs(r['signed_sum_residual']) for r in rows),
      max_scaled_residual=max(abs(r['signed_sum_residual'])/r['signed_sum_roundoff_bound'] for r in rows),
      bound_rule='8*eps_FP64*max(1,abs(E4)+abs(E8)+abs(E48)); algebraic identity with finite arithmetic residual recorded',
      imputation=0,new_model_evaluation=0,raw_tensor_read=0,PNG_generation=0,
      source_csv=csv_member,source_manifest=member(source/'manifest.json'),source_receipt=member(source/'rooted-receipt.json'),
      original_v1_unchanged=True,scientific_promotion=False)
    out.mkdir(parents=True)
    write_csv(out/'symmetric-attribution.csv',rows);write_csv(out/'symmetric-attribution-summary.csv',aggregate)
    save_json(out/'identity-checks.json',check)
    lines=['# Middle A0 대칭 attribution 정정 보충 — 설계 §13', '',
      ...
      '| --- | --- | --- | --- | --- | --- | --- | --- |']
    for r in aggregate:
        lines.append(f"| {r['metric']} | {r['quantity']} | {r['mean']:.9g} | {r['median']:.9g} | {r['p90']:.9g} | {r['max']:.9g} | {r['positive_n']} / {r['zero_n']} / {r['negative_n']} | {r['denominator']} |")
    lines += ['', 'RS는 Current rewrite100 requests, PS는 동일100 requests의 두 paraphrase prompt200개다. 평균은 해당 prompt-row 분모이며 PS를 독립200 requests라고 주장하지 않는다. Request/prompt별 signed 원값은 `symmetric-attribution.csv`에 있다. 총 개선≤0인 행도 제거하지 않으며 기여율은 계산하지 않았다.', '',
      '## 3. Identity·해석 경계', '',
      f"300행에서 대칭 합 identity failure=0. FP64 최대 절대 residual={check['max_absolute_residual']:.17g}, backward-roundoff bound 대비 최대 비율={check['max_scaled_residual']:.17g}. 수학적 항등식과 부동소수점 bitwise equality는 구분했다. 수치 상한은 `8 eps64 max(1, |E4|+|E8|+|E48|)`이며 성능 gate나 조정된 threshold가 아니다.", '',
      ...
      '## 4. Provenance와 재현', '',
      f"입력 CSV SHA256 `{csv_member['sha256']}`. v1 manifest SHA `{sha(source/'manifest.json')}`, receipt identity `{receipt['identity']}`. 이 보충은 이 CSV의 세 standalone scalar만 사용했으며 원본 NLL·tensor·모델을 다시 계산하지 않았다.", '',
      '```bash',f'{sys.executable} -m project.run_scripts.multilayer_joint_compensation.track_a.analyze_partial --attribution-supplement {source} --output <new-create-once-supplement>', '```','',
      'Source SHA, 입력 identity, 출력 members root는 `manifest.json`과 `rooted-receipt.json`에 있다. scientific_promotion=false.']
    write_identical_or_create(out/'attribution-correction-ko.md',('\n'.join(lines)+'\n').encode())
    require(member(old_csv)==csv_member,'V1_CSV_CHANGED_DURING_SUPPLEMENT')
    codefiles=[member(Path(__file__)),member(ROOT/'project/run_scripts/multilayer_joint_compensation/tests/test_a_partial_analysis.py')]
    ms=[dict(name=p.name,bytes=p.stat().st_size,mode=oct(stat.S_IMODE(p.stat().st_mode)),sha256=sha(p)) for p in sorted(out.iterdir())]
    sm=dict(schema='middle-A0-design13-symmetric-attribution-correction-v2',members=ms,members_root=digest(ms),
      input_members=[csv_member,member(source/'manifest.json'),member(source/'rooted-receipt.json')],
      parent_checkout_HEAD=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
      analysis_source_files=codefiles,analysis_source_identity='exact worktree file SHAs authoritative; parent checkout HEAD alone is not a claim that modified analysis code is committed',
      execution_source_HEAD=manifest['execution_source_head'],scientific_promotion=False)
    save_json(out/'manifest.json',sm)
    sr=dict(status='DESIGN13_ATTRIBUTION_CORRECTION_READY',manifest_sha256=sha(out/'manifest.json'),members_root=sm['members_root'],
            report_sha256=sha(out/'attribution-correction-ko.md'),source_v1_receipt_identity=receipt['identity'],
            new_model_GPU_evaluation_actions=0,original_publication_mutation=0,scientific_promotion=False)
    sr['identity']=digest(sr);save_json(out/'rooted-receipt.json',sr)
    print(json.dumps(dict(output=str(out),report_sha256=sr['report_sha256'],manifest_sha256=sr['manifest_sha256'],
                         receipt_sha256=sha(out/'rooted-receipt.json'),receipt_identity=sr['identity'])))


if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--a0');parser.add_argument('--common');parser.add_argument('--output',required=True)
    ...
    parser.add_argument('--attribution-supplement',type=Path,help='sealed v1 publication directory; CSV-only §13 correction, no tensor reads')
    args=parser.parse_args()
    if args.attribution_supplement:build_attribution_supplement(args)
    else:
        parser.error('--a0 and --common required') if not args.a0 or not args.common else None
        build(args)
