#!/usr/bin/env bash
# Fail-closed one-shot submit helper for the exact paired MV-1 untouched run.
...
readonly SBATCH_FILE="${REPO_ROOT}/project/run_scripts/session01_mv1mix_untouched_pair_server1.sbatch"
readonly RED_GATE="${REPO_ROOT}/audits/global/2026-07-31-session01-mv1mix-untouched-execution-preflight.md"
readonly C1_POSTRUN="${REPO_ROOT}/audits/global/2026-07-31-mv1mix-c1-pair-v1.postrun.md"
readonly RED_GATE_VERDICT='- 최종 판정: `PASS` — MV-1 untouched 동시 pair 한 건에만 유효'
readonly C1_POSTRUN_VERDICT='- 최종 판정: `UNTOUCHED PREPARE` — C1 pair clear 및 locked untouched 20 한 건 허용'
...
readonly PAIR_MARKER="${STATE_ROOT}/mv1mix_untouched_pair_v1.submitted"
readonly JOB_NAME="odeedit_mv1mix_untouched_pair_v1"
...
[[ "$#" -eq 0 ]] || {
  echo "this paired MV-1 untouched helper takes no arguments" >&2
  exit 2
}
[[ -x "${GIT_BIN}" \
  && -x "${GREP_BIN}" \
...
"${GIT_BIN}" ls-files --error-unmatch \
  ...
  audits/global/2026-07-31-session01-mv1mix-untouched-execution-preflight.md \
  audits/global/2026-07-31-mv1mix-c1-pair-v1.postrun.md \
  >/dev/null
[[ -z "$("${GIT_BIN}" status --porcelain --untracked-files=normal)" ]] || {
  echo "paired MV-1 untouched submission requires a clean repository" >&2
  exit 2
}
[[ "$("${GIT_BIN}" rev-parse HEAD)" == "$("${GIT_BIN}" rev-parse origin/main)" ]] || {
  echo "paired MV-1 untouched submission requires main == origin/main" >&2
  exit 2
}

require_exact_final_verdict "${RED_GATE}" "${RED_GATE_VERDICT}" "MV-1 untouched preflight"
require_exact_final_verdict "${C1_POSTRUN}" "${C1_POSTRUN_VERDICT}" "MV-1 C1 postrun"

[[ ! -e "${OUTPUT_ROOT}/mv1mix_llama_untouched_v1" \
  && ! -L "${OUTPUT_ROOT}/mv1mix_llama_untouched_v1" ]] || {
  echo "Llama untouched output already exists" >&2
  exit 2
}
[[ ! -e "${OUTPUT_ROOT}/mv1mix_qwen_untouched_v1" \
  && ! -L "${OUTPUT_ROOT}/mv1mix_qwen_untouched_v1" ]] || {
  echo "Qwen untouched output already exists" >&2
  exit 2
}
[[ ! -e "${PAIR_MARKER}" && ! -L "${PAIR_MARKER}" ]] || {
  echo "MV-1 untouched one-shot marker already exists" >&2
  exit 2
}
if [[ -n "$("${SQUEUE_BIN}" -h --name="${JOB_NAME}")" ]]; then
  echo "paired MV-1 untouched job is already active" >&2
  exit 2
fi
