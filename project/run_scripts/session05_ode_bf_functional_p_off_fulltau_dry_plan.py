#!/usr/bin/env python3
"""No-model deterministic plan for the S05 functional-P-off pair."""
...
JOB_NAMES = {
    "llama3-8b-inst": "odeedit_s05_fpfullr3_llama",
    "qwen2.5-7b-inst": "odeedit_s05_fpfullr3_qwen",
}
...
def build_plan(source_head: str, *, repository_root: Path = REPO_ROOT) -> dict[str, object]:
    if len(source_head) != 40 or any(
        ...
        raise ODEBFContractError("functional-P dry-plan source head differs")
    ...
    for alias in MODEL_ALIASES:
        forecast = forecast_functional_p_off_panel(artifact, base, alias)
        jobs.append(
            {
                "alias": alias,
                "job_name": JOB_NAMES[alias],
                "result_name": expected_functional_p_off_result_name(alias),
                "gpu": 1,
                "cpu": 8,
                "memory_mib": 65_000,
                "time": "24:00:00",
    ...
    return {
        "schema": "ode-edit-s05-functional-p-off-fulltau-p1r3-dry-plan/v1",
        ...
        "functional_p_observed_budget": 1.0e-3,
