#!/usr/bin/env python3
"""Fail-closed launcher for one B1 Official Native reference cell."""
...
from project.run_scripts.session05_ode_bf_p1r52_target_timescale_native_b100_preflight import (
    _source_manifest,
)


RUN_TOKEN = "p1r52-target-timescale-native-b100-user-override-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("Native final pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema")
        != "ode-edit-s05-p1r52-target-timescale-native-final-pre-gpu/v1"
        or value.get("instruction_id") != INSTRUCTION_ID
        or value.get("status") != "NATIVE_REFERENCE_FINAL_PRE_GPU_PASS"
        ...
        raise ODEBFContractError("Native final pre-GPU content differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--cell", required=True, type=int, choices=range(2))
    ...
    try:
        ...
        if (
            ...
            raise ODEBFContractError("Native source/output namespace differs")
        final = _load_final(
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("Native deployment identity differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r52-target-timescale-native-job-timing/v1",
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r52-target-timescale-native-failure/v1",
        )
        print(
            json.dumps(
                {
