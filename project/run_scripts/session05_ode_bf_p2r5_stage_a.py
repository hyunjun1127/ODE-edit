#!/usr/bin/env python3
"""Fail-close P2R5 SDRT Stage-A pilot runner."""
...
RUN_TOKEN = "p2r5-sdrt-stage-a-v1"
SOURCE_MANIFEST = "source_manifest_s05_p2r5_sdrt_stage_a.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P2R5 P2R4 source ancestry differs")
    manifest, raw_sha = load_rooted_json(
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST,
        expected_schema="ode-edit-s05-p2r5-sdrt-stage-a-source-manifest/v1",
    )
    entries = manifest.get("entries")
    if (
        manifest.get("instruction_id") != P2R5_INSTRUCTION_ID
        or manifest.get("exact_p2r4_parent") != PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P2R5 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P2R5 source manifest entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P2R5 source path differs")
        if path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P2R5 source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P2R5 source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = {
            **result,
            "p2r5_lock_sha256": lock_sha,
            "p2r5_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p2r5-sdrt-stage-a-job-failure/v1",
        )
        print(
            json.dumps(
                {
