#!/usr/bin/env python3
"""Fail-closed P2R2 Atomic paired-arm runner."""
...
from project.run_scripts.ode_bf.p2r2_residual_transport_writer import P2R2_INSTRUCTION_ID


RUN_TOKEN = "p2r2-semantic-conserving-residual-transport-writer-v1"
SOURCE_MANIFEST = "source_manifest_s05_p2r2_residual_transport_writer.json"
...
def _source_gate(source_head: str) -> str:
    ...
    manifest, raw_sha = load_rooted_json(
        ...
        expected_schema="ode-edit-s05-p2r2-residual-transport-source-manifest/v1",
    )
    entries = manifest.get("entries")
    if (
        manifest.get("instruction_id") != P2R2_INSTRUCTION_ID
        or manifest.get("parent") != PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P2R2 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P2R2 source manifest entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P2R2 source path differs")
        if path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P2R2 source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P2R2 source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_p1(
            ...
            p2r2_case_count=args.case_count,
            p2r2_attempt_suffix=args.attempt_suffix,
        )
        result = {
            **result,
            "p2r2_lock_sha256": lock_sha,
            "p2r2_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p2r2-job-failure/v1",
        )
        print(
            json.dumps(
                {
