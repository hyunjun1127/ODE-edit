#!/usr/bin/env python3
"""Fail-closed entry point for one P1R19 Dynamic trajectory."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cell", required=True, choices=STRENGTH_PRESERVING_CELLS
    )
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--source-head", required=True)
    parser.add_argument(
        "--run-token", required=True, choices=(STRENGTH_PRESERVING_RESULT_TOKEN,)
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = run_p1(
            ...
            strength_preserving_cell=args.cell,
        )
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=STRENGTH_PRESERVING_INSTRUCTION_ID,
            failure_schema="ode-edit-s05-strength-preserving-router-p1r19-a2-failure/v1",
        )
        print(
            json.dumps(
                {
