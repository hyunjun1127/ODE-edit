#!/usr/bin/env python3
"""Fail-closed P1R36 P1R35 full-matrix independent whole-B10 runner."""
...
from project.run_scripts.ode_bf.p1r36_independent_b10x10_runtime import INSTRUCTION_ID
from project.run_scripts.ode_bf.p1r36_independent_b10x10_runtime import METHODS


RUN_TOKEN = "p1r36-p1r35-independent-b10x10-v1"
SOURCE_MANIFEST = "source_manifest_s05_p1r36_p1r35_independent_b10x10.json"
PROTECTED_SCIENTIFIC_SHA256 = {
    ...
    "project/run_scripts/ode_bf/locks/numerical_lock_s05_p1r35_full_current_residual.json": "136c4286bc496373b0f14f8f84ddc4982abcba8c7dc8e30be9d11260b9766c89",
}
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P1R36 P1R35 scientific ancestry differs")
    for relative, expected in PROTECTED_SCIENTIFIC_SHA256.items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise ValueError("P1R35 protected scientific source differs")
    ...
    if (
        ...
        raise ValueError("P1R35 independent source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P1R35 independent source entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P1R35 independent source path differs")
        if path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P1R35 independent source bytes differ")
    ...
    if observed != sorted(set(observed)) or changed != set(observed) | {manifest_relative}:
        raise ValueError("P1R35 independent source closure differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        ) != lock["p1r35_numerical_lock_sha256"]:
            raise ValueError("P1R35 frozen numerical lock differs")
        ...
        result = run_p1(
            ...
            p1r36_independent_b10x10_method=args.method,
        )
        result = {
            **result,
            "p1r36_independent_lock_sha256": lock_sha,
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r36-p1r35-independent-b10x10-job-failure/v1",
        )
