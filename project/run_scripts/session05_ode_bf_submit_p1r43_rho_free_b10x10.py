#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R43."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r43_rho_free_b10x10.sbatch"
B1_SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r43_rho_free_b1.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r43-rho-free-b10x10"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r43-rho-free-b10x10"
BRANCH = "codex/p1r43-rho-free-semantic-first-strength-recovery-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 4
...
def submit(source_head: str, *, smoke: bool = False, attempt_suffix: str | None = None) -> dict[str, object]:
    ...
    if smoke:
        ...
        for job in plan["jobs"]:
            ...
            arm_token = arm.lower()
            job["method"] = f"PR-P1R43-B1-RHO-FREE-SEMANTIC-FIRST-{arm}"
            job["result_name"] = (
                f"s05-p1r43-rho-free-b1-{job['alias']}-{arm_token}-{attempt_suffix}-v1"
            )
            job["case_count"] = 1
    ...
    if available <= 0:
        raise ODEBFContractError("P1R43 server1 project GPU cap has no free allocation")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R43 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-3%{stage_concurrency}"
    intent = {
        "schema": f"ode-edit-s05-p1r43-rho-free-{phase}-intent/v1",
    ...
    submitted = _run(
        [
            ...
            "--job-name", f"odeedit_s05_p1r43_rho_free_{phase}",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R43 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R43 held scheduler contract differs")
    receipt = {
        "schema": f"ode-edit-s05-p1r43-rho-free-{phase}-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "job_id": job_id,
        "array": array,
