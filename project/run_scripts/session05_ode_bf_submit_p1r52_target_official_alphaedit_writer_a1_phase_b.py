#!/usr/bin/env python3
"""Held-inspect-release submitter for the released Phase-B arm."""
...
from project.run_scripts import session05_ode_bf_p1r52_target_official_alphaedit_writer_a1_phase_b_dry_plan as dry
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r52_target_official_alphaedit_writer_a1_phase_b.sbatch"
BRANCH = "codex/p1r52-target-official-alphaedit-writer-a1-phase-b-v1"
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("target/Official writer Phase-B source differs")
    output_root = REPO_ROOT / "local/odebf/results" / PHASE_B_RESULT_NAME
    state_root = REPO_ROOT / "local/odebf/state/p1r52-target-official-alphaedit-writer-a1-phase-b-v1"
    log_root = REPO_ROOT / "local/odebf/logs/p1r52-target-official-alphaedit-writer-a1-phase-b-v1"
    if any(path.exists() or path.is_symlink() for path in (output_root, state_root, log_root)):
        raise ODEBFContractError("target/Official writer Phase-B namespace exists")
    allocated, jobs = _gpu_jobs()
    if allocated + 1 > PROJECT_GPU_CAP:
        raise ODEBFContractError("target/Official writer Phase-B project GPU cap differs")
    memory_mib = _host_memory_available_mib()
    if memory_mib < 65000:
        raise ODEBFContractError("target/Official writer Phase-B host memory is below request")
    ...
    intent = {
        "schema": "ode-edit-s05-p1r52-target-official-alphaedit-writer-phase-b-submission-intent/v1",
    ...
    submitted = _run(
        [
            ...
            "devbox",
            "--job-name",
            "ode_p1r52_target_official_writer_b",
            "--output",
            str(log_root / "%j.out"),
            "--error",
            str(log_root / "%j.err"),
    ...
    if not job_id.isdigit() or not all(item in observed for item in required):
        if job_id.isdigit():
            _run(["scancel", job_id], check=False)
        raise ODEBFContractError("target/Official writer Phase-B held contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-target-official-alphaedit-writer-phase-b-submission/v1",
        "source_head": source_head,
        "branch": branch,
        "job_id": job_id,
        "batch_count": 10,
