#!/usr/bin/env python3
"""Held-inspect-release submitter for PIR-U post-energy-WARN R1."""
...
from project.run_scripts.ode_bf.p1r52_piru_postenergy_warn import RESULT_NAME
...
SBATCH = (
    REPO_ROOT
    / "project/run_scripts/session05_ode_bf_p1r52_piru_postenergy_warn_b100x10.sbatch"
)
BRANCH = "codex/p1r52-piru-seq-postenergy-warn-r1-v1"
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if allocated + 1 > PROJECT_GPU_CAP:
        raise ODEBFContractError("PIR-U post-energy-WARN project GPU cap differs")
    memory_mib = _host_memory_available_mib()
    if memory_mib < 65000:
        raise ODEBFContractError("PIR-U post-energy-WARN host memory is below request")
    ...
    intent = {
        "schema": "ode-edit-s05-p1r52-piru-postenergy-warn-submission-intent/v1",
    ...
    submitted = _run(
        [
            ...
            "--nodelist", "devbox", "--job-name", "ode_p1r52_piru_pewarn_r1",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("PIR-U post-energy-WARN scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("PIR-U post-energy-WARN held contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-piru-postenergy-warn-submission/v1",
        "source_head": source_head,
        "branch": branch,
        "job_id": job_id,
        "request_count": 1000,
