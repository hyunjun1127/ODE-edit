...
def submit_tech_r2_task4(source_head: str) -> dict[str, object]:
    if (
    ...
    selected = [job for job in plan["jobs"] if job["array_index"] == 4]
...
def main() -> int:
    ...
    parser.add_argument("--repair-task-index", type=int, choices=(4,))
    args = parser.parse_args()
    result = (
        submit_tech_r2_task4(args.source_head)
        if args.repair_task_index == 4
