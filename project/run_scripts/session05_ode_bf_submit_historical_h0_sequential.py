#!/usr/bin/env python3
"""Held-inspect-release submitter for ten independent-B10 Atomic jobs."""
...
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r23-independent-b10x10"
...
NAMESPACE = "s05-p1r23-progress-simplex-independent-b10x10-sh1-array-v1"
BRANCH = "codex/p1r23-progress-simplex-independent-b10x10-v1"
...
def _verify_source_manifest() -> tuple[str, str]:
    ...
    if observed_root != canonical_hash(value):
        raise ODEBFContractError("P1R23 independent B10 manifest root differs")
    entries = value.get("entries")
    if not isinstance(entries, list) or value.get("entry_count") != len(entries):
        raise ODEBFContractError("P1R23 independent B10 manifest inventory differs")
    for item in entries:
        ...
        if (
            ...
            raise ODEBFContractError(
                "P1R23 independent B10 manifest member differs"
            )
...
def submit(source_head: str) -> dict[str, object]:
    if _run(["git", "rev-parse", "HEAD"]).stdout.strip() != source_head or _run(["git", "branch", "--show-current"]).stdout.strip() != BRANCH or _run(["git", "status", "--porcelain", "--untracked-files=no"]).stdout:
        raise ODEBFContractError("P1R23 independent B10 execution source differs")
    ...
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R23 independent B10 result namespace exists")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R23 independent B10 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent_sha = _write_once(intent_path, {"schema": "ode-edit-s05-p1r23-independent-b10x10-intent/v1", "source_head": source_head, "source_manifest_sha256": source_manifest_sha, "source_manifest_root": source_manifest_root, "dry_plan": plan, "held_then_atomic_release": True, "array": "0-9%4"})
    job_id = _run(["sbatch", "--hold", "--parsable", "--chdir", str(REPO_ROOT), "--job-name", "odeedit_s05_p1r23_independent_b10x10", "--output", str(LOG_ROOT / "%A_%a.out"), "--error", str(LOG_ROOT / "%A_%a.err"), str(SBATCH), source_head, str(RESULT_PARENT)]).stdout.strip().split(";", 1)[0]
    if not job_id.isdigit():
        raise ODEBFContractError("P1R23 independent B10 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        _run(["scancel", job_id])
        raise ODEBFContractError("P1R23 independent B10 held scheduler contract differs")
    receipt = {"schema": "ode-edit-s05-p1r23-independent-b10x10-submission/v1", "source_head": source_head, "source_manifest_sha256": source_manifest_sha, "source_manifest_root": source_manifest_root, "job_id": job_id, "array": "0-9%4", "job_count": 10, "ode_job_count": 8, "official_alphaedit_job_count": 2, "independent_atomic_b10_case_count": 100, "max_concurrent_gpu": 4, "intent_sha256": intent_sha, "held_inspection_sha256": hashlib.sha256(observed.encode()).hexdigest(), "held_then_atomic_release": True}
