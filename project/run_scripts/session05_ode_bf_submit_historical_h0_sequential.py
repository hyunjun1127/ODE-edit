#!/usr/bin/env python3
"""Held-inspect-release submitter for the P1R20 eight-task array."""
...
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r20-historical-h0"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
NAMESPACE = "s05-p1r20-historical-h0-bg-sequential-sh1-array-v1"
BRANCH = "codex/p1r20-historical-h0-bg-sequential-v1"
...
def submit(source_head: str) -> dict[str, object]:
    if _run(["git", "rev-parse", "HEAD"]).stdout.strip() != source_head or _run(["git", "branch", "--show-current"]).stdout.strip() != BRANCH or _run(["git", "status", "--porcelain", "--untracked-files=no"]).stdout:
        raise ODEBFContractError("P1R20 execution source differs")
    plan = dry.build_plan(source_head)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R20 result namespace exists")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R20 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent_sha = _write_once(intent_path, {"schema": "ode-edit-s05-p1r20-intent/v1", "source_head": source_head, "dry_plan": plan, "held_then_atomic_release": True, "array": "0-7%4"})
    job_id = _run(["sbatch", "--hold", "--parsable", "--chdir", str(REPO_ROOT), "--job-name", "odeedit_s05_p1r20_historical_h0", "--output", str(LOG_ROOT / "%A_%a.out"), "--error", str(LOG_ROOT / "%A_%a.err"), str(SBATCH), source_head, str(RESULT_PARENT)]).stdout.strip().split(";", 1)[0]
    if not job_id.isdigit():
        raise ODEBFContractError("P1R20 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        _run(["scancel", job_id])
        raise ODEBFContractError("P1R20 held scheduler contract differs")
    receipt = {"schema": "ode-edit-s05-p1r20-submission/v1", "source_head": source_head, "job_id": job_id, "array": "0-7%4", "trajectory_count": 8, "max_concurrent_gpu": 4, "intent_sha256": intent_sha, "held_inspection_sha256": hashlib.sha256(observed.encode()).hexdigest(), "held_then_atomic_release": True}
