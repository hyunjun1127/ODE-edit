...
def submit(source_head: str) -> dict[str, object]:
    ...
    job_id = _run(["sbatch", "--hold", "--parsable", "--array", array_spec, "--chdir", str(REPO_ROOT), "--job-name", "odeedit_s05_p1r23_full6_historical_tech_r1", "--output", str(LOG_ROOT / "%A_%a.out"), "--error", str(LOG_ROOT / "%A_%a.err"), str(SBATCH), source_head, str(RESULT_PARENT)]).stdout.strip().split(";", 1)[0]
