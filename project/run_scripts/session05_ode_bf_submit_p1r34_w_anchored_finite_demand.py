#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R34 B1 and production."""
...
from project.run_scripts import session05_ode_bf_p1r34_w_anchored_finite_demand_dry_plan as dry
from project.run_scripts.ode_bf.contracts import ODEBFContractError

SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r34_w_anchored_finite_demand.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r34-w-anchored-finite-demand"
...
def submit(source_head: str, phase: str) -> dict[str, object]:
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R34 execution source differs")
    plan = dry.build_plan(source_head, phase)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R34 result namespace exists")
    ...
    submitted = _run([
        ...
        "--nodelist", "devbox", "--job-name", f"odeedit_s05_p1r34_{phase}",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R34 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R34 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r34-submission/v1", "source_head": source_head,
