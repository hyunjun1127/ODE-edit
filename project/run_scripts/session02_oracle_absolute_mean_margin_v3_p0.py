#!/usr/bin/env python3
"""Run the paired-stage oracle absolute-mean-margin V3 P0 profiler.
...
EXECUTION_TOKEN = "oracle-absolute-mean-margin-v3-p0"
OUTPUT_PREFIX = "session02-oracle-absolute-mean-margin-v3-p0"
...
def _exact_output(repo: Path, raw: Path, model_alias: str, proposal_id: str) -> Path:
    ...
    if candidate.resolve() != expected:
        raise RuntimeError("V3 P0 output root differs from its dry plan")
    return candidate
...
def dry_plan(oracle_lock: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-dry-plan/v1",
...
def run(args: argparse.Namespace) -> int:
    ...
    if easyedit_root != EASYEDIT_ROOT.resolve(strict=True):
        raise RuntimeError("V3 P0 EasyEdit root differs")
    ...
    if (
        ...
        raise RuntimeError("V3 P0 base lock identity differs")
    ...
    if os.environ.get("ODEEDIT_ORACLE_ABSOLUTE_V3_P0_SUBMISSION_AUTHORIZED") != EXECUTION_TOKEN:
        raise RuntimeError("separate V3 P0 execution authority is required")
    for key in ("HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE"):
        if os.environ.get(key) != "1":
            raise RuntimeError(f"{key}=1 is required")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("V3 P0 requires exactly one visible GPU")
    ...
    if _request_ids(requests) != base_lock["selection"]["request_ids"][:1]:
        raise RuntimeError("V3 P0 request identity differs")
    ...
    if setup_seconds < 0.0:
        raise RuntimeError("V3 P0 setup timer became negative")
    git_head = _git_head(repo)
    manifest = {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-manifest/v1",
    ...
    try:
        ...
        for arm in P0_ARMS:
            ...
            if calibration is None or target is None:
                raise RuntimeError("V3 P0 calibration is absent")
            ...
            if (
                ...
                raise RuntimeError("V3 P0 counter contract differs")
            if arm is not Arm.NATIVE_MEMIT and counters["N_bw"] != counters["N_field"]:
                raise RuntimeError("V3 P0 field/backward count differs")
            if arm is Arm.FULL_ODE_EDIT and backend.hook_reference_gate is None:
                raise RuntimeError("V3 P0 Full hook/reference gate is absent")
            ...
            if result.status == "event_hit" and max(
                ...
                raise RuntimeError("V3 P0 hit violates its aggregate floors")
            common = {
            ...
            controller_row = {
                "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-controller/v1",
                **common,
            ...
            compute_row = {
                "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-compute/v1",
                **common,
                **snapshot,
                **counters,
                "calibration_total_model_forwards": (
                    calibration.entry_forward_count
                    + calibration.oracle_forward_count
                ),
                "oracle_extra_model_forwards": calibration.oracle_forward_count,
            ...
            trace_row = {
                "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-strength/v1",
                **common,
    ...
    native_energy = by_arm[Arm.NATIVE_MEMIT.value]["terminal_net_c_energy"]
    native_energy_total = (
        math.fsum(float(value) for value in native_energy.values())
    ...
    geometry_relative_to_native = [
        {
            "arm": arm.value,
            "status": by_arm[arm.value]["status"],
            "terminal_net_c_energy": by_arm[arm.value][
                "terminal_net_c_energy"
            ],
            "terminal_net_c_distance": by_arm[arm.value][
                "terminal_net_c_distance"
            ],
            "native_terminal_c_energy_total": native_energy_total,
            "native_terminal_c_distance": by_arm[Arm.NATIVE_MEMIT.value][
                "terminal_net_c_distance"
            ],
        }
        for arm in P0_ARMS
    ...
    summary = {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p0-summary/v1",
