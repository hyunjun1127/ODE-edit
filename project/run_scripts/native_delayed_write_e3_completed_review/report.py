"""Reproducible Korean factual report and code-only static plots, CPU only."""
import csv
import io
import re
import numpy as np
...
def num(v,d=5):return 'NA' if v in ('',None) else f'{float(v):.{d}f}'
def pct(v):return num(float(v)*100,2)
def short(x):
    x=x.replace('BASE_ALPHAEDIT','Alpha').replace('BASE_MEMIT','MEMIT')
    return re.sub(r'_s(\d+)_t(\d+)',lambda m:f'({int(m[1])},{int(m[2])})',x)
def table(headers,data):
    def esc(v):return str(v).replace('|','/').replace('\n',' ')
    return '\n'.join(['| '+' | '.join(headers)+' |','|'+'|'.join(['---']*len(headers))+'|']+['| '+' | '.join(esc(v) for v in row)+' |' for row in data])+'\n'

def figures(end,paired,coverage):
    import matplotlib
    matplotlib.use('Agg')
    ...
    for family,color in [('BASE_ALPHAEDIT','#0072b2'),('BASE_MEMIT','#d55e00')]:
        rr=[r for r in end if r['endpoint'].startswith(family) and r['kind']=='N'];x=[int(r['endpoint'].split('_W')[1])*100 for r in rr]
        axs[0].scatter(x,[float(r['success'])/10 for r in rr],label=family.replace('BASE_',''),color=color,s=32)
        axs[1].scatter(x,[float(r['strict'])/10 for r in rr],label=family.replace('BASE_',''),color=color,s=32)
    for ax,title,key in zip(axs,['N preference (true NLL < new NLL)','N teacher-forced strict'],['success','strict']):
        w=next(r for r in end if r['endpoint']=='W0' and r['kind']=='N');ax.axhline(float(w[key])/10,color='gray',linestyle=':',label='W0')
        ax.set(xlabel='Seen edit requests (saved endpoint)',ylabel='Percent of fixed N1000',title=title);ax.legend();ax.grid(alpha=.2)
    fig.savefig(dest/'endpoint-N.png',dpi=150,metadata={'Software':'delayed-write completed CPU review'});plt.close(fig)
    variants=['dose_0p5','dose_1','dose_minus1','rotation_2026092401','rotation_2026092402','rotation_2026092403']
    pidx={(r['contrast'],r['kind']):r for r in paired};labels=[short(r['pair']) for r in coverage]
    vals=[];counts=[]
    for c in coverage:
        rr=[pidx[(c['pair']+'/'+v+' minus actual11','N')] for v in variants]
        vals.append([float(r['desired_nll_delta']) for r in rr]);counts.append([int(r['gained'])-int(r['lost']) for r in rr])
    fig,axs=plt.subplots(1,2,figsize=(13,7),layout='constrained')
    for ax,x,title,fmt in zip(axs,[np.array(vals),np.array(counts)],['N true-NLL delta: patch minus actual11','N success net change / 1000'],['.3f','d']):
        lim=max(abs(x.min()),abs(x.max()));im=ax.imshow(x,cmap='RdBu_r',vmin=-lim,vmax=lim,aspect='auto',interpolation='nearest')
        ax.set_yticks(range(12),labels);ax.set_xticks(range(6),['λ=.5','λ=1','λ=−1','rot01','rot02','rot03']);ax.set_title(title)
        for i in range(12):
            for j in range(6):ax.text(j,i,format(x[i,j],fmt),ha='center',va='center',fontsize=8,color='white' if abs(x[i,j])>lim*.55 else 'black')
        fig.colorbar(im,ax=ax,shrink=.6)
    fig.savefig(dest/'patch-N.png',dpi=150,metadata={'Software':'delayed-write completed CPU review'});plt.close(fig)

def main():
    end=rows('first-endpoint-table.csv');paired=rows('paired-summary.csv');coverage=rows('coverage.csv');factor=rows('factorial-summary.csv')
    metrics=rows('factorial-patch-metrics.csv');active=rows('active-overwrite.csv');atwrite=rows('e1-atwrite-paired.csv')
    idx={(r['endpoint'],r['kind']):r for r in end if r['panel']!='H_active_variants'}
    pp={(r['contrast'],r['kind']):r for r in paired if r['panel']!='H_active_variants'}
    fm={(r['endpoint'],r['kind']):r for r in metrics if r['panel']!='H_active_variants'}
    names=list(dict.fromkeys(r['endpoint'] for r in end));scope=read(AUDIT/'detail-checks.json');inventory=read(REPORT/'artifact-index-root.json')
    cost=read(OUTPUT/'science-terminal.json');figures(end,paired,coverage)
    text=['# BASE delayed-write E3 완료 상세 CPU 리뷰','',
      f'Instruction / nonce: `{INSTRUCTION}`. 실행 GPU **52823**, CPU collector **52824** 모두 `COMPLETED / 0:0`. G00–G70 11개 receipt의 연결과 승인된 **25 endpoint + 12 E3 조합(48 factorial, 72 modified)**을 확인했다. 이는 기술적 완료이며 가설·효능의 통과 판정이 아니다.',
      ...
      '', '## 1. 범위·실행 및 이력','',
      table(['Job','역할','시작–끝 (Asia/Seoul)','Parent allocation','종료'],[
      ...
      '', table(['Stage','기능','실제 상태'],[[r['stage'],{'G00':'자산/source/lock','G10':'bounded fidelity/repeat/hook/restore','G20':'E1 25 endpoint','G21':'E1 completeness','G30':'core4 factorial','G31':'core module identity','G40':'core4×6 patch','G50':'extension8 factorial','G51':'extension module identity','G60':'extension8×6 patch','G70':'CPU collector'}[r['stage']],r['status']] for r in rows('stage-coverage.csv')]),
      ...
      '', '## 3. E1 고정 패널 결과','',
      table(['Endpoint','R/100','P/200','N/1000','Base/256','joint/100'],[[short(n),*[idx[n,k]['success'] for k in ('R','P','N','BASE','R+twoP_joint')]] for n in names]),
      ...
      '', table(['Endpoint','TF token-micro % R/P/N/Base','TF prompt-macro % R/P/N/Base','strict count R/P/N/Base','joint strict/100'],[
        [short(n),' / '.join(pct(idx[n,k]['TF_token_micro']) for k in ('R','P','N','BASE')),' / '.join(pct(idx[n,k]['TF_prompt_macro']) for k in ('R','P','N','BASE')),
         ' / '.join(idx[n,k]['strict'] for k in ('R','P','N','BASE')),idx[n,'R+twoP_joint']['strict']]
        for n in ['W0','BASE_ALPHAEDIT_W001','BASE_ALPHAEDIT_W050','BASE_ALPHAEDIT_W090','BASE_ALPHAEDIT_W100','BASE_MEMIT_W001','BASE_MEMIT_W010','BASE_MEMIT_W020','BASE_MEMIT_W100']]),
      ...
      '', table(['Endpoint','W1→현재 R gained/lost','P gained/lost','N gained/lost','W0-correct N 잔존/876'],[
        [short(n),*['/'.join((next(x for x in atwrite if x['contrast']==n+' minus own W001' and x['kind']==k and x['panel']!='H_active_variants')['gained'],next(x for x in atwrite if x['contrast']==n+' minus own W001' and x['kind']==k and x['panel']!='H_active_variants')['lost'])) for k in ('R','P','N')],
        str(876-int(pp[n+' minus W0','N']['lost']))+'/876'] for n in ['BASE_ALPHAEDIT_W050','BASE_ALPHAEDIT_W090','BASE_ALPHAEDIT_W100','BASE_MEMIT_W010','BASE_MEMIT_W020','BASE_MEMIT_W100']]),
      ...
      '', table(['Endpoint','General natural NLL','W0 KL','TF micro %','W0 top1/8192'],[[short(n),num(idx[n,'GENERAL']['natural_nll_mean']),num(idx[n,'GENERAL']['W0_forward_KL']),pct(idx[n,'GENERAL']['TF_token_micro']),idx[n,'GENERAL']['W0_top1_matches']] for n in ['W0','BASE_ALPHAEDIT_W001','BASE_ALPHAEDIT_W050','BASE_ALPHAEDIT_W090','BASE_ALPHAEDIT_W100','BASE_MEMIT_W001','BASE_MEMIT_W010','BASE_MEMIT_W020','BASE_MEMIT_W100']]),
      ...
      '', table(['Endpoint','상태','Kind','원 target 성공/분모','당시 current 성공/분모'],[[short(r['endpoint']),r['status'],r['kind'],r['original_success']+'/'+r['original_count'],r['current_success']+'/'+r['current_count']] for r in active if r['endpoint'] in ('BASE_ALPHAEDIT_W050','BASE_ALPHAEDIT_W100','BASE_MEMIT_W020','BASE_MEMIT_W100') and r['status']=='SUPERSEDED']),
      ...
      '', table(['Pair','phase','00','10','01','11','NLL interaction','95% cluster CI'],[
        [short(c['pair']),c['phase'],*[num(fm[c['pair']+'/'+s,'N']['true_nll_mean']) for s in ('00','10','01','11')],
        num(next(r for r in factor if r['pair']==c['pair'] and r['kind']=='N')['true_nll_interaction']),
        '['+num(next(r for r in factor if r['pair']==c['pair'] and r['kind']=='N')['ci_low'])+', '+num(next(r for r in factor if r['pair']==c['pair'] and r['kind']=='N')['ci_high'])+']'] for c in coverage]),
      ...
      '', table(['Pair','actual11 N','λ1 N','gained/lost','Δ true NLL','95% CI','악화 tail q95/q99'],[
        [short(c['pair']),r['before_success']+'/1000',r['after_success']+'/1000',r['gained']+'/'+r['lost'],num(r['desired_nll_delta']),
        '['+num(r['ci_low'])+', '+num(r['ci_high'])+']',num(r['desired_nll_delta_q95'])+' / '+num(r['desired_nll_delta_q99'])]
        for c in coverage for r in [pp[c['pair']+'/dose_1 minus actual11','N']]]),
      ...
      table(['Pair','λ.5','λ1','λ−1','rot01','rot02','rot03'],[[short(c['pair']),*[num(pp[c['pair']+'/'+v+' minus actual11','N']['desired_nll_delta']) for v in ('dose_0p5','dose_1','dose_minus1','rotation_2026092401','rotation_2026092402','rotation_2026092403')]] for c in coverage]),
      '', '### λ1의 다른 패널 side-effect','',
      table(['Pair','R Δcount','P Δcount','Base Δcount','N strict Δcount','General ΔNLL','General ΔKL'],[
        [short(c['pair']),*[str(int(pp[c['pair']+'/dose_1 minus actual11',k]['gained'])-int(pp[c['pair']+'/dose_1 minus actual11',k]['lost'])) for k in ('R','P','BASE')],
         str(int(pp[c['pair']+'/dose_1 minus actual11','N']['strict_gained'])-int(pp[c['pair']+'/dose_1 minus actual11','N']['strict_lost'])),
         num(pp[c['pair']+'/dose_1 minus actual11','GENERAL']['natural_nll_delta']),num(pp[c['pair']+'/dose_1 minus actual11','GENERAL']['KL_delta'])] for c in coverage]),
      ...
      table(['항목','frozen 함수:줄','판정','범위/한계'],[[r['requirement'],Path(r['file']).name+':'+r['function']+':'+r['line'],r['verdict'],r['limitation']] for r in rows('source-conformance.csv')]),
      ...
      '', '## 7. 비용·저장·무결성','',
      table(['구간','forward calls','program increment sec','nested forward sec','selected W H2D bytes'],[[r['stage'],r['forward_calls'],num(r['program_seconds'],3),num(r['forward_seconds'],3),r['selected_weight_H2D_bytes']] for r in rows('compute.csv')]),
      ...
      '', table(['실측','값','주의'],[
        ['52823 GPU parent','12288 sec =3.413333 GPUh','GPU utilization 아님'],['52824 CPU parent','25 sec / GPU0','GPU초 합산0'],
        ['program wall',num(cost['cost']['program_seconds'],6)+'sec','load 포함; 마지막 receipt까지'],
        ['forward nested',num(cost['cost']['forward_seconds'],6)+'sec /39898 calls','program에 이미 포함'],
        ['model load',num(cost['cost']['model_load_seconds'],6)+'sec','program에 이미 포함'],
        ['GPU max allocated',str(cost['cost']['peak_gpu_bytes'])+'B ('+num(cost['cost']['peak_gpu_bytes']/2**30,3)+'GiB)','reserved/Slurm memory와 다른 계측'],
        ['host process ru_maxrss',str(cost['host_maxrss_bytes'])+'B ('+num(cost['host_maxrss_bytes']/2**30,3)+'GiB)','전체 Slurm step peak로 확대하지 않음'],
        ['selected-weight H2D counter',str(cost['cost']['selected_weight_H2D_bytes'])+'B','전체 PCIe/모든 activation 전송량 아님'],
        ['new output fullSHA',str(inventory['members'])+'files /'+str(inventory['bytes'])+'B','원 collector index와 일치; 모델/CP 중복 재해시0']]),
    ...
    # Reproducible derived report only; original source/raw/receipts stay read-only.
    with (REPORT/'report-ko.md').open('w') as f:f.write('\n'.join(text)+'\n')
    print('REPORT_WRITTEN',record(REPORT/'report-ko.md'))
