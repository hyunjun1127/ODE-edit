...
def inspect(text,job,attempt,mode):
 n=2 if mode=='technical' else 4
 name='odeedit_refresh_tech_s4' if mode=='technical' else 'odeedit_refresh_seq1000_s4'
 wall='02:00:00' if mode=='technical' else '12:00:00'
 for field in [f'JobId={job}',f'UserId={pwd.getpwuid(os.getuid()).pw_name}(',f'JobName={name}',
               f'ArrayTaskId=0-{n-1}','NumCPUs=8',f'TimeLimit={wall}','ReqNodeList=server4',str(attempt/'source/project/run_scripts/low_cost_write_donor_pilot/refresh.sbatch'),
               'JobState=PENDING','Reason=JobHeldUser']:
  assert field in text,('HELD_REQUEST_MISMATCH',field)
 assert 'ArrayTaskThrottle=2' in text or f'ArrayTaskId=0-{n-1}%2' in text
...
def submit(attempt,mode):
 ...
 helper='afterany_all_existing_admissions' if deps else command(['env',f'AGENT_GPU_CAPS_FILE={cap}','scripts/check-slurm-resource-cap.sh','server4','2','120832M'],w)
 ...
 n=2 if mode=='technical' else 4
 output=a/('technical' if mode=='technical' else 'output');output.mkdir(mode=0o700,exist_ok=False)
 slurm=control/'slurm';slurm.mkdir(mode=0o700)
 admission=save(control/'admission.json',dict(epoch=time.time(),hostname=command(['hostname'],w).strip(),cap_registry=dict(path=str(cap),sha256=file_sha(cap)),existing=existing,node_snapshot=sq,pending_snapshot=pending,helper=helper,memory=memory,partition=partition,cap_proof='array%2 only after all existing admissions' if deps else 'noexisting+array%2<=2',unrelated_job_mutation=0,gate=gate))
 args=['sbatch','--parsable','--hold',f'--array=0-{n-1}%2',f'--output={slurm}/%A_%a.out',f'--error={slurm}/%A_%a.err']
 if mode=='technical':args+=['--job-name=odeedit_refresh_tech_s4','--time=02:00:00']
 ...
 r=save(control/'released.json',dict(job=job,mode=mode,rows=rows,status='RELEASED',mapping=['native','I1'] if mode=='technical' else lock['array_mapping'],after_initial='CONTINUE_MIDDLE_USER_OVERRIDE',dependencies=deps))
 print(json.dumps(r))

if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--attempt',required=True);p.add_argument('--mode',choices=['technical','main'],required=True)
