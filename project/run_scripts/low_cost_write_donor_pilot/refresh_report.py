...
def report(attempt,root):
 ...
 paragraphs += ['Fullseen의 Current/suffix/old/Historical/first500 rows는 같은 endpoint identity에서 재사용했다. 별도 평가나 추가 독립 분모로 더하지 않는다. Online-at-write1000은 서로 다른10개state의 합이며 finalretention이 아니다.',
  ...
  'PNG는 이 코드가 CSV를 읽어 생성한다: final-six-policy.png, current-batch-curves.png, fixed-first500-retention.png, layer-update-magnitude.png, recorded-compute-cost.png. AI 이미지생성/수동그림수정0. 각PNG SHA는manifest와plot-reproduction.json에결속한다.',
