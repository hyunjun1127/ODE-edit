...
def seal(attempt,root):
 ...
 capsule=dict(execution_commit=lock['worktree_head'],execution_tree=lock['worktree_tree'],
  ...
  common_W4=lock['common_state']['weights']['4'],dataset_root=lock['dataset_root'],dataset_sha256=sha(Path(lock['dataset_root'])/'counterfact.json'),sample_root=lock['sample_root'],
 ...
 m['analysis_source_commit']=subprocess.check_output(['git','rev-parse','HEAD'],cwd=worktree,text=True).strip()
 m['analysis_source_tree']=subprocess.check_output(['git','rev-parse','HEAD^{tree}'],cwd=worktree,text=True).strip()
