#!/usr/bin/env python3
"""No-model deterministic plan for the S05 preservation-all-off pilot."""
...
JOB_NAMES = {
    "llama3-8b-inst": "odeedit_s05_alloffp1r4_llama",
    "qwen2.5-7b-inst": "odeedit_s05_alloffp1r4_qwen",
}
...
def build_plan(source_head: str, *, repository_root: Path = REPO_ROOT) -> dict[str, object]:
    if len(source_head) != 40 or any(
        ...
        raise ODEBFContractError("preservation-all-off dry-plan head differs")
    ...
    for alias in MODEL_ALIASES:
        forecast = forecast_preservation_all_off_panel(artifact, base, alias)
        jobs.append(
            {
                "alias": alias,
                "job_name": JOB_NAMES[alias],
                "result_name": expected_preservation_all_off_result_name(alias),
                "gpu": 1,
                "cpu": 8,
                "memory_mib": 65_000,
                "time": "24:00:00",
    ...
    return {
        "schema": "ode-edit-s05-preservation-alloff-fulltau-p1r4-dry-plan/v1",
...
def main() -> int:
    ...
    print(json.dumps(build_plan(args.source_head, repository_root=args.repository_root), sort_keys=True, separators=(",", ":")))
    return 0
