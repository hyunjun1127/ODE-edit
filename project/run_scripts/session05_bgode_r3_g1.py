#!/usr/bin/env python3
"""Fail-closed BGODE-R3 G1 natural multi-token probe entrypoint."""
...
from project.run_scripts.barrier_guided_ode.r3.g1_probe import INSTRUCTION_ID, run_g1
...
def validate_source_release(head: str, tree: str) -> dict[str, Any]:
    ...
    if observed_head != head or observed_tree != tree:
        raise ValueError("queued G1 source differs")
    if subprocess.check_output(
        ["git", "status", "--porcelain", "--untracked-files=no"], cwd=REPO_ROOT, text=True
    ):
        raise ValueError("queued G1 tracked source is dirty")
    ...
    for member in manifest.get("members", []):
        ...
        if (
            ...
            or subprocess.run(
                ["git", "diff", "--quiet", head, "--", relative], cwd=REPO_ROOT, check=False
            ).returncode != 0
        ):
            raise ValueError(f"G1 source member differs: {relative}")
        records.append(member)
    if sha256_bytes(canonical_json(records).encode("utf-8")) != manifest.get("members_root"):
        raise ValueError("G1 source manifest root differs")
...
def main() -> int:
    ...
    parser.add_argument("--model-alias", choices=tuple(RUN_IDS), required=True)
    parser.add_argument("--run-token", required=True)
    parser.add_argument("--preflight-only", action="store_true")
    args = parser.parse_args()
    if args.run_token != RUN_IDS[args.model_alias]:
        raise ValueError("G1 run token differs")
    ...
    plan: dict[str, Any] = {
        ...
        "batch_size": 1,
        "controller": "T0_FISHER_FULL_IDENTITY_THEN_FISHER_H_PROBE",
        "gpu_count": 1,
        ...
        "schema": "ode-edit-bgode-r3-g1-dry-plan/v1",
    ...
    if args.preflight_only:
        ...
        with offline_environment():
            tokenizer = AutoTokenizer.from_pretrained(**fixed_pretrained_kwargs(fixed_model_spec(args.model_alias)))
        ...
        if tokenization.identity != case["tokenization_identity"]:
            raise ValueError("preflight tokenization differs from the natural seal")
        plan.update(
            {
