#!/usr/bin/env python3
"""Self-contained, create-once P1R16 Stage-A source handoff."""
...
def _read_regular(path: Path) -> bytes:
    ...
    if path.is_symlink() or not stat.S_ISREG(mode):
        raise ODEBFContractError("P1R16 package member is not regular")
...
def _source_tree_scan(source_head: str) -> dict[str, Any]:
    ...
    if forbidden:
        raise ODEBFContractError("P1R16 source tree contains forbidden payload path")
    payload = {
        "schema": "ode-edit-s05-p1r16-source-tree-scan/v1",
...
def _source_inventory(source_head: str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if _git_blob_oid(data) != oid:
            raise ODEBFContractError("P1R16 source blob reconstruction differs")
        entries.append(
            {
                "path": path,
    ...
    payload = {
        "schema": "ode-edit-s05-p1r16-source-tree-inventory/v1",
        "source_head": source_head,
        "entries": entries,
...
def _lock_index(source_head: str) -> dict[str, Any]:
    ...
    for name in LOCK_FILES:
        ...
        if not git:
            raise ODEBFContractError("P1R16 package lock is not committed")
        metadata, observed_path = git.split("\t", 1)
        mode_text, kind, oid = metadata.split(" ", 2)
        if kind != "blob" or observed_path != relative or _git_blob_oid(data) != oid:
            raise ODEBFContractError("P1R16 package lock object differs")
        entries.append(
            {
                "path": relative,
                "mode": int(mode_text, 8),
    ...
    payload = {
        "schema": "ode-edit-s05-p1r16-lock-hash-index/v1",
        "source_head": source_head,
        "entries": entries,
        "entry_root": canonical_hash(entries),
    }
...
def _verify_bundle(bundle: Path, source_head: str) -> dict[str, Any]:
    ...
    if header != [f"{source_head} HEAD"]:
        raise ODEBFContractError("P1R16 bundle advertised ref differs")
    with tempfile.TemporaryDirectory(prefix="p1r16-bundle-proof-") as temporary:
        ...
        if "prerequisite" in verify.stderr.lower():
            raise ODEBFContractError("P1R16 bundle is thin")
        _run(("git", "fetch", str(bundle), "HEAD:refs/heads/p1r16"), cwd=bare)
        imported = _run(("git", "rev-parse", "refs/heads/p1r16"), cwd=bare).stdout.strip()
        if imported != source_head:
            raise ODEBFContractError("P1R16 bundle imported head differs")
        ...
        if parent != EXECUTION_PARENT:
            raise ODEBFContractError("P1R16 bundle parent differs")
    payload = {
        "source_head": source_head,
        "source_tree": tree,
    ...
    with tarfile.open(fileobj=buffer, mode="w", format=tarfile.PAX_FORMAT) as archive:
        for name, data in sorted(members.items()):
            ...
            if path.is_absolute() or ".." in path.parts or not name:
                raise ODEBFContractError("P1R16 archive path differs")
...
def create_package(source_head: str, output_parent: Path) -> dict[str, Any]:
    ...
    if head != source_head or parent != EXECUTION_PARENT or dirty:
        raise ODEBFContractError("P1R16 package source provenance differs")
    locks = REPO_ROOT / "project" / "run_scripts" / "ode_bf" / "locks"
    ...
    closure = validate_p1r16_source_closure(REPO_ROOT)
    destination = output_parent.resolve() / PACKAGE_ID
    if destination.exists() or destination.is_symlink():
        raise ODEBFContractError("P1R16 package destination collides")
    ...
    try:
        ...
        portable = {
            "schema": "ode-edit-s05-p1r16-stage-a-portable-contract/v1",
            ...
            "sh1_server1_pair_execution_owner": True,
            "sh2_read_only_package_verifier": True,
            "sh2_model_gpu_slurm_result_root_action_count": 0,
        }
        ...
        manifest = {
            "schema": "ode-edit-s05-p1r16-stage-a-package-manifest/v1",
        ...
        receipt = {
            "schema": "ode-edit-s05-p1r16-stage-a-handoff-receipt/v1",
...
def verify_local_package(package_directory: Path, source_head: str) -> dict[str, Any]:
    ...
    if root.name != PACKAGE_ID or root.is_symlink() or not root.is_dir():
        raise ODEBFContractError("P1R16 package root differs")
    ...
    for value, label in ((manifest, "manifest"), (receipt, "receipt")):
        ...
        if observed != canonical_hash(value):
            raise ODEBFContractError(f"P1R16 package {label} root differs")
    ...
    if (
        ...
        raise ODEBFContractError("P1R16 package outer identity differs")
    rows = manifest.get("members")
    if not isinstance(rows, list) or manifest.get("member_root") != canonical_hash(rows):
        raise ODEBFContractError("P1R16 package manifest differs")
    with tarfile.open(fileobj=io.BytesIO(archive), mode="r:") as tar:
        ...
        if any(
            ...
            raise ODEBFContractError("P1R16 package archive traversal differs")
        names = [item.name for item in tar_members]
        if len(names) != len(set(names)) or names != [row["path"] for row in rows]:
            raise ODEBFContractError("P1R16 package archive inventory differs")
        extracted: dict[str, bytes] = {}
        for item in tar_members:
            handle = tar.extractfile(item)
            if handle is None:
                raise ODEBFContractError("P1R16 package member is unreadable")
    ...
    for row in rows:
        ...
        if (
            ...
            raise ODEBFContractError("P1R16 package member identity differs")
    with tempfile.TemporaryDirectory(prefix="p1r16-local-verify-") as temporary:
