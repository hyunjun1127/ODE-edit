#!/usr/bin/env python3
"""Fail-closed server1 entry for the S05 target-new-NLL routing panel."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="session05-ode-bf-target-new-nll", allow_abbrev=False
    )
    parser.add_argument("--model", required=True, choices=MODEL_ALIASES)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--source-head", required=True)
    parser.add_argument(
        "--run-token", required=True, choices=(TARGET_NEW_RESULT_TOKEN,)
    )
    return parser
