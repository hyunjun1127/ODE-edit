...
READY_MANIFEST = Path(os.environ.get('EN_ADAPT_READY_MANIFEST', '/data/janghj/ODE-edit/local/runtime/server3-experiment-ready-v1/manifest.json'))
...
def verify_ready_inputs(manifest):
    ...
    reference_binding = READY_MANIFEST.parent / 'reference-stat-binding.json'
    if manifest.get('node') == 'server4':
        if not reference_binding.is_file():
            raise ValueError('S4_REFERENCE_STAT_SEAL_REQUIRED')
        for item in json.loads(reference_binding.read_text()):
            st=Path(item['path']).stat()
            if st.st_size!=item['bytes'] or [st.st_dev,st.st_ino,st.st_mtime_ns]!=item['stat']:
                raise ValueError('REFERENCE_STABLE_IDENTITY_CHANGED:'+item['path'])
...
class Runtime:
    def __init__(self, output):
        ...
        if not os.environ.get('SLURM_JOB_ID') or os.environ.get('SLURMD_NODENAME') != manifest.get('node', 'ubuntu'):
            raise ValueError('LOCKED_NODE_SLURM_ALLOCATION_REQUIRED')
        ...
        if manifest.get('node')=='server4' and int(os.environ.get('SLURM_MEM_PER_NODE','0'))>60416:
            raise ValueError('S4_MEMORY_CEILING')
        ...
        if (sys.version_info[:3], str(torch.__version__), transformers.__version__,
                numpy.__version__, scipy.__version__) != (tuple(manifest.get('python_version', [3, 12, 3])), '2.9.1+cu128', '4.44.2', '2.2.6', '1.15.3'):
            raise ValueError('PINNED_READY_PACKAGE_VERSIONS')
        if manifest.get('transformers_import') and str(Path(transformers.__file__).resolve()) != manifest['transformers_import']:
            raise ValueError('PINNED_TRANSFORMERS_IMPORT_PATH')
