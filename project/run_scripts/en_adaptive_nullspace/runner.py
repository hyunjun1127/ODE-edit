...
ROOT=Path(os.environ.get('EN_ADAPT_INPUT_ROOT','/data/janghj/ODE-edit/local/en-adaptive-nullspace/20260920-v1'))
...
def main_run(output):
 start=time.monotonic();rt=Runtime(output)
 binding=rt.ready_manifest
 obj=Objective(rt.model,binding.get('generated_root',ROOT/'inputs/generated-v1'),binding.get('reference_inputs',ROOT/'inputs/reference-inputs.json'));rt.objective=obj
 if binding.get('basis_pt'):
  basis_payload=torch.load(binding['basis_pt'],weights_only=True,mmap=True,map_location='cpu')
  V=basis_payload['basis'].numpy() if isinstance(basis_payload,dict) else basis_payload.numpy()
 else:V=np.load(ROOT/'inputs/pstar-derived-v1/basis.npy',mmap_mode='r',allow_pickle=False)
 ...
 for batch in range(1,4):
  ...
  for group in native_groups:
   entry=states[group] if group in states else dict(weight=rt.W0,M=torch.zeros_like(rt.P),ledger=[])
   ...
   # Science geometry consumes keys only. Release redundant prefix caches
   # before TSQR; T0 separately retains its oracle for physical comparisons.
  ...
  for arm in arms:
   ...
   save(bdir/f'{arm}-commit.json',dict(receipt=committed['receipt'],active_past_ids=p['active_ids'],ledger=registry_status(states[arm]['ledger'],[]),history_append=1,checkpoint=False))
