...
def run_controller(native_weight, gradient, first_delta, loss0, evaluate, geometry, *,
    ...
    # Design §6: ray curvature uses the fixed ideal D1. Armijo above
    # deliberately uses actual FP32 displacement; these are not interchangeable.
    slope = first['ideal_gradient_inner_product']
    if slope < 0:
        ...
        first['quadratic'] = dict(scale=None, reason='NO_IDEAL_DESCENT_DIRECTION')
    if accepted:
