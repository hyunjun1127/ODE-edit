#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R52 Phase-1B Atomic IL3-FULL."""
...
from project.run_scripts.session05_ode_bf_p1r52_target_depth_atomic_dry_plan import build_plan


SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r52_target_depth_atomic.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r52-target-depth-phase1b-v1"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r52-target-depth-phase1b-v1"
BRANCH = "codex/p1r52-target-depth-il1-il3full-v1"
PROJECT_GPU_CAP = 4
...
def active_gpus() -> tuple[int, list[dict[str, object]]]:
    lines = run(["squeue", "-h", "-u", "janghj", "-w", "server2", "-t", "RUNNING,CONFIGURING", "-o", "%i|%T|%b|%j"]).stdout.splitlines()
    total = 0
    jobs = []
    for line in lines:
        ...
        if count:
            total += count
            jobs.append({"job_id": job, "state": state, "gres": gres, "name": name, "gpu": count})
    return total, jobs
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R52 Phase-1B execution source differs")
    plan = build_plan(source_head)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["new_gpu_cells"]):
        raise ODEBFContractError("P1R52 Phase-1B result namespace exists")
    active, jobs = active_gpus()
    concurrency = min(4, PROJECT_GPU_CAP - active)
    if concurrency <= 0 or active + concurrency > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R52 Phase-1B GPU cap unavailable")
    namespace = f"s05-p1r52-target-depth-phase1b-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R52 Phase-1B submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-3%{concurrency}"
    intent = {
        "schema": "ode-edit-s05-p1r52-target-depth-phase1b-intent/v1",
    ...
    submitted = run([
        "sbatch", "--hold", "--parsable", "--array", array, "--chdir", str(REPO_ROOT),
        "--nodelist", "server2", "--job-name", "odeedit_s05_p1r52_target_depth_phase1b",
    ...
    if not job_id.isdigit() or not all(value in observed for value in ("JobState=PENDING", "Reason=JobHeldUser", "ReqNodeList=server2", "TRES=cpu=8,mem=60416M,node=1,billing=8,gres/gpu=1")):
        run(["scancel", job_id], check=False)
        raise ODEBFContractError("P1R52 Phase-1B held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-target-depth-phase1b-submission/v1",
        "source_head": source_head,
        "job_id": job_id,
        "array": array,
        "cell_count": 4,
