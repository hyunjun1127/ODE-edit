#!/usr/bin/env python3
"""Held-inspect-release submitter for P2R1 target-only Gate A."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p2r1_target_only.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p2r1-rms-tangent-target-only"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p2r1-rms-tangent-target-only"
BRANCH = "codex/p2r1-baseline-calibrated-rms-tangent-target-flow-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P2R1 execution source differs")
    if case_count not in (1, 10):
        raise ODEBFContractError("P2R1 case count differs")
    if attempt_suffix is not None and not attempt_suffix.replace("-", "").isalnum():
        raise ODEBFContractError("P2R1 attempt suffix differs")
    plan = dry.build_plan(
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P2R1 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-1%{stage_concurrency}"
    node_receipt = _run(["scontrol", "show", "node", "-o", "devbox"]).stdout.strip()
    intent = {
        "schema": "ode-edit-s05-p2r1-target-only-intent/v1",
    ...
    command = [
        ...
        "--job-name", f"odeedit_s05_p2r1_target_{phase.replace('-', '_')}",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P2R1 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P2R1 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p2r1-target-only-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "phase": phase,
        "job_id": job_id,
