#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R35 B1 and production."""
...
from project.run_scripts import session05_ode_bf_p1r35_full_current_residual_dry_plan as dry
from project.run_scripts.ode_bf.contracts import ODEBFContractError

SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r35_full_current_residual.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r35-full-current-residual"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
BRANCH = "codex/p1r35-p1r34-full-current-residual-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def _active_gpu_jobs() -> int:
    lines = _run(
        ["squeue", "-h", "-u", "janghj", "-w", "devbox", "-t", "RUNNING", "-o", "%i|%b"]
    ).stdout.splitlines()
...
def submit(source_head: str, phase: str) -> dict[str, object]:
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R35 execution source differs")
    plan = dry.build_plan(source_head, phase)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R35 result namespace exists")
    active = _active_gpu_jobs()
    if active + STAGE_GPU_MAX > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R35 server1 GPU cap differs")
    namespace = f"s05-p1r35-{phase}-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R35 submission namespace exists")
    log_root = REPO_ROOT / f"local/odebf/logs/p1r35-{phase}-{source_head[:12]}"
    log_root.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-p1r35-submission-intent/v1",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R35 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R35 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r35-submission/v1",
        "source_head": source_head,
        "phase": phase,
        "job_id": job_id,
        "array": "0-1%2",
