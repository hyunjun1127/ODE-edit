#!/usr/bin/env python3
"""Fail-closed P1R37 instantaneous-freeze independent B10x10 runner."""
...
from project.run_scripts.ode_bf.p1r37_independent_b10x10_runtime import METHODS
from project.run_scripts.ode_bf.p1r37_instantaneous_freeze import INSTRUCTION_ID


RUN_TOKEN = "p1r37-no-persistent-freeze-independent-b10x10-v1"
SOURCE_MANIFEST = "source_manifest_s05_p1r37_no_persistent_freeze_independent_b10x10.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", P1R36_PARENT, source_head],
        cwd=REPO_ROOT,
        check=False,
    ).returncode != 0:
        raise ValueError("P1R37 P1R36 scientific ancestry differs")
    for relative, expected in PROTECTED_PARENT_SHA256.items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise ValueError("P1R37 protected parent source differs")
    manifest_path = REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST
    manifest, raw_sha = load_rooted_json(
        manifest_path,
        expected_schema=(
            "ode-edit-s05-p1r37-no-persistent-freeze-independent-b10x10-"
            "source-manifest/v1"
        ),
    )
    ...
    if (
        ...
        or manifest.get("parent_checkpoint") != P1R36_PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P1R37 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P1R37 source manifest entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P1R37 source path differs")
        if (
    ...
    changed = set(
        subprocess.run(
            ["git", "diff", "--name-only", P1R36_PARENT, source_head],
            cwd=REPO_ROOT,
            check=True,
            text=True,
    ...
    if observed != sorted(set(observed)) or changed != set(observed) | {
        ...
        raise ValueError("P1R37 source closure differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        _, lock_sha = load_and_validate_lock(
        ...
        result = run_p1(
            ...
            p1r37_independent_b10x10_method=args.method,
        )
        result = {
            **result,
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r37-no-persistent-freeze-job-failure/v1",
        )
        print(
            json.dumps(
                {
