#!/usr/bin/env python3
"""Held-inspect-release submitter for the PIR-U sequential B1000 cell."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r52_piru_sequential_b100x10.sbatch"
BRANCH = "codex/p1r52-piru-seq-10xb100-hon-v1"
PROJECT_GPU_CAP = 4
...
def submit(source_head: str) -> dict[str, object]:
    ...
    state_root = REPO_ROOT / "local/odebf/state/p1r52-piru-sequential-10xb100-v1"
    log_root = REPO_ROOT / "local/odebf/logs/p1r52-piru-sequential-10xb100-v1"
    ...
    if allocated + 1 > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R52 PIR-U project GPU cap differs")
    ...
    namespace = f"p1r52-piru-sequential-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R52 PIR-U submission namespace exists")
    log_root.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-p1r52-piru-sequential-submission-intent/v1",
    ...
    submitted = _run(
        [
            ...
            "--nodelist", "devbox", "--job-name", "odeedit_s05_p1r52_piru_seq_b1000",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R52 PIR-U scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R52 PIR-U held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-piru-sequential-submission/v1",
        "source_head": source_head,
        "branch": branch,
        "job_id": job_id,
        "request_count": 1000,
