#!/usr/bin/env python3
"""Held-inspect-release submitter for the two P1R52 sequential jobs."""
...
from project.run_scripts import session05_ode_bf_p1r52_sequential_dry_plan as dry
from project.run_scripts.ode_bf.contracts import ODEBFContractError


SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r52_sequential.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r52-llama-soft-sequential-historical-10xb10-v1"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r52-llama-soft-sequential-historical-10xb10-v1"
BRANCH = "codex/p1r52-llama-soft-seq-hist-10xb10-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def _run(args: list[str], *, check: bool = True) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        args, cwd=REPO_ROOT, check=check, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def _active_gpu_allocations() -> tuple[int, list[dict[str, object]]]:
    lines = _run(
        ["squeue", "-h", "-u", "janghj", "-w", "devbox", "-t", "RUNNING,CONFIGURING", "-o", "%i|%T|%b"]
    ).stdout.splitlines()
    total = 0
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R52 sequential execution source differs")
    plan = dry.build_plan(source_head)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R52 sequential result namespace exists")
    active, active_jobs = _active_gpu_allocations()
    if active + STAGE_GPU_MAX > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R52 sequential server1 GPU cap would be exceeded")
    memory_mib = _host_memory_available_mib()
    namespace = f"s05-p1r52-sequential-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R52 sequential submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-p1r52-sequential-submission-intent/v1",
    ...
    command = [
        ...
        "--job-name", "odeedit_s05_p1r52_seq",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R52 sequential scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R52 sequential held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-sequential-submission/v1",
        "source_head": source_head,
        "branch": branch,
        "job_id": job_id,
        "array": "0-1%2",
