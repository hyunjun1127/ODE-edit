#!/usr/bin/env python3
"""Run V3 sequential P1 without replaying the legacy R2 diagnostic."""
...
EXECUTION_TOKEN = "oracle-absolute-mean-margin-v3-p1-after-p0-pass"
P1_OUTPUT_PREFIX = "session02-oracle-absolute-mean-margin-v3-p1"
...
def _exact_output(repo: Path, raw: Path, model_alias: str, proposal_id: str) -> Path:
    ...
    if candidate.resolve() != expected:
        raise RuntimeError("V3 P1 output root differs from its dry plan")
    return candidate
...
def dry_plan(oracle_lock: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-dry-plan/v1",
...
def _total_terminal_c_energy(row: Mapping[str, Any]) -> float | None:
    ...
    if not values or not all(math.isfinite(value) and value >= 0.0 for value in values):
        raise RuntimeError("V3 P1 terminal C-energy is invalid")
...
def run(args: argparse.Namespace) -> int:
    ...
    if easyedit_root != EASYEDIT_ROOT.resolve(strict=True):
        raise RuntimeError("V3 P1 EasyEdit root differs")
    ...
    if (
        ...
        raise RuntimeError("V3 P1 base lock identity differs")
    ...
    if output_arg.exists():
        raise RuntimeError("V3 P1 output root already exists")
    if os.environ.get("ODEEDIT_ORACLE_ABSOLUTE_V3_P1_SUBMISSION_AUTHORIZED") != EXECUTION_TOKEN:
        raise RuntimeError("conditional V3 P1 execution authority is required")
    for key in ("HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE"):
        if os.environ.get(key) != "1":
            raise RuntimeError(f"{key}=1 is required")
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError("V3 P1 requires exactly one visible GPU")
    ...
    if _request_ids(requests) != base_lock["selection"]["request_ids"]:
        raise RuntimeError("V3 P1 request identity differs")
    ...
    if config.event_tolerance != oracle_lock["event"]["oracle_validity_epsilon"]:
        raise RuntimeError("V3 P1 validity epsilon differs")
    weights = {
    ...
    if setup_seconds < 0.0:
        raise RuntimeError("V3 P1 setup timer became negative")
    setup_share = setup_seconds / (len(P1_ARMS) * len(requests))
    ...
    manifest = {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-manifest/v1",
        ...
        "legacy_diagnostic_provenance": {
            **dict(oracle_lock["legacy_diagnostic_provenance"]),
            "legacy_replay_executed": False,
            "existing_legacy_diagnostic_only": True,
        },
        "artifact_firewall": {
        ...
        for arm in P1_ARMS:
            ...
            for order_position, request in enumerate(requests):
                ...
                if failure_exact is False or success_exact is False:
                    raise RuntimeError("V3 P1 edit transaction identity differs")
                ...
                if calibration is None or target is None:
                    raise RuntimeError("V3 P1 calibration is absent")
                if (
                    ...
                    raise RuntimeError("V3 P1 counter contract differs")
                ...
                if result.status == "event_hit" and max(
                    ...
                    raise RuntimeError("V3 P1 hit violates its aggregate floors")
                common = {
            ...
            for bundle in pending:
                ...
                controller_row = {
                    "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-controller/v1",
                    **common,
                ...
                compute_row = {
                    "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-compute/v1",
                    **common,
                    **snapshot,
                    **counters,
                    ...
                    "oracle_extra_model_forwards": bundle[
                        "calibration"
                ...
                mechanism_row = {
                    "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-mechanism/v1",
                    **common,
                    "mechanism": mechanism,
                    "sequential_state": sequential,
                }
                strength_row = {
                    "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-strength/v1",
                    **common,
    ...
    geometry_relative_to_native = []
    for position, rows in sorted(compute_by_position.items()):
        native = rows[Arm.NATIVE_MEMIT.value]
        native_energy = _total_terminal_c_energy(native)
        native_distance = native.get("terminal_net_c_distance")
        for arm in P1_ARMS:
            current = rows[arm.value]
            current_energy = _total_terminal_c_energy(current)
            current_distance = current.get("terminal_net_c_distance")
            geometry_relative_to_native.append(
                {
                    "case_id": case_ids[position],
                    "order_position": position,
                    "arm": arm.value,
                    ...
                    "terminal_c_distance": current_distance,
                    "native_terminal_c_energy_total": native_energy,
                    "native_terminal_c_distance": native_distance,
                    "energy_over_native": (
                    ...
                    "distance_over_native": (
                        float(current_distance) / float(native_distance)
                        if current_distance is not None
                        and native_distance is not None
                        and float(native_distance) > 0.0
                        else None
                    ),
                    "divergent_sequential_arm_histories": True,
                }
    ...
    case_12498_full = next(
        row
        for row in compact_records
        if row["arm"] == Arm.FULL_ODE_EDIT.value and row["case_id"] == "12498"
    )
    v2_case12498_counterfactual = {
        "model": args.model_alias,
        "case_id": "12498",
        "v3_status": case_12498_full["status"],
        "v3_first_hit_round": (
            case_12498_full["K_acc"]
            if case_12498_full["status"] == "event_hit"
            else None
        ),
        "saved_accepted_rounds_vs_v2_smax6": (
            6 - case_12498_full["K_acc"]
            if case_12498_full["status"] == "event_hit"
            else 0
        ),
        "derived_only_from_actual_v3_execution": True,
        "pre_run_prediction_used": False,
    }

    summary = {
        "schema_version": "ode-edit-oracle-absolute-mean-margin-v3-p1-summary/v1",
        "status": "COMPLETE_DEVELOPMENT_NO_EVALUATION",
        "model": args.model_alias,
        "record_count": records,
        "sequential_streams": stream_summaries,
        "v3_records": compact_records,
        "v3_compute_ratios": ratios,
        "geometry_relative_to_native": geometry_relative_to_native,
        "v2_case12498_counterfactual": v2_case12498_counterfactual,
        "legacy_diagnostic_provenance": dict(
            oracle_lock["legacy_diagnostic_provenance"]
        ),
        "legacy_replay_executed": False,
        "existing_legacy_diagnostic_only": True,
        "evaluation_count": 0,
