#!/usr/bin/env python3
"""Held-inspect-release submitter for P2R7."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p2r7_atomic.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p2r7-p2target-p1dw-shared-writer"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p2r7-p2target-p1dw-shared-writer"
BRANCH = "codex/p2r7-p2target-p1dw-shared-writer-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P2R7 execution source differs")
    if phase not in ("pilot", "b10x10"):
        raise ODEBFContractError("P2R7 phase differs")
    if attempt_suffix is not None and not attempt_suffix.replace("-", "").isalnum():
        raise ODEBFContractError("P2R7 attempt suffix differs")
    plan = dry.build_plan(source_head, phase=phase, attempt_suffix=attempt_suffix)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P2R7 result namespace exists")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P2R7 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    node = _run(["scontrol", "show", "node", "-o", "devbox"]).stdout.strip()
    array = f"0-1%{concurrency}"
    intent = {
        "schema": "ode-edit-s05-p2r7-submission-intent/v1",
        ...
        "node_resource_receipt_sha256": hashlib.sha256(node.encode()).hexdigest(),
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P2R7 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P2R7 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p2r7-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "phase": phase,
        "job_id": job_id,
