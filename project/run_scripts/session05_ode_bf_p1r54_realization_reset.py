#!/usr/bin/env python3
"""Fail-closed launcher for one realization-reset array cell."""
...
RUN_TOKEN = "p1r54-realization-reset-four-runs-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("P1R54 reset pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema") != "ode-edit-s05-p1r54-realization-reset-final-pre-gpu/v1"
        ...
        or value.get("required_array") != "0-3%4"
        or value.get("source_manifest") != source_manifest(source_head, source_tree)
    ):
        raise ODEBFContractError("P1R54 reset pre-GPU content differs")
    value["identity_sha256"] = identity
    return value


def _owned_runtime_root(path: Path, *, parent: Path, expected_name: str) -> Path:
    ...
    if (
        ...
        raise ODEBFContractError("P1R54 reset create-once runtime root differs")
    return resolved
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            raise ODEBFContractError("P1R54 reset source/output differs")
        ...
        if final["dry_plan"]["cells"][args.cell]["role"] != config.role:
            raise ODEBFContractError("P1R54 reset released cell differs")
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("P1R54 reset deployment differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r54-realization-reset-cell-timing/v1",
        ...
        result.update(
            {
                ...
                "state_terminal_sha256": _atomic_write_once(
                    ...
                    {
                        "schema": "ode-edit-s05-p1r54-realization-reset-cell-state/v1",
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r54-realization-reset-cell-failure/v1",
        )
