#!/usr/bin/env python3
"""Fail-closed PIR-U post-energy-WARN R1 sequential 10xB100 runner."""
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("PIR-U post-energy-WARN source ancestry differs")
    ...
    if (
        ...
        raise ValueError("PIR-U post-energy-WARN source manifest header differs")
    observed: list[str] = []
    for entry in manifest["entries"]:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("PIR-U post-energy-WARN source entry differs")
        ...
        if (
            ...
            raise ValueError("PIR-U post-energy-WARN source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("PIR-U post-energy-WARN source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result.update(
            {
                "postenergy_warn_lock_sha256": lock_sha,
                "postenergy_warn_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r52-piru-postenergy-warn-job-failure/v1",
        )
        print(
            json.dumps(
                {
