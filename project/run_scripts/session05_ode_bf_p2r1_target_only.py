#!/usr/bin/env python3
"""Fail-closed P2R1 target-only runner."""
...
from project.run_scripts.ode_bf.p2r1_rms_tangent_target import P2R1_INSTRUCTION_ID
...
RUN_TOKEN = "p2r1-baseline-calibrated-rms-tangent-target-only-v1"
SOURCE_MANIFEST = "source_manifest_s05_p2r1_rms_tangent_target_only.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P2R1 source ancestry differs")
    manifest, raw_sha = load_rooted_json(
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST,
        expected_schema="ode-edit-s05-p2r1-rms-tangent-source-manifest/v1",
    )
    entries = manifest.get("entries")
    if (
        manifest.get("instruction_id") != P2R1_INSTRUCTION_ID
        or manifest.get("parent") != PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P2R1 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P2R1 source manifest entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P2R1 source path differs")
        if path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P2R1 source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P2R1 source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_p1(
            ...
            p2r1_target_only_case_count=args.case_count,
            p2r1_attempt_suffix=args.attempt_suffix,
        )
        result = {
            **result,
            "p2r1_lock_sha256": lock_sha,
            "p2r1_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p2r1-target-only-job-failure/v1",
        )
        print(
            json.dumps(
                {
