#!/usr/bin/env bash
# Fail-closed one-shot submit helper for the exact paired MV-1 C0 pilot.
...
readonly SBATCH_FILE="${REPO_ROOT}/project/run_scripts/session01_mv1_c0_pair_server1.sbatch"
readonly RED_GATE="${REPO_ROOT}/audits/global/2026-07-30-session01-mv1-c0-pilot-execution-preflight.md"
...
readonly PAIR_MARKER="${STATE_ROOT}/mv1_c0p_pair_v1.submitted"
readonly JOB_NAME="odeedit_mv1_c0p_pair_v1"

[[ "$#" -eq 0 ]] || {
  echo "this paired MV-1 C0 pilot helper takes no arguments" >&2
  exit 2
}
...
"${GIT_BIN}" ls-files --error-unmatch \
  project/run_scripts/session01_mv1_c0_server1.sbatch \
  project/run_scripts/session01_mv1_c0_pair_server1.sbatch \
  project/run_scripts/submit_session01_mv1_c0_pair_server1.sh \
  project/run_scripts/ode_edit_motivation/mv0_fidelity.py \
  project/run_scripts/ode_edit_motivation/mv1_calibration.py \
  project/run_scripts/ode_edit_motivation/tests/test_mv1_calibration.py \
  plans/global/2026-07-30-session01-mv1-implementation-spec.md \
  audits/global/2026-07-30-mv0-pair-c3-v3.postrun.md \
  audits/global/2026-07-30-session01-mv1-c0-pilot-execution-preflight.md \
  >/dev/null
[[ -z "$("${GIT_BIN}" status --porcelain --untracked-files=normal)" ]] || {
  echo "paired MV-1 C0 pilot submission requires a clean repository" >&2
  exit 2
}
[[ "$("${GIT_BIN}" rev-parse HEAD)" == "$("${GIT_BIN}" rev-parse origin/main)" ]] || {
  echo "paired MV-1 C0 pilot submission requires main == origin/main" >&2
  exit 2
}
grep -Fxq -- \
  '- 최종 판정: `PASS` — MV-1 C0 3-case 동시 pilot 한 건에만 유효' \
  "${RED_GATE}" || {
  echo "MV-1 C0 pilot execution preflight has not passed" >&2
  exit 2
}
grep -Fxq -- \
...
[[ ! -e "${OUTPUT_ROOT}/mv1_llama_c0p_v1" ]] || {
  echo "Llama pilot output already exists" >&2
  exit 2
}
[[ ! -e "${OUTPUT_ROOT}/mv1_qwen_c0p_v1" ]] || {
  echo "Qwen pilot output already exists" >&2
  exit 2
}
[[ ! -e "${PAIR_MARKER}" ]] || {
  echo "MV-1 C0 pilot one-shot marker already exists" >&2
  exit 2
}
if [[ -n "$("${SQUEUE_BIN}" -h --name="${JOB_NAME}")" ]]; then
  echo "paired MV-1 C0 pilot job is already active" >&2
  exit 2
fi
