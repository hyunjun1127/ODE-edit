...
def publish(report,output,repo):
    ...
    fig.savefig(report/'b1-canonical-cost.png',metadata={'Software':'BPCW CPU publication.py'});plt.close(fig)
    ...
    refs=table(s['references'],['arm','split','documents','positions','retained_sequences','token_flips','EOS_sequences','censored_sequences','mean_d'])
...
space status `{s['technical']['space_status']}`; rank/spectrum/ambiguity와 실제 검사는 edit-null-space.json 및 기술 receipt를 참조한다.
...
'''
    ...
    create_json(report/'publication-checks.json',dict(GFM_column_rows_checked=table_rows,internal_pipes_escaped=True,
        ...
        code_generated_PNG=True,raw_prompt_or_tensor_in_report=False,counts_from_independent_reducer=True))
