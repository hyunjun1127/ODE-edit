#!/usr/bin/env python3
"""Model-free deterministic P1R15 Stage-A plan."""
...
def build_plan(source_head: str, *, repository_root: Path = REPO_ROOT) -> dict[str, object]:
    if not _valid_head(source_head):
        raise ODEBFContractError("P1R15 dry-plan source head differs")
    ...
    source, source_sha = load_and_validate_p1r15_source_manifest(
        root, locks / P1R15_SOURCE_MANIFEST_FILE, source_head=source_head
    )
    closure = validate_p1r15_source_closure(root)
    ...
    for alias in MODEL_ALIASES:
        ...
        forecasts[alias] = p1r15_stage_a_forecast(alias).raw_free_payload()
    ...
    payload: dict[str, object] = {
        "schema": "ode-edit-s05-p1r15-stage-a-dry-plan/v1",
        ...
        "source_contract": p1r15_source_contract(),
        ...
        "stage_a_context_sha256_by_alias": {
            alias: expected_p1r15_stage_a_context_sha256(seal, alias)
            for alias in MODEL_ALIASES
        },
        ...
        "server1_owner": LLAMA_ALIAS,
        "server2_owner_after_package_pass": QWEN_ALIAS,
...
def main() -> int:
    ...
    if any(os.environ.get(name) != "1" for name in ("HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE")):
        raise ODEBFContractError("P1R15 dry-plan offline environment differs")
    print(
        json.dumps(
