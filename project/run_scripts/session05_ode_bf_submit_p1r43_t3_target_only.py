#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R43_T3 target-only Gate A."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r43_t3_target_only.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r43-t3-fixed-w0-target-horizon"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r43-t3-fixed-w0-target-horizon"
BRANCH = "codex/p1r43-t3-fixed-w0-target-horizon-ablation-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R43_T3 execution source differs")
    if case_count not in (1, 10):
        raise ODEBFContractError("P1R43_T3 case count differs")
    if attempt_suffix is not None and not attempt_suffix.replace("-", "").isalnum():
        raise ODEBFContractError("P1R43_T3 attempt suffix differs")
    plan = dry.build_plan(
    ...
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R43_T3 result namespace exists")

    active, active_jobs = _active_gpu_allocations()
    available = PROJECT_GPU_CAP - active
    if available <= 0:
        raise ODEBFContractError("P1R43_T3 server1 project GPU cap has no free allocation")
    stage_concurrency = min(STAGE_GPU_MAX, available)
    if active + stage_concurrency > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R43_T3 server1 project GPU cap differs")
    phase = "first-b10" if case_count == 1 else "b10x10"
    namespace_tail = (
        f"{attempt_suffix}-{source_head[:12]}" if attempt_suffix else source_head[:12]
    )
    namespace = f"s05-p1r43-t3-fixed-w0-target-horizon-{phase}-{namespace_tail}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R43_T3 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-1%{stage_concurrency}"
    node_receipt = _run(["scontrol", "show", "node", "-o", "devbox"]).stdout.strip()
    intent = {
        "schema": "ode-edit-s05-p1r43-t3-fixed-w0-target-horizon-intent/v1",
    ...
    command = [
        ...
        "--job-name", f"odeedit_s05_p1r43_t3_target_{phase.replace('-', '_')}",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R43_T3 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R43_T3 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r43-t3-fixed-w0-target-horizon-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "phase": phase,
        "job_id": job_id,
        ...
        "request_attempt_count": 20 * case_count,
