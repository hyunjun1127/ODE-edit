...
def first_b100(root,output):
    ...
    qwen_measured=(root/'chain-3-qwen2.5-7b-inst-JV_NATIVE/batch-01/complete.json').exists()
    if qwen_measured:names.append((3,'qwen2.5-7b-inst','JV_NATIVE'))
    ...
    if qwen_measured:
        if any(records[1][2][k]!=records[3][2][k] for k in ('W_entry','M_entry','request_order_sha256')) or records[1][3]['fixed_z_sha256']!=records[3][3]['fixed_z_sha256']:
            raise RuntimeError('QWEN_FIRST_B100_COMMON_ENTRY_Z')
    ...
    qjvp=records[3][1]['jvp_ledger']['wall_seconds'] if qwen_measured else jvp*ratio
    cost[4]=cost[2]-.8*jvp;cost[5]=cost[3]-.8*qjvp
    ...
    for i in range(6):
        ...
        estimates.append(dict(cell=i,alias=aliases[i],arm=arms[i],first_B100_seconds=cost[i],
            first_B100_status='MEASURED' if i in records else 'UNMEASURED_ENGINEERING_PROXY',
    ...
    text=['# Alpha-JV sequential — 첫 B100 및 비용 예고',f'\nKST: {stamp}',
        ('\n네 cell의 첫 B100이 실제 완료됐다. 아래 성능은 모두 실측이다. ' if qwen_measured else '\n세 cell의 첫 B100이 실제 완료됐다. Qwen JV는 cap3에 따라 queued이며 아래 성능 표에는 추정하지 않는다. ')+
        '이는 at-write current-B100이며 sequential final-W10 retention 결과가 아니다.',
    ...
    text+=['\n'+('두 모델 각각' if qwen_measured else 'Llama')+' O/JV는 첫 W0/M0, request/order와 accepted-z가 exact 일치한다. 이후에는 각 arm의 자기 W/M에서 z를 다시 계산하므로 shared-z 인과 비교로 부르지 않는다.',
        ...
        ('\nQwen JV까지 B100 비용은 실측으로 대체했다. ' if qwen_measured else '\nQwen JV는 아직 B100 실측이 없어 Llama JV write 비용에 두 모델의 이미 완료된 2-batch smoke writer 비율을 적용한 engineering proxy다. ')+
        'L8-only는 20→4 JVP 감소만 반영한 proxy이며 실제 overhead를 숨기지 않는다. 과학 metric imputation0.',
    ...
    manifest=dict(source_head=source['head'],source_tree=source['tree'],sample_root=sample['ordered_root'],
        ...
        valid_first_B100_cells=sorted(records),queued_cell=None if qwen_measured else 3,scientific_imputation_count=0,model_replay_count=0)
    ...
    jwrite(out/'rooted-first-b100-receipt.json',dict(
        ...
        status='FIRST_B100_ALL_4_MEASURED' if qwen_measured else 'FIRST_B100_PARTIAL_3_OF_4_MEASURED',scientific_imputation_count=0,model_replay_count=0))
    return out
