#!/usr/bin/env python3
"""Fail-closed P2R4 Phase-B clamp ON/OFF paired-writer runner."""
...
from project.run_scripts.ode_bf.p2r4_phaseb_atomic_runtime import (
    CLAMP_POLICIES,
    P2R4_PHASEB_INSTRUCTION_ID,
)


RUN_TOKEN = "p2r4-phaseb-clamp-on-off-p2r2-v2-writer-v1"
SOURCE_MANIFEST = "source_manifest_s05_p2r4_phaseb_clamp_on_off_writer.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P2R2 source ancestry differs")
    manifest, raw_sha = load_rooted_json(
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST,
        expected_schema="ode-edit-s05-p2r4-phaseb-source-manifest/v1",
    )
    entries = manifest.get("entries")
    if (
        manifest.get("instruction_id") != P2R4_PHASEB_INSTRUCTION_ID
        or manifest.get("writer_parent") != PARENT
        or not isinstance(entries, list)
        or not entries
    ):
        raise ValueError("P2R4 Phase-B source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P2R4 Phase-B source manifest entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P2R4 Phase-B source path differs")
        if path.stat().st_size != entry.get("size") or sha256_file(path) != entry.get("sha256"):
            raise ValueError("P2R4 Phase-B source bytes differ")
        observed.append(relative)
    if observed != sorted(set(observed)):
        raise ValueError("P2R4 Phase-B source manifest ordering differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--clamp-policy", required=True, choices=CLAMP_POLICIES)
    ...
    try:
        ...
        result = run_p1(
            ...
            p2r4_phaseb_case_count=args.case_count,
            p2r4_phaseb_clamp_policy=args.clamp_policy,
            p2r4_phaseb_attempt_suffix=args.attempt_suffix,
        )
        result = {
            **result,
            "p2r4_phaseb_lock_sha256": lock_sha,
            "p2r4_phaseb_lock_root": lock["root_digest"],
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p2r4-phaseb-job-failure/v1",
        )
        print(
            json.dumps(
                {
