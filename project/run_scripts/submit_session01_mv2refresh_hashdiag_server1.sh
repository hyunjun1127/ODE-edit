#!/usr/bin/env bash
# Fail-closed one-shot submission for the technical-only CUDA hash RCA.

set -euo pipefail
umask 077

readonly REPO_ROOT="/mnt/raid5/janghj/ODE-edit"
readonly SESSION_ID="019fb1ea-03cb-7c20-bb3b-eba5f8d6f5f2"
readonly GIT_BIN="/usr/bin/git"
readonly GREP_BIN="/usr/bin/grep"
readonly INSTALL_BIN="/usr/bin/install"
readonly MKDIR_BIN="/usr/bin/mkdir"
readonly CHMOD_BIN="/usr/bin/chmod"
readonly SBATCH_BIN="/usr/bin/sbatch"
readonly SQUEUE_BIN="/usr/bin/squeue"
readonly SBATCH_FILE="${REPO_ROOT}/project/run_scripts/session01_mv2refresh_hashdiag_server1.sbatch"
readonly DIAG_GATE="${REPO_ROOT}/audits/global/2026-07-31-session01-mv2-cuda-hash-diagnostic-preflight.md"
readonly DIAG_VERDICT='- 최종 판정: `PASS` — Llama first-case CUDA hash RCA diagnostic 1회에만 유효'
readonly OUTPUT_ROOT="${REPO_ROOT}/local/results/raw/session01_motivation"
readonly LOG_ROOT="${REPO_ROOT}/local/logs/slurm/session01_motivation"
readonly STATE_ROOT="${REPO_ROOT}/local/state/slurm-submissions/session01_motivation"
readonly PRIOR_MARKER="${STATE_ROOT}/mv2refresh_pair_v1_retry2.submitted"
readonly DIAG_MARKER="${STATE_ROOT}/mv2refresh_hashdiag_v1.submitted"
readonly JOB_NAME="odeedit_mv2refresh_pair_v1"

[[ "$#" -eq 0 ]] || {
  echo "MV-2 CUDA hash diagnostic helper takes no arguments" >&2
  exit 2
}
[[ -x "${GIT_BIN}" \
  && -x "${GREP_BIN}" \
  && -x "${INSTALL_BIN}" \
  && -x "${MKDIR_BIN}" \
  && -x "${CHMOD_BIN}" \
  && -x "${SBATCH_BIN}" \
  && -x "${SQUEUE_BIN}" ]] || {
  echo "required fixed system command is unavailable" >&2
  exit 2
}

cd "${REPO_ROOT}"
[[ "$("${GIT_BIN}" rev-parse --show-toplevel)" == "${REPO_ROOT}" ]] || exit 2
[[ "$("${GIT_BIN}" config --local agent.id)" == "head-server1-gh" ]] || exit 2
[[ "$("${GIT_BIN}" config --local agent.role)" == "global-head" ]] || exit 2
[[ "$("${GIT_BIN}" config --local agent.hostname)" == "server1" ]] || exit 2
[[ "$("${GIT_BIN}" branch --show-current)" == "main" ]] || exit 2
"${GIT_BIN}" ls-files --error-unmatch \
  project/run_scripts/session01_mv2refresh_server1.sbatch \
  project/run_scripts/session01_mv2refresh_hashdiag_server1.sbatch \
  project/run_scripts/submit_session01_mv2refresh_hashdiag_server1.sh \
  project/run_scripts/ode_edit_motivation/hooks.py \
  project/run_scripts/ode_edit_motivation/mv2_refresh.py \
  project/run_scripts/ode_edit_motivation/tests/test_hooks_artifacts.py \
  project/run_scripts/ode_edit_motivation/tests/test_mv2_refresh.py \
  audits/global/2026-07-31-session01-mv2-cuda-hash-diagnostic-preflight.md \
  >/dev/null
[[ -z "$("${GIT_BIN}" status --porcelain --untracked-files=normal)" ]] || {
  echo "CUDA hash diagnostic requires a clean repository" >&2
  exit 2
}
[[ "$("${GIT_BIN}" rev-parse HEAD)" == "$("${GIT_BIN}" rev-parse origin/main)" ]] || {
  echo "CUDA hash diagnostic requires main == origin/main" >&2
  exit 2
}

[[ -f "${DIAG_GATE}" && ! -L "${DIAG_GATE}" ]] || {
  echo "CUDA hash diagnostic audit is unavailable" >&2
  exit 2
}
[[ "$("${GREP_BIN}" -Fxc -- "${DIAG_VERDICT}" "${DIAG_GATE}" || true)" == "1" \
  && "$("${GREP_BIN}" -Ec -- '^- 최종 판정:' "${DIAG_GATE}" || true)" == "1" ]] || {
  echo "CUDA hash diagnostic exact verdict is unavailable or ambiguous" >&2
  exit 2
}
[[ -f "${PRIOR_MARKER}/job-id" \
  && ! -L "${PRIOR_MARKER}" \
  && ! -L "${PRIOR_MARKER}/job-id" \
  && "$(<"${PRIOR_MARKER}/job-id")" =~ ^[0-9]+$ ]] || {
  echo "prior repair-execution marker is unavailable" >&2
  exit 2
}
[[ ! -e "${OUTPUT_ROOT}/mv2refresh_llama_e0_v1" \
  && ! -L "${OUTPUT_ROOT}/mv2refresh_llama_e0_v1" \
  && ! -e "${OUTPUT_ROOT}/mv2refresh_qwen_e0_v1" \
  && ! -L "${OUTPUT_ROOT}/mv2refresh_qwen_e0_v1" ]] || {
  echo "canonical MV-2 output path is not empty" >&2
  exit 2
}
[[ ! -e "${DIAG_MARKER}" && ! -L "${DIAG_MARKER}" ]] || {
  echo "CUDA hash diagnostic marker already exists" >&2
  exit 2
}
if [[ -n "$("${SQUEUE_BIN}" -h --name="${JOB_NAME}")" ]]; then
  echo "an MV-2 job is already active" >&2
  exit 2
fi

"${REPO_ROOT}/scripts/check-session-boundary.sh" "${SESSION_ID}"
"${REPO_ROOT}/scripts/check-slurm-resource-cap.sh" server1 1 65000
"${INSTALL_BIN}" -d -m 700 "${LOG_ROOT}" "${OUTPUT_ROOT}" "${STATE_ROOT}"
"${MKDIR_BIN}" -m 700 "${DIAG_MARKER}" || exit 2

job_id="$("${SBATCH_BIN}" --parsable --export=NONE "${SBATCH_FILE}")"
printf '%s\n' "${job_id}" >"${DIAG_MARKER}/job-id"
"${CHMOD_BIN}" 600 "${DIAG_MARKER}/job-id"
printf '%s\n' "${job_id}"
