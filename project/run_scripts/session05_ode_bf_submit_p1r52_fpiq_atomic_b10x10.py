#!/usr/bin/env python3
"""Held-inspect-release submitter for three P1R52-FPiQ cells."""
...
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r52-fpiq-atomic-b10x10-v1"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r52-fpiq-atomic-b10x10-v1"
BRANCH = "codex/p1r52-fpiq-atomic-b10x10-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 3
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R52-FPiQ submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-p1r52-fpiq-submission-intent/v1",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R52-FPiQ scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R52-FPiQ held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-fpiq-submission/v1",
        "source_head": source_head,
        "branch": BRANCH,
        "job_id": job_id,
        "array": "0-2%3",
