...
from .technical import check, current_physical
...
def load_teacher(rt,lock):
    ...
    return GeneratedTeacherStore(Path(manifest['path']).parent,manifest['path'],
        ...
        expected_binding=binding,require_upstream_cache=True),ready
...
def run(lock):
    ...
    try:
        ...
        before=rt.byte_hash_nonselected();create_json(out/'nonselected-before.json',before)
        ...
        t=begin('bounded_actual_independent_physical_checks')
        technical=check(rt,reference,current,WN,rows,out/'technical')
        timing['technical_seconds']=time.monotonic()-t
        create_json(out/'technical/current-work.json',dict(work=current.work,weight=current.collect_weight_timing()))
        ...
        t=begin('bounded_selected_physical_parity')
        checked={tensor_sha(WN):member(out/'technical/checks.json')}
        for arm in ARMS:
            h=tensor_sha(weights[arm])
            if h in checked:
                create_json(out/'technical'/f'{arm}-selected-physical.json',dict(reused=checked[h],weight_sha256=h,new_forwards=0))
            else:
                value=current_physical(rt,current,weights[arm],rows)
                checked[h]=create_json(out/'technical'/f'{arm}-selected-physical.json',value)
                if not value['pass']:raise ValueError('SELECTED_BOUNDED_PHYSICAL_PARITY_FAILED')
        timing['selected_physical_parity_seconds']=time.monotonic()-t
        ...
        parity=compare_exact(exacts[ARMS[0]],exacts[ARMS[1]])
        parity.update(history_memory_exact=commits[ARMS[0]]['identity']['M']==commits[ARMS[1]]['identity']['M'],
            checkpoints_CPU_verified=True,GPU_continuation='NOT_TESTED_B2_UNAUTHORIZED')
        ...
        t=begin('terminal_integrity')
        after=rt.byte_hash_nonselected()
        if before!=after:raise ValueError('B1_NONSELECTED_BYTES_MUTATION')
        ...
        create_json(out/'terminal.json',dict(status='B1_COMPLETE',science_efficiency_parity=parity['status'],
            ...
            preparation=lock['generated_ready'],input=store.receipt,technical=member(out/'technical/checks.json'),
