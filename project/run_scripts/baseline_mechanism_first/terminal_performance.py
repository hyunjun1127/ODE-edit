...
def run(model,tok,weight,history,records,terminal_batch,bindings,expected_w,expected_m,output,evaluate):
    ...
    if type(terminal_batch) is not int or not 1 <= terminal_batch <= 100 or n > len(records):
        raise ContractBoundary('TERMINAL_PERFORMANCE_SCOPE')
    ...
    for label,panel in [('current',records[n-100:n]),('seen-full',records[:n])]:
        baseline=normalize(json.loads(Path(sources[label]['path']).read_text()),panel)
