#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R52 target-depth target-only stages."""
...
from project.run_scripts import session05_ode_bf_p1r52_target_depth_dry_plan as dry
from project.run_scripts.ode_bf.contracts import ODEBFContractError


SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r52_target_depth.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r52-target-depth-il1-il3full-v1"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r52-target-depth-il1-il3full-v1"
...
def _active_gpu_allocations() -> tuple[int, list[dict[str, object]]]:
    ...
    for line in lines:
        job_id, state, gres, name = (line.split("|", 3) + ["", "", ""])[:4]
        counts = [
            int(value)
        ...
        if count:
            observed.append(
                {"job_id": job_id, "state": state, "gres": gres, "name": name, "gpu": count}
            )
            total += count
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R52 target-depth execution source differs")
    if case_count not in (1, 10):
        raise ODEBFContractError("P1R52 target-depth case count differs")
    if attempt_suffix is not None and not attempt_suffix.replace("-", "").isalnum():
        raise ODEBFContractError("P1R52 target-depth attempt suffix differs")
    plan = dry.build_plan(
        source_head, case_count=case_count, attempt_suffix=attempt_suffix
    )
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R52 target-depth result namespace exists")

    active, active_jobs = _active_gpu_allocations()
    available = PROJECT_GPU_CAP - active
    if available <= 0:
        raise ODEBFContractError("P1R52 target-depth server2 GPU cap has no free allocation")
    stage_concurrency = min(STAGE_GPU_MAX, available)
    if active + stage_concurrency > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R52 target-depth server2 GPU cap differs")
    phase = "b1" if case_count == 1 else "b10x10"
    tail = f"-{attempt_suffix}" if attempt_suffix else ""
    namespace = f"s05-p1r52-target-depth-{phase}{tail}-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R52 target-depth submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-3%{stage_concurrency}"
    node_receipt = _run(["scontrol", "show", "node", "-o", "server2"]).stdout.strip()
    intent = {
        "schema": "ode-edit-s05-p1r52-target-depth-intent/v1",
    ...
    command = [
        ...
        "server2",
        "--job-name",
        f"odeedit_s05_p1r52_target_depth_{phase}",
        "--output",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R52 target-depth scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R52 target-depth held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r52-target-depth-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "phase": phase,
        "job_id": job_id,
