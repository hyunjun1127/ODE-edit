#!/usr/bin/env python3
"""Held-inspect-release submitter for the two P1R22 C0/C1 jobs."""
...
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r22-atomic-runtime-conformance-tech-r4"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
NAMESPACE = "s05-atomic-runtime-optimization-p1r22-c0c1-sh2-array-tech-r4-v1"
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if (
        ...
        raise ODEBFContractError("P1R22 C0/C1 execution source differs")
    ...
    result_names = tuple(
        f"s05-atomic-runtime-optimization-p1r22-c0c1-{job['alias']}-tech-r4-v1"
        for job in plan["jobs"]
    )
    if any((RESULT_PARENT / name).exists() for name in result_names):
        raise ODEBFContractError("P1R22 C0/C1 result namespace exists")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R22 C0/C1 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-atomic-runtime-optimization-p1r22-c0c1-intent/v1",
    ...
    submitted = _run(
        [
            ...
            "--job-name", "odeedit_s05_p1r22_atomic_c0c1", "--output",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R22 C0/C1 scheduler ID differs")
    ...
    receipt = {
        "schema": "ode-edit-s05-atomic-runtime-optimization-p1r22-c0c1-submission/v1",
        "source_head": source_head,
        "job_id": job_id,
        "array": "0-1%2",
        "model_job_count": 2,
