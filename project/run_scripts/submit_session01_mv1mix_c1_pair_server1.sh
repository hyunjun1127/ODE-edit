#!/usr/bin/env bash
# Fail-closed one-shot submit helper for the exact paired MV-1 C1 fold-0 run.
...
readonly SBATCH_FILE="${REPO_ROOT}/project/run_scripts/session01_mv1mix_c1_pair_server1.sbatch"
readonly RED_GATE="${REPO_ROOT}/audits/global/2026-07-31-session01-mv1mix-c1-execution-preflight.md"
readonly D1_POSTRUN="${REPO_ROOT}/audits/global/2026-07-31-mv1mix-d1-pair-v1.postrun.md"
...
readonly PAIR_MARKER="${STATE_ROOT}/mv1mix_c1_pair_v1.submitted"
readonly JOB_NAME="odeedit_mv1mix_c1_pair_v1"

[[ "$#" -eq 0 ]] || {
  echo "this paired MV-1 C1 helper takes no arguments" >&2
  exit 2
}
[[ -x "${GIT_BIN}" \
  && -x "${GREP_BIN}" \
...
"${GIT_BIN}" ls-files --error-unmatch \
  project/run_scripts/session01_mv1mix_c1_server1.sbatch \
  project/run_scripts/session01_mv1mix_c1_pair_server1.sbatch \
  project/run_scripts/submit_session01_mv1mix_c1_pair_server1.sh \
  ...
  audits/global/2026-07-31-session01-mv1mix-c1-execution-preflight.md \
  audits/global/2026-07-31-mv1mix-d1-pair-v1.postrun.md \
  >/dev/null
[[ -z "$("${GIT_BIN}" status --porcelain --untracked-files=normal)" ]] || {
  echo "paired MV-1 C1 submission requires a clean repository" >&2
  exit 2
}
[[ "$("${GIT_BIN}" rev-parse HEAD)" == "$("${GIT_BIN}" rev-parse origin/main)" ]] || {
  echo "paired MV-1 C1 submission requires main == origin/main" >&2
  exit 2
}
"${GREP_BIN}" -Fxq -- \
  '- 최종 판정: `PASS` — MV-1 C1 fold-0 동시 pair 한 건에만 유효' \
  "${RED_GATE}" || {
  echo "MV-1 C1 execution preflight has not passed" >&2
  exit 2
}
"${GREP_BIN}" -Fxq -- \
  '- 최종 판정: `C1 PREPARE` — D1 technical PASS 및 frozen policy/forecast 생성 허용' \
  "${D1_POSTRUN}" || {
  echo "MV-1 D1 continuation gate is unavailable" >&2
  exit 2
}

[[ ! -e "${OUTPUT_ROOT}/mv1mix_llama_c1_v1" ]] || {
  echo "Llama C1 output already exists" >&2
  exit 2
}
[[ ! -e "${OUTPUT_ROOT}/mv1mix_qwen_c1_v1" ]] || {
  echo "Qwen C1 output already exists" >&2
  exit 2
}
[[ ! -e "${PAIR_MARKER}" ]] || {
  echo "MV-1 C1 one-shot marker already exists" >&2
  exit 2
}
if [[ -n "$("${SQUEUE_BIN}" -h --name="${JOB_NAME}")" ]]; then
  echo "paired MV-1 C1 job is already active" >&2
  exit 2
fi
