#!/usr/bin/env python3
"""Fail-closed entry for the fresh S05 cold Structural-P/soft-P pilot."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="session05-ode-bf-cold-structp-softp-noveto",
        allow_abbrev=False,
    )
    parser.add_argument("--model", required=True, choices=MODEL_ALIASES)
    parser.add_argument("--output-root", required=True, type=Path)
    parser.add_argument("--source-head", required=True)
    parser.add_argument("--run-token", required=True, choices=(COLD_RESULT_TOKEN,))
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        failure_sha256, failure = write_p1_failure_once(
            ...
            instruction_id=COLD_INSTRUCTION_ID,
            failure_schema=(
                "ode-edit-s05-cold-structp-softp-noveto-p1r6-failure/v1"
            ),
        )
        print(
            json.dumps(
