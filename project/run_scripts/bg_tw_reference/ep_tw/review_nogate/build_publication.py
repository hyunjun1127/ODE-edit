"""Create-once raw-free publication from already reduced sealed evidence."""
import argparse
import csv
import json
...
from review_nogate import ROOT,REPORT,read,ref,save,table,sha
...
def run(worktree,output):
    w,o=Path(worktree),Path(output);o.mkdir(parents=True,exist_ok=False);local=ROOT/'completed-review-v1'
    groups=['metrics-v3','state-v1','baselines-v1','mechanism-v1','evidence-v1'];inputs=[]
    for group in groups:
        for p in sorted((local/group).iterdir()):
            if p.suffix not in ('.csv','.json'):continue
            assert not (o/p.name).exists();shutil.copyfile(p,o/p.name);inputs.append(dict(ref(p),published_name=p.name))
    shutil.copyfile(local/'scheduler-once.json',o/'scheduler-once.json')
    final=rows(o/'first-final-table.csv');batch=rows(o/'batch-current-metrics.csv');whole=rows(o/'whole-prefix-metrics.csv')
    candidates=rows(o/'candidate-details.csv');policy=rows(o/'batch-policy.csv');mechanism=rows(o/'per-batch-mechanism.csv')
    paired=rows(o/'paired-transitions.csv');cohorts=rows(o/'cohort-retention.csv');bases=rows(o/'baseline-first1000.csv')
    baselinepairs=rows(o/'baseline-paired.csv');cost=read(o/'metrics-summary.json')['cost_totals'];state=read(o/'state-summary.json')
    execution=read(ROOT/'execution.lock.json');terminal=read(ROOT/'scientific-v1/terminal.json');entry=read(ROOT/'scientific-v1/entry.json')
    ...
    table(o/'coverage.csv',[dict(requirement=a,status=b,evidence_or_limitation=c) for a,b,c in coverage])
    compute=[dict(component='job47962_new_scientific_allocated',seconds=7694,kind='NEW',additive_group='allocated',evidence='scheduler-once.json'),
             dict(component='job47884_prior_failure',seconds=473,kind='REUSED_HISTORICAL',additive_group='old_failure',evidence='execution.lock.cost_plan'),
             dict(component='job47942_prior_failure',seconds=69,kind='REUSED_HISTORICAL',additive_group='old_failure',evidence='execution.lock.cost_plan'),
             dict(component='teacher47592_prior',seconds=98,kind='REUSED_HISTORICAL',additive_group='teacher',evidence='execution.lock.cost_plan'),
             dict(component='old_nativefit_nested_in47884',seconds=286.54453050531447,kind='NESTED_DO_NOT_ADD',additive_group='none',evidence='old failed source receipt'),
             dict(component='model_load',seconds=entry['model_seconds'],kind='NEW_NESTED_IN_ALLOCATED',additive_group='none',evidence='entry.json')]
    for k,v in cost.items():
        if 'seconds' in k or k.endswith(('_total','_FB','_read')):
            compute.append(dict(component=k,seconds=v,kind='RECORDED_COMPONENT_MAY_NEST',additive_group='none',evidence='cost-by-batch.csv'))
    table(o/'compute-summary.csv',compute)
    report=f'''# EP-TW-1 job47962 완료 리뷰와 설계 대비 실제 동작
...
'''
    report+=md(['상태/범위','지표','n/d (%)','desired TF-strict n/d','desired token correct/d'],
               [('W10 / first1000',r['metric'],n(r),f"{r['desired_strict_n']}/{r['desired_strict_d']}",f"{r['desired_token_correct']}/{r['desired_token_d']}") for r in final])
    report+='''
...
'''
    report+=md(['항목','identity/관측'],[
        ['실행 commit',execution['source_head']],['실행 tree',execution['source_tree']],
        ['실행 archive SHA',execution['source_archive']['sha256']],['execution.lock SHA',sha(ROOT/'execution.lock.json')],
        ['분석 코드 commit (publication 전)',head],['분석 코드 tree',tree],
        ['출발 main','833e6396522924b8bbb26eedadf763e2d3ebd4d4'],
        ['추가 지시 수신 main','0043108edc25bd6173e4e1995713b9a7ea79840b'],
        ['원 raw 절대경로',str(ROOT/'scientific-v1')],['terminal SHA',sha(ROOT/'scientific-v1/terminal.json')],
        ['시간(KST)','2026-09-15 14:32:06–16:40:20'],['scheduler elapsed/GPU allocation','7694 s / 1GPU =2.137222 GPUh'],
        ['program wall',terminal['seconds']],['실제 initial marker','INITIAL_EXECUTION_OBSERVED_WITH_VALIDATION_SKIPPED'],
...
'''
    data=[]
    for b in range(1,11):
        for s in ('ENTRY','RAW','SELECTED'):
            rr=[next(r for r in batch if r['batch']==str(b) and r['state']==s and r['population']=='CURRENT' and r['metric']==m) for m in ('RS','PS','NS')]
            data.append([b,s,*[n(r) for r in rr],f"{rr[0]['desired_strict_n']}/100",f"{rr[1]['desired_strict_n']}/200"])
    report+=md(['B','state','RS','PS','NS','R strict','P strict'],data)
    report+='''
...
'''
    report+=md(['state/population','metric','n/d (%)'],[[r['batch']+' / '+r['scope'],r['metric'],n(r)] for r in whole if r['scope'] in ('FULL_SEEN','FIRST500','ONLINE_OWN_BATCH_NOT_RETENTION')])
    report+='''
...
'''
    report+=md(['B','q','||gE||','||gD||','cos','projection coeff','〈gE,d〉','〈gD,d〉'],
        [[r['batch'],f(r,'q_recorded'),f(r,'ge_norm'),f(r,'gd_norm'),f(r,'gradient_cos'),f(r,'coefficient_recorded'),f(r,'ge_dot_d_recorded'),f(r,'gd_dot_d_recorded')] for r in mechanism])
    report+='''
...
'''
    report+=md(['B','alpha','ball requests','post ball excess','retraction','||C||','||CA||','〈gE,C〉','〈gD,C〉'],
        [[r['batch'],f(r,'alpha'),r['ball_clamped_requests'],f(r,'ball_excess_after_recorded'),f(r,'trust_retraction'),f(r,'C_norm_recorded'),f(r,'CA_norm_recorded'),f(r,'ge_dot_C_recorded'),f(r,'gd_dot_C_recorded')] for r in mechanism])
    report+='''
...
'''
    report+=md(['B','candidate','ΔE vsRAW','ΔD64 vsRAW','strict IDs lost','feasible','selected','reason'],
        [[r['batch'],r['candidate'],f(r,'E_minus_RAW'),f(r,'D64_minus_RAW'),r['RAW_strict_lost'],r['feasible'],r['selected'],r['reason']] for r in candidates])
    report+='''
...
'''
    report+=md(['B','selected','||Vp−We|| recorded','||Wsel−Vp||','correction/native %','||Wsel−We|| CPU','correction/native cos','orthogonal norm'],
        [[r['batch'],r['selected'],f(r,'native_action_norm'),f(r,'actual_correction_norm'),100*f(r,'correction_native_ratio'),r.get('selected_increment_norm'),r.get('correction_native_cos'),r.get('correction_orthogonal_norm')] for r in mechanism])
    report+='''
...
'''
    report+=md(['comparison','metric','before→after','lost/conditional d','gained/conditional d','strict lost/gained','harm p95','harm p99'],
        [[r['comparison'],r['metric'],r['before_num']+'→'+r['after_num'],r['lost']+'/'+r['conditional_loss_den'],r['gained']+'/'+r['conditional_recovery_den'],r['strict_lost']+'/'+r['strict_gained'],f(r,'desired_NLL_harm_p95'),f(r,'desired_NLL_harm_p99')]
         for r in paired if r['comparison'] in ('ATWRITE_TO_W10','W5_TO_W10_FIRST500')])
    report+='''
...
'''
    report+=md(['metric','NLL','mean','median','p90','p95','p99','max'],
        [[r['metric'],k,*[f(r,k+'_nll_'+q) for q in ('mean','median','p90','p95','p99','max')]] for r in final for k in ('new','true')])
    report+=md(['metric','desiredmargin mean','median','p95','p99','min'],
        [[r['metric'],*[f(r,'desired_margin_'+q) for q in ('mean','median','p95','p99','min')]] for r in final])
    report+='''
...
'''
    for family in ('AlphaEdit','MEMIT','PRE_EDIT'):
        report+='\n### '+family+' reference\n'
        pp=list(dict.fromkeys(r['policy'] for r in bases if family.upper() in r['policy'].upper()))
        data=[]
        for label in pp:
            rr=[next(r for r in bases if r['policy']==label and r['metric']==m) for m in ('RS','PS','NS')]
            data.append([label,*[n(r) for r in rr]])
        if family=='AlphaEdit':data.append(['EP-TW-1',*[n(r) for r in final]])
        report+=md(['reference / W10 (W0별도)','RS','PS','NS'],data)
    report+='''
...
'''
    selectedcost=[('model/load',entry['model_seconds'],'program component'),('native instrumented fit',cost['native_seconds'],'parent timer'),
        ('compute_z',cost['z_seconds'],'nested in native'),('native keys',cost['native_keys_seconds'],'nested in native'),
        ('canonical readout',cost['readout_seconds'],'nested in native'),('native RHS solve',cost['solve_seconds'],'nested in native'),
        ('mapA setup',cost['map_seconds'],'separate map solve+setup'),('current gradient',cost['current_gradient_total'],'total; FB not separate'),
        ('S64 gradient',cost['S64_gradient_total'],'parent incl. FB/read'),('S64 forward/backward',cost['S64_gradient_FB'],'nested'),
        ('S64 teacher read',cost['S64_teacher_read'],'nested'),('candidate screen',cost['screen_seconds'],'scoring+policy'),
        ('final history',cost['history_seconds'],'selected endpoint'),('selected evaluation',cost['selected_evaluation_seconds'],'observer; other eval gaps remain')]
    report+=md(['component','seconds','nesting/meaning'],selectedcost)
    report+='''
...
'''
    report+=md(['required coverage','status','evidence / limitation'],coverage)
    report+='''
## 10. 재현·파일 안내
...
'''
    with (o/'diagnostic-report-ko.md').open('x') as fobj:fobj.write(report)
    save(o/'comparison-capsule.json',dict(execution_head=execution['source_head'],execution_tree=execution['source_tree'],
        analysis_code_head=head,analysis_code_tree=tree,execution_lock=ref(ROOT/'execution.lock.json'),
        model_revision=execution['model_revision'],dtype=execution['dtype'],attention=execution['attention'],seed=execution['seed'],
        canonical_microbatch=16,physical_layer=4,projector_mapping=execution['projector_mapping'],
        dataset_sha256='3d7f5e31db47b23f3a281bb6fb447c2fb1776e3d4721cdfb5fe5c6f998f10ae1',
        initial='W0_COLD_M0',requests=1000,batches=10,other_entry_imports=0,
        numerical_validation='NOT_ESTABLISHED',validation_mode='SKIPPED_USER_DIRECTED',
        scientific_promotion=False,claim_decision='PENDING_GH_REVIEW'))
    save(o/'publication-inputs.json',dict(aggregate_inputs=inputs,raw_payload_copied=False,
        source_root=str(w),source_code_head=head,local_reducer_attempts=['metrics-v1 analysis-only scalar/dict timer error preserved','metrics-v2 provisional','metrics-v3 final'],
        original_reports_modified=False,new_GPU=0))
    print(json.dumps({'report_bytes':(o/'diagnostic-report-ko.md').stat().st_size,'report_sha256':sha(o/'diagnostic-report-ko.md'),'output':str(o)}))
