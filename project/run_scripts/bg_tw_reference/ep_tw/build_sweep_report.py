...
'''
    # This publication belongs to the explicit completed-review recall. Old
    # source/dispatch and subsequent user pause remain historical facts.
    text=text.replace('CAKE의 no-weight-storage와 monitoring pause는 이 task에 상속하지 않았고 CAKE에 대한 자원 admission 외 점검·변경0이다.',
        'CAKE의 no-weight-storage는 sweep에 적용하지 않았다. Sweep monitoring은 이후 사용자 지시로 중지했고, 이번 completed-review recall에서만 지정 네 job의 terminal 확인과 CPU 분석을 재개했다. CAKE는 별도 10k 보고서로 리뷰하며 실행 변경0이다.')
    extra=p/'design-conformance-ko.md'
