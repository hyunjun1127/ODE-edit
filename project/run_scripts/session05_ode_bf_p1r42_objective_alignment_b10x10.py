#!/usr/bin/env python3
"""Fail-closed P1R42 objective-alignment runner."""
...
from project.run_scripts.ode_bf.p1r42_independent_b10x10_runtime import (
    B1_METHODS,
    INSTRUCTION_ID,
    METHODS,
)
...
RUN_TOKEN = "p1r42-objective-alignment-b10x10-v1"
SOURCE_MANIFEST = "source_manifest_s05_p1r42_objective_alignment_b10x10.json"
...
def _source_gate(source_head: str) -> str:
    if subprocess.run(
        ...
        raise ValueError("P1R42 source ancestry differs")
    for relative, expected in PROTECTED_SOURCE_SHA256.items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise ValueError("P1R42 protected source differs")
    manifest_path = (
        REPO_ROOT / "project/run_scripts/ode_bf/locks" / SOURCE_MANIFEST
    )
    manifest, raw_sha = load_rooted_json(
        manifest_path,
        expected_schema="ode-edit-s05-p1r42-objective-alignment-source-manifest/v1",
    )
    ...
    if (
        ...
        raise ValueError("P1R42 source manifest header differs")
    observed: list[str] = []
    for entry in entries:
        relative = entry.get("path") if isinstance(entry, dict) else None
        if not isinstance(relative, str):
            raise ValueError("P1R42 source entry differs")
        path = REPO_ROOT / relative
        if path.is_symlink() or not path.is_file():
            raise ValueError("P1R42 source path differs")
        if (
            path.stat().st_size != entry.get("size")
            or sha256_file(path) != entry.get("sha256")
        ):
            raise ValueError("P1R42 source bytes differ")
    ...
    if observed != sorted(set(observed)) or changed != set(observed) | {
        ...
        raise ValueError("P1R42 source closure differs")
    return raw_sha
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_p1(
            ...
            p1r42_independent_b10x10_method=args.method,
            p1r42_attempt_suffix=args.attempt_suffix,
        )
        result = {
            **result,
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r42-objective-alignment-b10x10-job-failure/v1",
        )
        print(
            json.dumps(
                {
