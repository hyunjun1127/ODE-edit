...
def observe(job,output,interval,reason):
    scheduler=subprocess.run(['scontrol','show','job',str(job),'-o'],capture_output=True,text=True,check=True).stdout
    ...
    print(json.dumps(dict(path=str(path),**row),ensure_ascii=False))
