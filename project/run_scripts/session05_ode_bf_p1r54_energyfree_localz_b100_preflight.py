#!/usr/bin/env python3
"""No-model/no-CUDA final gate for the server4 P1R54 two-arm array."""
...
from project.run_scripts.ode_bf.p1r54_energyfree_localz import INSTRUCTION_ID
from project.run_scripts.ode_bf.p1r54_energyfree_localz_b100 import ROLES, role_for_cell
...
from project.run_scripts.session05_ode_bf_p1r54_energyfree_localz_b100_dry_plan import (
    build_plan,
)


CONTRACT = Path(
    "/data/janghj/ODE-edit/local/state/p1r54-energyfree-localz-llama-b100-v1/authoritative-contract.txt"
)
CONTRACT_SHA256 = "6fcfc7df83c9b94d38accecb5fa600c33a8a1741f579881d9f1fab05cd0a3d58"
CONTRACT_BYTES = 12_866
CONTRACT_LINES_WC = 610
PROJECT_GPU_CAP = 4
ARRAY_CONCURRENCY = 2
NUMERICAL_LOCK = Path(
    "project/run_scripts/ode_bf/locks/numerical_lock_s05_p1r54_energyfree_localz_llama_b100_v1.json"
)
P1R52_REFERENCE_ROOT = Path(
    "/data/janghj/ODE-edit/local/worktrees/p1r52-target-timescale-b100-v1/local/odebf/results"
)
P1R53_REFERENCE_ROOT = Path(
    "/data/janghj/ODE-edit/local/worktrees/p1r53-request-local-speed-llama-b100-v1/local/odebf/results"
)
REFERENCES = {
    "global_p1r52_z0_coarse": (
        P1R52_REFERENCE_ROOT / "s05-p1r52-target-timescale-b100-z0-coarse-tech-r4-v1",
    ...
    "p1r53_lp_s": (
        P1R53_REFERENCE_ROOT / "s05-p1r53-request-local-speed-llama-b100-lp-s-tech-r1-v1",
        "a9edd666ed08d40352183448e059716eb93e0d2cea1a48ab84930988a209edb0",
        "73b159986dee39780c3126f158b480aad7d1b952d7900b7818b60ae97a999519",
    ),
    "p1r53_lfd_e": (
        P1R53_REFERENCE_ROOT / "s05-p1r53-request-local-speed-llama-b100-lfd-e-tech-r1-v1",
        "daba8439b5e24aab8aab06787b8214e109f6b05519d717cb2ebb73fc1a6c72d4",
        "b404cc17063d7313c1d64472c07f00a1372fc8c65c9ed4d2d2bf18c39447f4e1",
    ),
    "native_alphaedit": (
        P1R52_REFERENCE_ROOT / "s05-p1r52-target-timescale-native-b100-official-alphaedit-user-override-v1",
    ...
    "native_memit": (
        P1R52_REFERENCE_ROOT / "s05-p1r52-target-timescale-native-b100-official-memit-user-override-tech-r2-v1",
...
SOURCE_FILES = (
    ...
    "project/run_scripts/session05_ode_bf_p1r54_energyfree_localz_b100.py",
    "project/run_scripts/session05_ode_bf_p1r54_energyfree_localz_b100_dry_plan.py",
    "project/run_scripts/session05_ode_bf_p1r54_energyfree_localz_b100_preflight.py",
    "project/run_scripts/session05_ode_bf_p1r54_energyfree_localz_b100_server4.sbatch",
    str(NUMERICAL_LOCK),
...
def _source_manifest(head: str, tree: str) -> Mapping[str, Any]:
    ...
    for relative in SOURCE_FILES:
        ...
        if path.is_symlink() or not path.is_file():
            raise ODEBFContractError(f"P1R54 source member differs: {relative}")
        entries.append(
            {"path": relative, "bytes": path.stat().st_size, "sha256": sha256_file(path)}
        )
    payload: dict[str, Any] = {
        "schema": "ode-edit-s05-p1r54-energyfree-localz-source-manifest/v1",
        "source_head": head,
        "source_tree": tree,
        "entries": entries,
    }
...
def _verify_contract() -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ODEBFContractError("P1R54 authoritative contract identity differs")
    payload.decode("utf-8")
    return {
        "path": str(CONTRACT),
        "sha256": CONTRACT_SHA256,
        "bytes": CONTRACT_BYTES,
        "wc_lines": CONTRACT_LINES_WC,
        "logical_records_without_terminal_newline": len(payload.decode("utf-8").splitlines()),
        "mode": "0600",
        "full_read": True,
    }
...
def build_receipt(*, source_head: str, final_receipt: Path, session_id: str) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise ODEBFContractError("P1R54 server4/source/cap gate differs")
    ...
    if (
        ...
        or [row.get("arm") for row in lock.get("arms", [])] != ["FZ", "PDZ"]
        or lock.get("rho", {}).get("refresh_count") != 0
        or any(lock.get("forbidden_decision_access", {}).values())
        or lock.get("heldout_outer_indices") != [8]
    ):
        raise ODEBFContractError("P1R54 numerical lock differs")
    ...
    dry_plan = build_plan()
    final: dict[str, Any] = {
        "schema": "ode-edit-s05-p1r54-energyfree-localz-final-pre-gpu/v1",
        ...
        "dry_plan": dry_plan,
        "mechanical_gate": {
            "batch_permutation_add_remove_duplicate_b10_b100_invariance": "PASS",
            "fz_velocity_norm_equals_rho": "PASS",
            "pdz_bounds_and_stable_deficit": "PASS",
            "active_semantic_descent_and_existing_preservation": "PASS",
            "forbidden_decision_access_count": 0,
            "additional_forward_backward_count": 0,
            "per_request_backward_loop_count": 0,
            "p1r53_existing_arms_unchanged": "PASS",
            "c3_teacher_cache_W0_identity": "PASS",
        },
