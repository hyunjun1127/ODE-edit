#!/usr/bin/env python3
"""Held-inspect-release submitter for P1R51 RSA-A1 phases."""
...
from project.run_scripts.ode_bf.p1r51_independent_runtime import PHASES


SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p1r51_rsa_a1.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p1r51-rsa-a1"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p1r51-rsa-a1"
BRANCH = "main"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def submit(source_head: str, *, phase: str, attempt_suffix: str | None = None) -> dict[str, object]:
    head = _run(["git", "rev-parse", "HEAD"]).stdout.strip()
    branch = _run(["git", "branch", "--show-current"]).stdout.strip()
    dirty = _run(["git", "status", "--porcelain", "--untracked-files=no"]).stdout
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P1R51 execution source differs")
    if phase not in PHASES:
        raise ODEBFContractError("P1R51 phase differs")
    plan = dry.build_plan(source_head, phase, attempt_suffix=attempt_suffix)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R51 result namespace exists")

    active, active_jobs = _active_gpu_allocations()
    available = PROJECT_GPU_CAP - active
    if available <= 0:
        raise ODEBFContractError("P1R51 server1 project GPU cap has no free allocation")
    stage_concurrency = min(STAGE_GPU_MAX, available, 2)
    if active + stage_concurrency > PROJECT_GPU_CAP:
        raise ODEBFContractError("P1R51 server1 project GPU cap differs")
    namespace_tail = f"-{attempt_suffix}" if attempt_suffix else ""
    namespace = f"s05-p1r51-rsa-a1-{phase}{namespace_tail}-{source_head[:12]}-v1"
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R51 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    array = f"0-1%{stage_concurrency}"
    intent = {
        "schema": "ode-edit-s05-p1r51-rsa-a1-submission-intent/v1",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R51 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P1R51 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p1r51-rsa-a1-submission/v1",
        "source_head": source_head,
        "branch": BRANCH,
        "phase": phase,
        "job_id": job_id,
        "array": array,
        "job_count": 2,
        "endpoint_attempt_count": plan["total_endpoint_attempts"],
...
def main() -> int:
    ...
    print(json.dumps(submit(args.source_head, phase=args.phase, attempt_suffix=args.attempt_suffix), sort_keys=True, separators=(",", ":")))
    return 0
