#!/usr/bin/env python3
"""Fail-closed entry for the S05 NEWNLL functional H/P soft-hard pilot."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="session05-ode-bf-newnll-p-soft-hard",
        allow_abbrev=False,
    )
    ...
    parser.add_argument(
        ...
        choices=(NEWNLL_P_SOFT_HARD_RESULT_TOKEN,),
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        failure_sha256, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=NEWNLL_P_SOFT_HARD_INSTRUCTION_ID,
            failure_schema="ode-edit-s05-newnll-p-soft-hard-failure/v1",
        )
        print(json.dumps({
            "status": "FAIL_CLOSED",
            "instruction_id": NEWNLL_P_SOFT_HARD_INSTRUCTION_ID,
