#!/usr/bin/env python3
"""Fail-closed launcher for one Phase-3 cached K-step cell."""
...
SOURCE_FILES = (
    ...
    "project/run_scripts/session05_ode_bf_p1r52_c_writer_phase3.py",
    "project/run_scripts/session05_ode_bf_p1r52_c_writer_phase3.sbatch",
    "project/run_scripts/session05_ode_bf_p1r52_c_writer_phase3_dry_plan.py",
...
def source_manifest(head: str, tree: str) -> dict[str, object]:
    rows = [
        {"path": path, "bytes": (REPO_ROOT / path).stat().st_size, "sha256": sha256_file(REPO_ROOT / path)}
        for path in SOURCE_FILES
    ]
    payload: dict[str, object] = {
        "schema": "ode-edit-s05-p1r52-c-writer-phase3-source-manifest/v1",
        "source_head": head,
        "source_tree": tree,
        "entries": rows,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        timing = {
            "schema": "ode-edit-s05-p1r52-c-writer-phase3-job-timing/v1",
            "role": role,
            "cell": args.cell,
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            failure_schema="ode-edit-s05-p1r52-c-writer-phase3-failure/v1",
        )
        print(
            json.dumps(
                {"status": "FAIL_CLOSED", "exception_class": failure["exception_class"], "failure_sha256": failure_sha}
            ),
            file=sys.stderr,
        )
        return 1
