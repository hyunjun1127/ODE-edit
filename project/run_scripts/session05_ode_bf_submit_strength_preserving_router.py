#!/usr/bin/env python3
"""Held-inspect-release submitter for the P1R19 eight-task array."""
...
from project.run_scripts import session05_ode_bf_strength_preserving_router_dry_plan as dry
...
def submit(source_head: str) -> dict[str, object]:
    ...
    if (
        source_head != head
        or branch != "codex/odeeditsh2-s05-strength-preserving-router-p1r19-v1"
        or dirty
    ):
        raise ODEBFContractError("P1R19 execution source differs")
    plan = dry.build_plan(source_head)
    if any((RESULT_PARENT / str(job["result_name"])).exists() for job in plan["jobs"]):
        raise ODEBFContractError("P1R19 result namespace exists")
    ...
    if any(path.exists() or path.is_symlink() for path in (intent_path, receipt_path)):
        raise ODEBFContractError("P1R19 submission namespace exists")
    LOG_ROOT.mkdir(mode=0o700, parents=True, exist_ok=True)
    intent = {
        "schema": "ode-edit-s05-strength-preserving-router-p1r19-a2-intent/v1",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P1R19 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        raise ODEBFContractError("P1R19 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-strength-preserving-router-p1r19-a2-submission/v1",
        "source_head": source_head,
        "job_id": job_id,
        "array": "0-7%4",
        "trajectory_count": 8,
