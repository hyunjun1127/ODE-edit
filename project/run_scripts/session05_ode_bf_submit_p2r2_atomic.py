#!/usr/bin/env python3
"""Held-inspect-release submitter for P2R2 Atomic paired arms."""
...
SBATCH = REPO_ROOT / "project/run_scripts/session05_ode_bf_p2r2_atomic.sbatch"
STATE_ROOT = REPO_ROOT / "local/odebf/state/p2r2-semantic-conserving-residual-transport"
RESULT_PARENT = REPO_ROOT / "local/odebf/results"
LOG_ROOT = REPO_ROOT / "local/odebf/logs/p2r2-semantic-conserving-residual-transport"
BRANCH = "codex/p2r2-semantic-conserving-residual-transport-writer-v1"
PROJECT_GPU_CAP = 4
STAGE_GPU_MAX = 2
...
def submit(
    ...
    *,
    ...
    if source_head != head or branch != BRANCH or dirty:
        raise ODEBFContractError("P2R2 execution source differs")
    ...
    namespace = f"s05-p2r2-{phase}-{namespace_tail}-v1"
    ...
    command = [
        ...
        "--job-name", f"odeedit_s05_p2r2_{phase.replace('-', '_')}",
    ...
    if not job_id.isdigit():
        raise ODEBFContractError("P2R2 scheduler ID differs")
    ...
    if not all(item in observed for item in required):
        ...
        raise ODEBFContractError("P2R2 held scheduler contract differs")
    receipt = {
        "schema": "ode-edit-s05-p2r2-submission/v1",
        "source_head": source_head,
        "source_parent": parent,
        "phase": phase,
        "job_id": job_id,
