...
def publish(aggregates, figures, output, technical, n4_reuse, allocation):
    ...
    sections = ['# SL-ZFlow W0→SEQ1000 사실 보고', '',
        ...
        '순수 filesystem I/O 시간은 별도로 계측하지 않았으며 commit timer에 포함된다. 별도 기록된 parity/cost/prepare 시간을 뺀 나머지는 publication·hash·전송·기타를 포함하는 nonexclusive remainder이지 순수 I/O 시간이 아니다. Batch별 checkpoint_bundle_bytes는 실제 저장량이다. CPU MaxRSS는 Slurm의 sampled 값이며 torch GPU peak와 같은 정의가 아니다.', '',
