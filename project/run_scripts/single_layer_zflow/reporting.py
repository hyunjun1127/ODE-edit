...
def publish(aggregates, figures, output, technical, n4_reuse, allocation):
    ...
    sections = ['# SL-ZFlow W0→SEQ1000 사실 보고', '',
        ...
        '## 5. 비용·오류와 자원', '',
        table(jobs['jobs'], [('job_id','Job'),('scope','범위'),('state','상태'),('exit_code','Exit'),('allocated_gpu_seconds','할당 GPU-sec'),('slurm_sampled_max_rss','Slurm sampled CPU MaxRSS')]), '',
        ...
        f"기존 N4 job{reuse['historical_job']}의 기록된 process 시간은 {reuse['historical_process_seconds']:.6f}s이며 과거 비용이다. 새 기술+MAIN 할당 합계는 {sum(row['allocated_gpu_seconds'] for row in jobs['jobs'])} GPU-sec다. N4와 MAIN의 observer 일정·구성요소 및 host/backend가 동일한 시간 실험으로 검증된 것은 아니므로 이 두 process 시간만으로 순수 optimizer speedup을 계산하지 않는다.", '',
