...
case "${TRACK}" in
  memit)
    readonly JOB_NAME="odeedit_agate_memit_pair_v1"
    readonly LLAMA_RUN="agate_memit_llama_g0_v1"
    readonly QWEN_RUN="agate_memit_qwen_g0_v1"
    readonly PAIR_MARKER="${STATE_ROOT}/agate_memit_pair_v1.submitted"
    ;;
  alphaedit_projected)
    readonly JOB_NAME="odeedit_agate_alpha_pair_v1"
    readonly LLAMA_RUN="agate_alpha_llama_g0_v1"
    readonly QWEN_RUN="agate_alpha_qwen_g0_v1"
    readonly PAIR_MARKER="${STATE_ROOT}/agate_alpha_pair_v1.submitted"
    ;;
  *)
...
if [[ "${TRACK}" == "alphaedit_projected" ]]; then
  readonly MEMIT_MARKER="${STATE_ROOT}/agate_memit_pair_v1.submitted"
  ...
  for summary in \
    "${OUTPUT_ROOT}/agate_memit_llama_g0_v1/summary.json" \
    "${OUTPUT_ROOT}/agate_memit_qwen_g0_v1/summary.json"; do
