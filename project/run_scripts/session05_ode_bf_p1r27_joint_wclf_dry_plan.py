#!/usr/bin/env python3
"""No-model P1R27 B1/Atomic dry plan."""
...
from project.run_scripts.ode_bf.p1r27_joint_wclf import P1R27_INSTRUCTION_ID, P1R27_METHOD_ID
from project.run_scripts.ode_bf.p1r27_joint_wclf_panel import expected_p1r27_result_name, forecast_p1r27_panel


def build_plan(source_head: str, phase: str, *, repository_root: Path = REPO_ROOT) -> dict[str, object]:
    if phase not in ("smoke", "production"):
        raise ValueError("P1R27 dry phase differs")
    roles = (
        ("P1R27_B1_RS_PAIR", "P1R27_B1_BG_PAIR")
        if phase == "smoke"
        else ("P1R27_B10_RS_PAIR", "P1R27_B10_BG_PAIR")
    )
    jobs: list[dict[str, object]] = []
    locks = repository_root / "project/run_scripts/ode_bf/locks"
    for alias in ("llama3-8b-inst", "qwen2.5-7b-inst"):
        forecast = forecast_p1r27_panel(
        ...
        for role in roles:
            jobs.append({
                ...
                "trajectory_count": 2,
                "result_name": expected_p1r27_result_name(alias, role),
                "gpu": 1,
                "cpu": 8,
                "memory_mib": 65_000,
                "time": "23:59:00",
    ...
    return {
        "schema": "ode-edit-s05-p1r27-dry-plan/v1",
