#!/usr/bin/env python3
"""Fail-closed launcher for P1R54 FZ C3 sequential B1->B10."""
...
RUN_TOKEN = "p1r54-fz-c3-sequential-10xb100-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("P1R54 FZ sequential pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema")
        != "ode-edit-s05-p1r54-fz-c3-sequential-final-pre-gpu/v1"
        ...
        raise ODEBFContractError("P1R54 FZ sequential pre-GPU content differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            raise ODEBFContractError("P1R54 FZ sequential source/output differs")
        final = _load_final(
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("P1R54 FZ sequential deployment differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r54-fz-c3-sequential-job-timing/v1",
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r54-fz-c3-sequential-failure/v1",
        )
        print(
            json.dumps(
                {
