...
def update_figure(rows,labels=None):
    show=lambda arm:(labels or {}).get(arm,arm)
    ...
    for ax,method in zip(axes[0],METHODS):
        for i,var in enumerate(VARIANTS):
            ...
            ax.bar(np.arange(2)+(i-1)*.22,vals,.22,label=show(method+'_'+var) if labels else var,color=COLORS[var])
...
def generate(out,dest,labels=None,command=None):
    show=lambda arm:(labels or {}).get(arm,arm)
    legend=lambda method,var:show(method+'_'+var) if labels else var
    ...
    def write(fig,name,used,caption):
        ...
        manifest.append(dict(path=path.name,sha256=sha(path),bytes=path.stat().st_size,source_sha256=sha(Path(__file__)),inputs=[dict(path=n+'.csv',sha256=sha(out/(n+'.csv'))) for n in used],
                             command=command or 'python -m project.run_scripts.blue_lifelong_analysis.plots --out PACKAGE --dest NEW_FIGURE_DIRECTORY',backend='Agg',DPI=DPI,figsize_inches=size,axes=axes_spec,colors=COLORS,seed='NO_RANDOMNESS',ordering='methods MEMIT/AlphaEdit; variants ORIGINAL/L4_ONLY/L8_ONLY',display_labels=labels or {},caption=caption+(' All measured arms are blue=True; base methods are not plotted.' if labels else '')))
    if labels:
        fig,axes=plt.subplots(2,1,figsize=(12,8),squeeze=False)
        for ax,method in zip(axes.flat,METHODS):
            aa=[method+'_'+v for v in VARIANTS];xs=np.arange(3)
            for j,tag in enumerate(MULT):ax.bar(xs+(j-1)*.24,[100*float(selected(inputs['final-summary'],a)[0][tag+'_rate']) for a in aa],.24,label=tag)
            ax.set_xticks(xs,[show(a) for a in aa]);ax.set_ylim(0,100);ax.set_ylabel('Canonical preference success (%)');ax.legend();ax.set_title(method+' BLUE: final W100 on all 10,000 requests')
    else:
        ...
        ax.set_xticks(xs,[show(a) for a in ARMS],rotation=22,ha='right');ax.set_ylim(0,100);ax.set_ylabel('Canonical preference success (%)');ax.legend();ax.set_title('Final W100: all 10,000 edited requests')
    ...
    for j,method in enumerate(METHODS):
        for k,tag in enumerate(MULT):
            ax=axes[j,k]
            for v in VARIANTS:
                rr=selected(cum,method+'_'+v,tag);ax.plot([100*int(r['batch']) for r in rr],[100*float(r['rate']) for r in rr],'-o',ms=3,label=legend(method,v),color=COLORS[v])
    ...
    for ax,arm in zip(axes.flat,PANEL_ARMS):
        ...
        ax.set_title(show(arm));ax.set_ylim(0,100);ax.set_xlim(1,100);ax.set_xlabel('Sequential batch');ax.set_ylabel('RS (%)');ax.legend(fontsize=7)
    ...
    for ax,arm in zip(axes.flat,PANEL_ARMS):
        ...
        im=ax.imshow(a,vmin=0,vmax=100,aspect='auto',interpolation='nearest',cmap='viridis');ax.set_title(show(arm));ax.set_yticks(range(len(SCHEDULE)),SCHEDULE);ax.set_xticks([0,24,49,74,99],[1,25,50,75,100]);ax.set_xlabel('Write cohort (100 requests)');ax.set_ylabel('Checkpoint batch')
    fig.colorbar(im,ax=axes.ravel().tolist(),label='RS (%)',shrink=.7)
    write(fig,'cohort-retention-heatmap',['cohort-retention'],'Each occupied cell100 requests; rows12 frozen checkpoints; future/unseen cohorts masked, no interpolation/imputation. Panel rows MEMIT/AlphaEdit, columns BLUE(L4+L8)/BLUE_L4_ONLY/BLUE_L8_ONLY.' if labels else 'Each occupied cell100 requests; rows12 frozen checkpoints; future/unseen cohorts masked, no interpolation/imputation. Panel rows MEMIT/AlphaEdit, columns original/L4/L8.')
        ...
        for j,method in enumerate(METHODS):
            for k,tag in enumerate(MULT):
                ...
                for v in VARIANTS:
                    ...
                    for st,style in [('median','-'),('p90','--')]:ax.plot(x,[float(r[f'{side}_nll_request_{st}']) for r in rr],style,label=legend(method,v)+' '+st,color=COLORS[v])
    ...
    for tag in ['RS','NS']:
        ...
        for ax,arm in zip(axes.flat,PANEL_ARMS):
            ...
            ax.set_ylim(0,100);ax.set_xlim(0,10000);ax.set_title(show(arm)+' '+tag);ax.set_xlabel('Seen requests');ax.set_ylabel('Success (%)');ax.legend(fontsize=7)
    ...
    write(update_figure(inputs['layer-updates'],labels),'layer-wise-update-magnitude',['layer-updates'],'Each value averages100 observed batch-net update norms; unselected layer zero by support definition. Magnitude not squared norm/native action; no uniform reference line.')
    ...
    ax.bar(np.arange(6),edit,label='Native writer (inclusive)');ax.bar(np.arange(6),ev,bottom=edit,label='Endpoint evaluation');ax.set_xticks(np.arange(6),[show(a) for a in ARMS],rotation=22,ha='right');ax.set_ylabel('Recorded wall time (hours)');ax.set_title('Writer and evaluator accounting');ax.legend()
