#!/usr/bin/env python3
"""Fail-closed launcher for one P1R54 energy-free Local-Z B100 arm."""
...
RUN_TOKEN = "p1r54-energyfree-localz-llama-b100-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("P1R54 final pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema") != "ode-edit-s05-p1r54-energyfree-localz-final-pre-gpu/v1"
        ...
        or value.get("array_concurrency") != 2
        or value.get("source_manifest") != _source_manifest(source_head, source_tree)
    ):
        raise ODEBFContractError("P1R54 final pre-GPU content differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            raise ODEBFContractError("P1R54 source/output namespace differs")
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("P1R54 deployment identity differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r54-energyfree-localz-job-timing/v1",
        ...
        result.update(
            {
                "source_manifest_sha256": _atomic_write_once(
                    args.output_root / "source-manifest.json", _source_manifest(head, tree)
                ),
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r54-energyfree-localz-failure/v1",
        )
        print(
            json.dumps(
                {
