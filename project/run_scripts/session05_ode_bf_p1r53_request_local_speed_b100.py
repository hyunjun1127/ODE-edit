#!/usr/bin/env python3
"""Fail-closed launcher for one P1R53 request-local-speed B100 arm."""
...
RUN_TOKEN = "p1r53-request-local-speed-llama-b100-v1"


def _load_final(path: Path, *, source_head: str, source_tree: str) -> dict[str, object]:
    if path.is_symlink() or not path.is_file():
        raise ODEBFContractError("P1R53 final pre-GPU receipt differs")
    value = json.loads(path.read_bytes())
    identity = value.pop("identity_sha256", None)
    if (
        identity != canonical_hash(value)
        or value.get("schema") != "ode-edit-s05-p1r53-request-local-speed-final-pre-gpu/v1"
        ...
        raise ODEBFContractError("P1R53 final pre-GPU content differs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (
            ...
            raise ODEBFContractError("P1R53 source/output namespace differs")
        final = _load_final(
        ...
        if deployment.identity_sha256 != final["deployment_identity"]:
            raise ODEBFContractError("P1R53 deployment identity differs")
        result = run_p1(
            repo_root=REPO_ROOT,
        ...
        timing: dict[str, object] = {
            "schema": "ode-edit-s05-p1r53-request-local-speed-job-timing/v1",
        ...
        failure_sha, failure = write_p1_failure_once(
            ...
            repo_root=REPO_ROOT,
            instruction_id=INSTRUCTION_ID,
            failure_schema="ode-edit-s05-p1r53-request-local-speed-failure/v1",
        )
        print(
            json.dumps(
                {
