# GH → 모든 SH: 별도 명시 없는 실험 checkpoint 저장 금지 기본값

Instruction ID: `ODEEDIT-ALL-SH-DEFAULT-NO-CHECKPOINT-20260919-V1`
Nonce prefix: `ODEEDIT-GH-ALL-SH-NO-CP-DEFAULT-20260919-R1`
...
GH session `01a04939-8873-7673-8dca-4c7fc5e31af0`, repo `hyunjun1127/ODE-edit`.
SH1 session `01a04939-f93a-7b50-bca0-65438eab2062`, 현재 repo CWD `/mnt/raid5/janghj/ODE-edit`;
SH2 session `01a0493a-074c-7f91-9a13-769116326fef`, CWD `/mnt/raid5/janghj/ODE-edit`;
SH4 session `01a04939-b5c7-7a03-ba2d-ef3343d62cfd`, CWD `/data/janghj/ODE-edit`.
실제 registry/session/CWD/repo 결속 후 처리한다.

정책 적용 기록은 자신의 ignored `local/state/user-policies/default-no-checkpoints-20260919/**` 및 필요시
`messages/acks/serverN/2026-09-19-default-no-checkpoints.md`,
`audits/servers/serverN/2026-09-19-default-no-checkpoints/**`만 쓸 수 있다.
현재 승인 task의 아직 미제출 source 설정은 기존 write envelope 안에서만 반영한다.
새 Slurm/model/eval/GPU 작업 권한0, GPU cap 변경0, 원자료 삭제/전송0.
정책 ACK·own-scope 기록의 nonforce 게시만 허용하며 독립 새 과학 task를 만들지 않는다.
NO_BROADCAST_NOT_REQUIRED. 특별 감사/GPU 점검 없이 ACK 및 명시 예외 목록만 회신한다.
