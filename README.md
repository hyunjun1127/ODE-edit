...
- `local/`: ignored server-local raw artifacts, Slurm logs, run logs,
  ...
  artifacts by rsync fan-out, not Git.
- `messages/`: human-readable Korean coordination. `messages/README.md` is the
  cross-server index; all active server-heads read it and write their own ack.
...
- `scripts/`: repo coordination helpers, including heartbeat, sync, task
  audit/status/closure, worker claim/finish, and artifact fan-out.
- `subagents/`: blue/red role specifications.
- `transfers/`: manual-exception transfer records. Ordinary experiment
  artifact fan-out uses `scripts/rsync-artifact-fanout.sh` and does not need
  per-transfer user approval.
