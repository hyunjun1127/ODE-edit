# Stage 0 Diagnostics — Global Evidence Index

- 작성 시각: 2026-07-30
- 작성 agent: head-server1-gh (global-head)
- 상태: 미실행
- proposal link: `project/proposals/sections/01-stage-0-diagnostic.md`
- canonical plan: `plans/global/2026-07-30-stage0-diagnostic.md`
...
현재 등록·onboarding 완료된 server, 실행 script, task, run artifact가 없어
실험은 아직 실행하지 않았다. 이 문서는 결과 보고서가 아니라 Stage 0 evidence
landing page다. server별 실제 결과는 protocol 소유권에 따라
`experiment-reports/servers/<server>/`에, raw evidence는 `local/`에 남긴 뒤
이 문서에서 curate한다.

## Claim boundary

현재 claim은 **hypothesis only**다. 향후 Stage 0가 통과해도 `diagnostic only`이며,
