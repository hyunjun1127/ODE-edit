...
embedded=report.read_text().split('<!-- SWEEP_CANDIDATES_BEGIN -->')[1].split('<!-- SWEEP_CANDIDATES_END -->')[0]
table=[line.strip().strip('|').split('|') for line in embedded.splitlines() if line.startswith('| JV-')]
assert len(table)==len(configs)==10
for cells,cfg in zip(table,configs,strict=True):
    cells=[x.strip() for x in cells]
    assert cells[0]==cfg['candidate_id']
    for index,key in [(2,'lambda_response'),(3,'T'),(4,'N'),(5,'h'),(6,'main_JVP_per_B100'),(7,'main_JVP_per_1000_chain')]:
        assert float(cells[index])==float(cfg[key]),(cells[0],key)
    assert cells[8]==('조건부·미제출' if cfg['candidate_id']=='JV-RES-N16' else '미제출')
...
validation=dict(status='PASS_PUBLICATION_CPU_REPRODUCTION_AND_LOCAL_REPORT_LINKS',
    combined_report_candidate_rows_verified=len(table),
