# AlphaEdit JV sequential 1,000: GH 통합 분석 및 Sweep 후보

**통합 v2**: 2026-09-07 사용자 지시에 따라 독립 분석과 sweep 후보표를 이 문서 한 곳에 합쳤다. 아래 §13에는 개별 configuration 10개의 전체 후보표, 원본 compute-z 고정 범위, 실행 순서와 해석 주의사항이 모두 포함되어 있다. CSV/JSON은 재현·검산용 부록이며 별도 설명 문서를 읽을 필요는 없다. 기존 로컬 v1은 보존했다. 수치·후보 설정은 변경하지 않았으며 신규 실험은 제출하지 않았다.
...
후보 전체를 아래 표에 직접 통합했다. [sweep-candidates.csv](sweep-candidates.csv)와 [sweep-candidate-policy.json](sweep-candidate-policy.json)은 동일 내용의 기계 판독·검산 부록이다. 이는 **후보 산출이지 실행 승인/제출/선택된 최적값이 아니다.** 코드는 [sweep_candidates.py](sweep_candidates.py)로 재현한다. 원본 AlphaEdit z 최적화는 모두 고정한다.
...
모든 행은 Llama/Qwen 양 모델의 JV_NATIVE에 적용하는 후보이며 N0, 원본 AlphaEdit compute-z·P·L2·history 정책을 유지한다. 아래 JVP 수는 active5-layer 기준 main JVP만 센 값이다. Entry qref용 추가5 layer solves, z 최적화, evaluator, history finalization 비용은 별도다. **전 행 미제출/HOLD**이며 N16만 조건부 refinement다.

<!-- SWEEP_CANDIDATES_BEGIN -->
| 후보 ID | 비교 목적 | λ | T | N | h | JVP/B100 | JVP/1000 chain | 상태 |
|---|---|---:|---:|---:|---:|---:|---:|---|
| JV-BASE | 공통 기준 | 0.1 | 2 | 4 | 0.5 | 20 | 200 | 미제출 |
| JV-LAM-001 | λ operating point | 0.01 | 2 | 4 | 0.5 | 20 | 200 | 미제출 |
| JV-LAM-00316 | λ operating point | 0.03162277660168379 | 2 | 4 | 0.5 | 20 | 200 | 미제출 |
| JV-LAM-0316 | λ operating point | 0.31622776601683794 | 2 | 4 | 0.5 | 20 | 200 | 미제출 |
| JV-LAM-1 | λ operating point | 1 | 2 | 4 | 0.5 | 20 | 200 | 미제출 |
| JV-RES-N2 | 고정 T 해상도 | 0.1 | 2 | 2 | 1 | 10 | 100 | 미제출 |
| JV-RES-N8 | 고정 T 해상도 | 0.1 | 2 | 8 | 0.25 | 40 | 400 | 미제출 |
| JV-RES-N16 | 고정 T 해상도 | 0.1 | 2 | 16 | 0.125 | 80 | 800 | 조건부·미제출 |
| JV-HOR-T1 | 고정 h horizon | 0.1 | 1 | 2 | 0.5 | 10 | 100 | 미제출 |
| JV-HOR-T4 | 고정 h horizon | 0.1 | 4 | 8 | 0.5 | 40 | 400 | 미제출 |
<!-- SWEEP_CANDIDATES_END -->
