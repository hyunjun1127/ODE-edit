...
Cell 상태: `{"BLOCKED": 25, "PASS": 5, "FAILED": 1}`. Geometry는 actual W0+12개 W/M, 동일 random vector256개를 사용하는11개 구간을 검사했다. C00/C01 등 모델·writer·evaluator gate의 성공 여부는 cell 표의 실제 status만 따른다.
...
|hypothesis|status|basis|provisional|
|---|---|---|---|
|H1|SUPPORTED|이 단일 stream의 저장된 at-write/같은 cohort 전이에서 RS·PS와 NS의 시간적 양상이 다름. B100 all-case 조건부 loss는55/9993,423/19403,11547/72505이며 conflict-free 및 strict 전이를 별도 보고. 누적 history의 인과효과나 일반 capacity 판정 아님.|False|
|H2|UNRESOLVED|C01 실패로 사전 지정 operator/demand/activation 의존 분석 미실행. 구현 또는 작은 B1 재현 오차를 기전 증거로 대체하지 않음.|False|
|H3|UNRESOLVED|C01 실패로 사전 지정 operator/demand/activation 의존 분석 미실행. 구현 또는 작은 B1 재현 오차를 기전 증거로 대체하지 않음.|False|
|H4|UNRESOLVED|C01 실패로 사전 지정 operator/demand/activation 의존 분석 미실행. 구현 또는 작은 B1 재현 오차를 기전 증거로 대체하지 않음.|False|
...
|stage|job|scheduler_state|allocated_gpu_seconds|runner_seconds|numerical_status|
|---|---|---|---|---|---|
|original_gate|51071|FAILED|230|227.417|FAILED_TECHNICAL_P_HEADER; NS parity FAILED|
|technical_continuation|51116|COMPLETED|39|35.0469|FAILED_NUMERICAL_CONTRACT|
...
```bash
python -B -m project.run_scripts.checkpoint_mechanism_audit.reporting --results /mnt/raid5/janghj/ODE-edit/local/checkpoint-mechanism-audit/20260920-v1/attempt-v1/results --output <NEW_EMPTY_OUTPUT> --cell-statuses /mnt/raid5/janghj/ODE-edit/local/checkpoint-mechanism-audit/20260920-v1/attempt-v1/publication-input-r1/cell-statuses.json --context /mnt/raid5/janghj/ODE-edit/local/checkpoint-mechanism-audit/20260920-v1/attempt-v1/publication-input-r1/report-context.json --final
```
...
|---|---|---|---|
|A00|input_binding|PASS|Verified independent component receipt; not C01 promotion|
|A01|functional_ledger|PASS|Verified independent component receipt; not C01 promotion|
|B00|weight_history|PASS|Verified independent component receipt; not C01 promotion|
|C00|prefix_parity|PASS|Verified independent component receipt; not C01 promotion|
|C01|B1_reproduction|FAILED|NS row NLL/margin and four M1 Gram elements exceed unchanged contract; other B1 subchecks retained|
|C02|key_bank|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O000|history_operator_pilot|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O001|history_operator_pilot|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O010|history_operator_pilot|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O100|history_operator_pilot|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O005|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O020|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O030|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O040|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O050|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O060|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O070|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O080|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|O090|history_operator_extension|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N001|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N002|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N006|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N011|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N021|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N051|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|N091|native_demand_modes|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|E00|algebra_controls|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|F00|activation_bookkeeping|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|G001_010|suffix_mechanism|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|G050_100|suffix_mechanism|BLOCKED|Dependency C01 NUMERICAL_CONTRACT_UNRESOLVED; not submitted; no numerical fallback|
|Z00|report|PASS|Terminal collection report only; dependent measurements remain BLOCKED|
