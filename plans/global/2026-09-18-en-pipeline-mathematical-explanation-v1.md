...
$$
Z_t=\operatorname{NativeZOpt}_{\ell=4}(W_{t-1},\mathcal E_t),
$$
$$
\Delta W_{N,t}
=\operatorname{NativeWrite}_4
...
$$
K_{E,t}=\operatorname{concat}_{x\in\mathcal X_{E,t},\ j\in\mathrm{valid}(x)}k_j(x).
$$
...
\(P_*=VV^\top\), \(V^\top V=I_r\), \(r=\operatorname{rank}(P_*)\)이고 \(J_t=V^\top K_{E,t}\)라 두면
...
$$
q_t=r-\operatorname{rank}(V^\top K_{E,t}),
...
$$
W_{t,j}^{\mathrm{cand}}=\operatorname{FP32}(W_{N,t}+D_{t,j}^{\mathrm{ideal}}),\qquad
D_{t,j}^{\mathrm{actual}}=\operatorname{FP64}(W_{t,j}^{\mathrm{cand}})-\operatorname{FP64}(W_{N,t}).
$$
...
W_t=
\begin{cases}
W_{t,j}^{\mathrm{cand}},&\text{objective·수치·품질 조건을 모두 통과한 후보를 수용할 때},\\
W_{N,t},&\text{정상적인 탐색 실패로 native fallback할 때}.
\end{cases}
$$

기술 오류를 정상 fallback으로 감추지는 않는다. 다음 batch는 이 \(W_t\)에서 시작한다. 공식 P/N은 후보 선택 이후 observer이며 gradient·step 선택에는 쓰지 않는다.
...
q_{\le t}=r-\operatorname{rank}(V^\top K_{\le t})
$$
