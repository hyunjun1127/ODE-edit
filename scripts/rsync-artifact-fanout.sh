...
usage() {
  cat <<'USAGE'
Usage:
  scripts/rsync-artifact-fanout.sh [--dry-run] [--target SERVER] LOCAL_PATH

Fan out one Git-excluded local artifact path to configured active servers.
...
while IFS=$'\t' read -r server alias repo_path rest; do
  ...
  echo "Rsync ${source_rel} -> ${server}:${dest}"
